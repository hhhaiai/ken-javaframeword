...
rl.addAutoDecoArea("mainForm", "queryDialogContent", "protocolTrafficList");
...
function query(){
    ...
    mainForm.action = "${rootPath}protocolTraffic_list";
    mainForm.submit(); 
}
...
function exportPdf() {
    var mainForm = document.mainForm;
	mainForm.action = "${rootPath}protocolTraffic_dumpPDF?method=protocolTraffic_list&isPdf=1";
	mainForm.submit();
}

// 流量趋势
function showFlowTrend(protocolId) {
	var mainForm = document.mainForm;
	mainForm.action ='${rootPath}trend/protocolTrafficTrend_list?protocolId=' + protocolId;
	mainForm.submit();
}
</script>
...
<div class="std_info">
    <div class="page_wrapper limit_770">
        ...
        <div class="report">
            ...
            <table id="protocolTrafficList" class="data_list" width=100% cellSpacing=0 cellPadding=0 border=0>
            <tr>
                <th>&nbsp;</th>
                <th>协议</th>
                <th>总流量(MB)</th>
            ...
            <tr>
                ...
                <s:param value="dbModels['default'][#dbModel.index]['src_protocol_total'] / 1 + dbModels['default'][#dbModel.index]['dst_protocol_total'] / 1"/>
                </s:text>
                </td>
                ...
                <td><s:property value="dbModels['default'][#dbModel.index]['dst_protocol_total']" /></td>
                <td><span ctype="ProgressBar" barSkin="green" progress="<s:property value="dbModels['default'][#dbModel.index]['dst_protocol_percentage']" />"></span></td>
                <td><s:property value="dbModels['default'][#dbModel.index]['dst_protocol_percentage']" />%</td>
                <td>
                <img style="cursor:pointer" height="14" width="14" src="${rootPath}resource/image/icons/trend.png" border="0" onclick="showFlowTrend('<s:property value="dbModels['default'][#dbModel.index]['protocol_id']" />')">
                </td>
            </tr>
            </s:iterator>
            </table>
