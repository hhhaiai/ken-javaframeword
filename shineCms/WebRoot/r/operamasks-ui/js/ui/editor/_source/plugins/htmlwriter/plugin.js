...
OMEDITOR.plugins.add( 'htmlwriter' );
 ...
 */
OMEDITOR.htmlWriter = OMEDITOR.tools.createClass(
{
	base : OMEDITOR.htmlParser.basicWriter,
	...
	$ : function()
	{
		...
		for ( var e in OMEDITOR.tools.extend( {}, dtd.$nonBodyContent, dtd.$block, dtd.$listItem, dtd.$tableContent ) )
		{
			this.setRules( e,
				{
					indent : 1,
	...
	proto :
	{
		...
		attribute : function( attName, attValue )
		{
			...
			if ( typeof attValue == 'string' )
			{
				...
				attValue = OMEDITOR.tools.htmlEncodeAttr( attValue );
			}
		...
		text : function( text )
		{
			...
			{
				this.indentation();
				!this._.inPre  && ( text = OMEDITOR.tools.ltrim( text ) );
			}
		 ...
		 *
		 * By default, all elements available in the {@link OMEDITOR.dtd.$block),
		 * {@link OMEDITOR.dtd.$listItem} and {@link OMEDITOR.dtd.$tableContent}
