...
OMEDITOR.plugins.add( 'iframedialog',
{
	requires : [ 'dialog' ],
	onLoad : function()
	{
		 ...
		 * @example
		 */
		OMEDITOR.dialog.addIframe = function( name, title, src, minWidth, minHeight, onContentLoad, userDefinition )
		{
			var element =
			{
				type : 'iframe',
		...
		(function()
		{
			...
			var iframeElement = function( dialog, elementDefinition, htmlList )
			{
				...
				var _ = ( this._ || ( this._ = {} ) ),
					contentLoad = elementDefinition.onContentLoad && OMEDITOR.tools.bind( elementDefinition.onContentLoad, this ),
					cssWidth = OMEDITOR.tools.cssLength( elementDefinition.width ),
					cssHeight = OMEDITOR.tools.cssLength( elementDefinition.height );
				_.frameId = OMEDITOR.tools.getNextId() + '_iframe';
				...
				if ( typeof( elementDefinition.onContentLoad ) == 'function' )
					attributes.onload = 'OMEDITOR.tools.callFunction(%1);';

				OMEDITOR.ui.dialog.uiElement.call( this, dialog, elementDefinition, myHtml, 'iframe',
						{
							width : cssWidth,
							height : cssHeight
						}, attributes, '' );
				...
				dialog.on( 'show', function()
					{
						var iframe = OMEDITOR.document.getById( _.frameId ),
							parentContainer = iframe.getParent(),
							callIndex = OMEDITOR.tools.addFunction( contentLoad ),
							html = myHtml.replace( '%1', callIndex ).replace( '%2', OMEDITOR.tools.htmlEncode( elementDefinition.src ) );
						parentContainer.setHtml( html );
					} );
			};

			iframeElement.prototype = new OMEDITOR.ui.dialog.uiElement;

			OMEDITOR.dialog.addUIElement( 'iframe',
				{
