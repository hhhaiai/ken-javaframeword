...
(function()
{
	...
	var checkHasBlock = function( element )
	{
		...
		for ( var i = 0, count = childNodes.count() ; i < count ; i++ )
		{
			...
			if ( child.type == OMEDITOR.NODE_ELEMENT && OMEDITOR.dtd.$block[ child.getName() ] )
				return true;
		}
	 ...
	 * @class
	 */
	OMEDITOR.dom.elementPath = function( lastNode )
	{
		var block = null;
		var blockLimit = null;
		var elements = [];
		...
		while ( e )
		{
			if ( e.type == OMEDITOR.NODE_ELEMENT )
			{
				if ( !this.lastElement )
					this.lastElement = e;

				var elementName = e.getName();
				if ( OMEDITOR.env.ie && e.$.scopeName != 'HTML' )
...
OMEDITOR.dom.elementPath.prototype =
{
	/**
	 * Compares this element path with another one.
	 * @param {OMEDITOR.dom.elementPath} otherPath The elementPath object to be
