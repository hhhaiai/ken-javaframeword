...
(function()
{
	...
	var whitespaces = OMEDITOR.dom.walker.whitespaces(),
		bookmarks = OMEDITOR.dom.walker.bookmark(),
		nonEmpty = function( node ){ return !( whitespaces( node ) || bookmarks( node ) ); };

	OMEDITOR.plugins.list = {
		/*
		...
		listToArray : function( listNode, database, baseArray, baseIndentLevel, grandparentNode )
		{
			...
			for ( var i = 0, count = listNode.getChildCount() ; i < count ; i++ )
			{
				...
				if( listItem.type == OMEDITOR.NODE_ELEMENT && listItem.getName() in OMEDITOR.dtd.$list )
					OMEDITOR.plugins.list.listToArray( listItem, database, baseArray, baseIndentLevel + 1 );
				...
				if ( database )
					OMEDITOR.dom.element.setMarker( database, listItem, 'listarray_index', baseArray.length );
				baseArray.push( itemObj );

				for ( var j = 0, itemChildCount = listItem.getChildCount(), child; j < itemChildCount ; j++ )
				{
					child = listItem.getChild( j );
					if ( child.type == OMEDITOR.NODE_ELEMENT && listNodeNames[ child.getName() ] )
						// Note the recursion here, it pushes inner list items with
						// +1 indentation in the correct order.
						OMEDITOR.plugins.list.listToArray( child, database, baseArray, baseIndentLevel + 1, itemObj.grandparent );
					else
		...
		arrayToList : function( listArray, database, baseIndex, paragraphMode, dir )
		{
			...
			var doc = listArray[ baseIndex ].parent.getDocument(),
				retval = new OMEDITOR.dom.documentFragment( doc ),
				rootNode = null,
				currentIndex = baseIndex,
				indentLevel = Math.max( listArray[ baseIndex ].indent, 0 ),
				currentListItem = null,
				orgDir,
				paragraphName = ( paragraphMode == OMEDITOR.ENTER_P ? 'p' : 'div' );
			while ( 1 )
			{
				...
				else if ( item.indent == Math.max( indentLevel, 0 ) + 1 )
				{
					...
					var currDir = listArray[ currentIndex - 1 ].element.getDirection( 1 ),
						listData = OMEDITOR.plugins.list.arrayToList( listArray, null, currentIndex, paragraphMode,
						currDir != orgDir ? orgDir: null );

					// If the next block is an <li> with another list tree as the first
					// child, we'll need to append a filler (<br>/NBSP) or the list item
					// wouldn't be editable. (#6724)
					if ( !currentListItem.getChildCount() && OMEDITOR.env.ie && !( doc.$.documentMode > 7 ))
				...
				else if ( item.indent == -1 && !baseIndex && item.grandparent )
				{
					...
					else
					{
						...
						if ( dir || item.element.hasAttributes() || paragraphMode != OMEDITOR.ENTER_BR )
						{
							...
							if ( !dir && paragraphMode == OMEDITOR.ENTER_BR && !currentListItem.hasAttributes() )
								currentListItem = new OMEDITOR.dom.documentFragment( doc );
						}
						else
							currentListItem = new OMEDITOR.dom.documentFragment( doc );
					}

					if ( currentListItem.type == OMEDITOR.NODE_ELEMENT )
					{
					...
					if ( currentListItem.type == OMEDITOR.NODE_DOCUMENT_FRAGMENT
						 && currentIndex != listArray.length - 1 )
					{
						var last = currentListItem.getLast();
						if ( last && last.type == OMEDITOR.NODE_ELEMENT
						...
						if ( !( last = currentListItem.getLast( nonEmpty )
							&& last.type == OMEDITOR.NODE_ELEMENT
							&& last.getName() in OMEDITOR.dtd.$block ) )
						{
							...
							currentListItem.$.firstChild )
					{
						...
						if ( firstChild.type == OMEDITOR.NODE_ELEMENT && firstChild.isBlockBoundary() )
						{
							var tmp = new OMEDITOR.dom.documentFragment( doc );
					...
					if ( !OMEDITOR.env.ie && ( currentListItemName == 'div' || currentListItemName == 'p' ) )
			...
			if ( database )
			{
				...
				while ( currentNode )
				{
					if ( currentNode.type == OMEDITOR.NODE_ELEMENT )
						OMEDITOR.dom.element.clearMarkers( database, currentNode );
	...
	function onSelectionChange( evt )
	{
			  ...
			  && !element.equals( blockLimit ); i++ )
		{
			if ( listNodeNames[ elements[ i ].getName() ] )
				return this.setState( this.type == elements[ i ].getName() ? OMEDITOR.TRISTATE_ON : OMEDITOR.TRISTATE_OFF );
		}

		return this.setState( OMEDITOR.TRISTATE_OFF );
	}
	...
	function changeListType( editor, groupObj, database, listsCreated )
	{
		...
		var listArray = OMEDITOR.plugins.list.listToArray( groupObj.root, database ),
			selectedListItems = [];
		...
		for ( var i = 0 ; i < groupObj.contents.length ; i++ )
		{
			...
			OMEDITOR.dom.element.setMarker( database, itemNode, 'list_item_processed', true );
		}
		...
		var newList = OMEDITOR.plugins.list.arrayToList( listArray, database, null, editor.config.enterMode );
		...
		function compensateBrs( isStart )
		{
			if ( ( boundaryNode = docFragment[ isStart ? 'getFirst' : 'getLast' ]() )
				 ...
				 && ( siblingNode = groupObj.root[ isStart ? 'getPrevious' : 'getNext' ]
				      ( OMEDITOR.dom.walker.whitespaces( true ) ) )
	...
	listCommand.prototype = {
		exec : function( editor )
		{
			...
			if ( this.state == OMEDITOR.TRISTATE_OFF )
			{
				...
				{
					config.enterMode == OMEDITOR.ENTER_BR ?
						body.appendBogus() :
						ranges[ 0 ].fixBlock( 1, config.enterMode == OMEDITOR.ENTER_P ? 'p' : 'div' );
			...
			while ( ( range = rangeIterator.getNextRange() ) && ++index )
			{
				...
				if ( startNode.type == OMEDITOR.NODE_ELEMENT && startNode.getName() == 'td' )
					range.setStartAt( boundaryNodes.startNode, OMEDITOR.POSITION_AFTER_START );

				if ( endNode.type == OMEDITOR.NODE_ELEMENT && endNode.getName() == 'td' )
					range.setEndAt( boundaryNodes.endNode, OMEDITOR.POSITION_BEFORE_END );

				var iterator = range.createIterator(),
					block;

				iterator.forceBrBreak = ( this.state == OMEDITOR.TRISTATE_OFF );
				...
				while ( ( block = iterator.getNextParagraph() ) )
				{
					...
					if( block.getCustomData( 'list_block' ) )
						...
						OMEDITOR.dom.element.setMarker( database, block, 'list_block', 1 );

					var path = new OMEDITOR.dom.elementPath( block ),
					...
					for ( var i = pathElementsCount - 1; i >= 0 && ( element = pathElements[ i ] ); i-- )
					{
							 ...
							 && blockLimit.contains( element ) )     // Don't leak outside block limit (#3940).
						{
							...
							else
							{
								...
								OMEDITOR.dom.element.setMarker( database, element, 'list_group_object', groupObj );
							}
							processedFlag = 1;
							break;
						}
					...
					else
					{
						...
						OMEDITOR.dom.element.setMarker( database, root, 'list_group_object_' + index, groupObj );
						listGroups.push( groupObj );
					}
				}
			}
			...
			while ( listGroups.length > 0 )
			{
				...
				else if ( this.state == OMEDITOR.TRISTATE_ON && listNodeNames[ groupObj.root.getName() ] )
			...
			for ( i = 0 ; i < listsCreated.length ; i++ )
			{
				...
				( mergeSibling = function( rtl ){
					...
					var sibling = listNode[ rtl ?
						'getPrevious' : 'getNext' ]( OMEDITOR.dom.walker.whitespaces( true ) );
		...
		return function( listItem )
		{
					...
					|| nodeBefore.value && ( tailNbspmatch = nodeBefore.value.match( tailNbspRegex ) ) ) )
			{
				...
				if ( !( tailNbspmatch && tailNbspmatch.index ) && fillerNode == children[ 0 ] )
					children[ 0 ] = ( isHtmlFilter || OMEDITOR.env.ie ) ?
					                 new OMEDITOR.htmlParser.text( '\xa0' ) :
									 new OMEDITOR.htmlParser.element( 'br', {} );
	...
	OMEDITOR.plugins.add( 'list',
	{
		init : function( editor )
		{
			...
			editor.on( 'selectionChange', OMEDITOR.tools.bind( onSelectionChange, numberedListCommand ) );
			editor.on( 'selectionChange', OMEDITOR.tools.bind( onSelectionChange, bulletedListCommand ) );
		},
