...
OMEDITOR.dialog.add( 'colordialog', function( editor )
	{
		// Define some shorthands.
		var $el = OMEDITOR.dom.element,
			$doc = OMEDITOR.document,
			$tools = OMEDITOR.tools,
		...
		{
			if ( ! ( evt instanceof OMEDITOR.dom.event ) )
				evt = new OMEDITOR.dom.event( evt );
		...
		{
			if ( ! ( event instanceof OMEDITOR.dom.event ) )
				event = event.data;
		...
		var onMouseout = $tools.addFunction( clearHighlight ),
			...
			onClickHandler = OMEDITOR.tools.addFunction( onClick ),
			onFocus = updateHighlight,
			onBlur = clearHighlight;

		var onKeydownHandler = OMEDITOR.tools.addFunction( function( ev )
		{
		...
		function createColorTable()
		{
			...
			function appendColorCell( targetRow, color )
			{
				...
				cell.append( OMEDITOR.dom.element.createFromHtml(
						'<a href="javascript: void(0);" role="option"' +
						' aria-posinset="' + index + '"' +
						' aria-setsize="' + 13 * 18 + '"' +
						' style="cursor: pointer;display:block;width:100%;height:100% " title="'+ OMEDITOR.tools.htmlEncode( color )+ '"' +
						' onkeydown="OMEDITOR.tools.callFunction( ' + onKeydownHandler + ', event, this )"' +
						' onclick="OMEDITOR.tools.callFunction(' + onClickHandler + ', event, this ); return false;"' +
						' tabindex="-1"><span class="cke_voice_label">' + color + '</span>&nbsp;</a>', OMEDITOR.document ) );
			}
					...
					[
						{
							...
							[
								{
									type : 'html',
									html :	'<table role="listbox" aria-labelledby="' + tableLabelId + '" onmouseout="OMEDITOR.tools.callFunction( ' + onMouseout + ' );">' +
											( !OMEDITOR.env.webkit ? html : '' ) +
										'</table><span id="' + tableLabelId + '" class="cke_voice_label">' + lang.options +'</span>',
									onLoad : function()
									{
										var table = OMEDITOR.document.getById( this.domId );
										table.on( 'mouseover', updateHighlight );
										// In WebKit, the table content must be inserted after this event call (#6150)
										OMEDITOR.env.webkit && table.setHtml( html );
									},
									focus: function()
									{
