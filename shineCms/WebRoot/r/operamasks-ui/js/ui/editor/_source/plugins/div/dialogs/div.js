...
(function()
{
	...
	function addSafely( collection, element, database )
	{
		...
		{
			element.is && OMEDITOR.dom.element.setMarker( database, element, 'block_processed', true );
	...
	function divDialog( editor, command )
	{
		...
		var divLimitDefinition = ( function(){
			...
			var definition = OMEDITOR.tools.extend( {}, OMEDITOR.dtd.$blockLimit );
		...
		{
			var pathElements = new OMEDITOR.dom.elementPath( element ).elements;
			var divLimit;
		...
		function createDiv( editor )
		{
			...
			var blockTag = editor.config.enterMode == OMEDITOR.ENTER_DIV ? 'div' : 'p';
			...
			for ( i = 0 ; i < blockGroups.length ; i++ )
			{
				...
				divElement = new OMEDITOR.dom.element( 'div', editor.document );
				...
				for ( j = 0 ; j < blockGroups[ i ].length ; j++ )
				{
					...
					{
						currentNode.is && OMEDITOR.dom.element.setMarker( database, currentNode, 'block_processed', true );
		...
		{
			var path = new OMEDITOR.dom.elementPath( editor.getSelection().getStartElement() ),
		 ...
		 *  * OMEDITOR.dom.range.Iterator
		 *  * OMEDITOR.dom.domWalker
		...
		function commitInternally( targetFields )
		{
			var dialog = this.getDialog(),
				 element = dialog._element && dialog._element.clone()
						 || new OMEDITOR.dom.element( 'div', editor.document );
		...
		// Registered 'OMEDITOR.style' instances.
		var styles = {} ;
		/**
					...
					if ( stylesDefinitions )
					{
						...
						for ( var i = 0 ; i < stylesDefinitions.length ; i++ )
						{
							...
							if ( styleDefinition.element && styleDefinition.element == 'div' )
							{
								...
								styles[ styleName ] = new OMEDITOR.style( styleDefinition );
	...
	OMEDITOR.dialog.add( 'creatediv', function( editor )
		{
			return divDialog( editor, 'creatediv' );
		} );
	OMEDITOR.dialog.add( 'editdiv', function( editor )
		{
...
/*
 * @name OMEDITOR.config.div_wrapTable
