...
(function()
{
	...
	function onSelectionChange( evt )
	{
		...
		if ( !firstBlock )
			return this.setState( OMEDITOR.TRISTATE_DISABLED );
	...
	{
		return node.type = OMEDITOR.NODE_ELEMENT && node.is( 'li' );
	}
	...
	indentCommand.prototype = {
		exec : function( editor )
		{
			...
			function indentList( listNode )
			{
				...
				var listArray = OMEDITOR.plugins.list.listToArray( listNode, database );
				...
				for ( i = startItem.getCustomData( 'listarray_index' ); i <= lastItem.getCustomData( 'listarray_index' ); i++ )
				{
					...
					listArray[ i ].parent = new OMEDITOR.dom.element( listRoot.getName(), listRoot.getDocument() );
				}
				...
				var newList = OMEDITOR.plugins.list.arrayToList( listArray, database, null, editor.config.enterMode, listNode.getDirection() );
				...
				if ( pendingLis && pendingLis.length )
				{
					for (  i = 0; i < pendingLis.length ; i++ )
					{
							   ...
							   followingList.getName() in listNodeNames )
						{
							...
							if ( OMEDITOR.env.ie && !li.getFirst( function( node ){ return isNotWhitespaces( node ) && isNotBookmark( node ); } ) )
			...
			function indentElement( element, dir )
			{
				...
				if ( self.useIndentClasses )
				{
					...
					element.$.className = OMEDITOR.tools.ltrim( element.$.className.replace( self.classNameRegex, '' ) );
					if ( indentStep > 0 )
				...
				OMEDITOR.dom.element.setMarker( database, element, 'indent_processed', 1 );
				return true;
			}
			...
			while ( ( range = iterator.getNextRange() ) )
			{
				...
				while ( nearestListBlock && !( nearestListBlock.type == OMEDITOR.NODE_ELEMENT &&
				...
				if ( !nearestListBlock )
				{
					...
					if ( selectedNode
						&& selectedNode.type == OMEDITOR.NODE_ELEMENT
						&& selectedNode.getName() in listNodeNames)
					{
						range.setStartAt( selectedNode, OMEDITOR.POSITION_AFTER_START );
						range.setEndAt( selectedNode, OMEDITOR.POSITION_BEFORE_END );
				...
				// <ul>[<li>...</li>]</ul> =>	<ul><li>[...]</li></ul>
				if ( nearestListBlock && range.startContainer.type == OMEDITOR.NODE_ELEMENT
				...
				if ( nearestListBlock && range.endContainer.type == OMEDITOR.NODE_ELEMENT
	...
	OMEDITOR.plugins.add( 'indent',
	{
		init : function( editor )
		{
			...
			editor.on( 'selectionChange', OMEDITOR.tools.bind( onSelectionChange, indent ) );
			editor.on( 'selectionChange', OMEDITOR.tools.bind( onSelectionChange, outdent ) );
 ...
 * @name OMEDITOR.config.indentOffset
 * @type Number
 * @default 40
 * @example
 ...
 * @name OMEDITOR.config.indentUnit
 * @type String
 * @default 'px'
 * @example
 ...
 * @name OMEDITOR.config.indentClasses
 * @type Array
 * @default null
 * @example
