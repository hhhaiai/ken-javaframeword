...
OMEDITOR.plugins.add( 'floatpanel',
{
	requires : [ 'panel' ]
});
...
(function()
{
	...
	function getPanel( editor, doc, parentElement, definition, level )
	{
		...
		var key = OMEDITOR.tools.genKey( doc.getUniqueId(), parentElement.getUniqueId(), editor.skinName, editor.lang.dir,
		...
		if ( !panel )
		{
			panel = panels[ key ] = new OMEDITOR.ui.panel( doc, definition );
			panel.element = parentElement.append( OMEDITOR.dom.element.createFromHtml( panel.renderHtml( editor ), doc ) );
	...
	OMEDITOR.ui.floatPanel = OMEDITOR.tools.createClass(
	{
		...
		proto :
		{
			...
			showBlock : function( name, offsetParent, corner, offsetX, offsetY )
			{
				...
				this._.returnFocus = this._.editor.focusManager.hasFocus ? this._.editor : new OMEDITOR.dom.element( OMEDITOR.document.$.activeElement );
				...
				if ( !this._.blurSet )
				{
					...
					var focused = OMEDITOR.env.ie ? iframe : new OMEDITOR.dom.window( iframe.$.contentWindow );
				...
				OMEDITOR.tools.setTimeout( function()
					{
						...
						var panelLoad = OMEDITOR.tools.bind( function ()
						{
							...
							if ( block.autoSize )
							{
								...
								if ( OMEDITOR.env.gecko || OMEDITOR.env.opera )
								...
								// (#3426)
								if ( OMEDITOR.env.ie && OMEDITOR.env.quirks && width > 0 )
								...
								// http://en.wikipedia.org/wiki/Internet_Explorer_box_model_bug
								// (#3426)
								if ( OMEDITOR.env.ie && OMEDITOR.env.quirks && height > 0 )
							...
							if ( OMEDITOR.env.ie )
							{
								var offsetParent = new OMEDITOR.dom.element( element.$.offsetParent ),
			...
			hide : function( returnFocus )
			{
				if ( this.visible && ( !this.onHide || this.onHide.call( this ) !== true ) )
				{
					...
					OMEDITOR.env.gecko && this._.iframe.getFrameDocument().$.activeElement.blur();
					...
					if ( focusReturn )
					{
						...
						if ( OMEDITOR.env.webkit && focusReturn.type )
			...
			showAsChild : function( panel, blockName, offsetParent, corner, offsetX, offsetY )
			{
				...
				if ( OMEDITOR.env.ie7Compat || ( OMEDITOR.env.ie8 && OMEDITOR.env.ie6Compat ) )
				{
					setTimeout(function()
						{
	...
	{
		var isLastInstance = OMEDITOR.tools.isEmpty( OMEDITOR.instances );
