...
OMEDITOR.dialog.add( 'checkspell', function( editor )
{
	var number = OMEDITOR.tools.getNextNumber(),
	...
	window._cancelOnError = function( m )
	{
		...
		{
			OMEDITOR.document.getById( iframeId ).setStyle( 'display', 'none' );
			var errorBox = OMEDITOR.document.getById( errorBoxId );
	...
	function initAndSpell( dialog )
	{
		var LangComparer = new window._SP_FCK_LangCompare(),							// Language abbr standarts comparer.
			pluginPath = OMEDITOR.getUrl( editor.plugins.wsc.path + 'dialogs/' ),			// Service paths corecting/preparing.
		...
		OMEDITOR.document.getById( errorBoxId ).setStyle( 'display', 'none' );
		OMEDITOR.document.getById( iframeId ).setStyle( 'display', 'block' );
	}
	...
	return {
		...
		onShow : function()
		{
			...
			OMEDITOR.document.getById( textareaId ).setValue( sData );
