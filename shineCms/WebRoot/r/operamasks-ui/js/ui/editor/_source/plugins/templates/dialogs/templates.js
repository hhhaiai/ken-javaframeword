...
(function()
{
	...
	OMEDITOR.dialog.add( 'templates', function( editor )
		{
			...
			function renderTemplatesList( container, templatesDefinitions )
			{
				...
				{
					var definition = OMEDITOR.getTemplates( templatesDefinitions[ i ] ),
			...
			{
				var item = OMEDITOR.dom.element.createFromHtml(
				...
				if ( template.image && imagesPath )
					html += '<td class="cke_tpl_preview_img"><img src="' + OMEDITOR.getUrl( imagesPath + template.image ) + '"' + ( OMEDITOR.env.ie6Compat ? ' onload="this.width=this.width"' : '' ) + ' alt="" title=""></td>';
			...
			{
				var dialog = OMEDITOR.dialog.getCurrent(),
			...
			// Load skin at first.
			OMEDITOR.skins.load( editor, 'templates' );

			var listContainer;

			var templateListLabelId = 'cke_tpl_list_label_' + OMEDITOR.tools.getNextNumber(),
				lang = editor.lang.templates,
				config = editor.config;
			return {
				title :editor.lang.templates.title,

				minWidth : OMEDITOR.env.ie ? 440 : 400,
				minHeight : 340,
				...
				onShow : function()
				{
					...
					OMEDITOR.loadTemplates( config.templates_files, function()
						{
