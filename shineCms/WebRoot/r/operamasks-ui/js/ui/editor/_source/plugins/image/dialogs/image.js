...
(function()
{
	var imageDialog = function( editor, dialogType )
	{
		...
		var switchLockRatio = function( dialog, value )
		{
			...
			var ratioButton = OMEDITOR.document.getById( btnLockSizesId );
			if ( dialog.lockRatio )
			...
			if ( OMEDITOR.env.hc )
			{
				...
				icon.setHtml(  dialog.lockRatio ? OMEDITOR.env.ie ? '\u25A0': '\u25A3' : OMEDITOR.env.ie ? '\u25A1' : '\u25A2' );
			}
		...
		var onImgLoadEvent = function()
		{
			...
			// Hide loader
			OMEDITOR.document.getById( imagePreviewLoaderId ).setStyle( 'display', 'none' );
			...
			if ( this.firstLoad )
				OMEDITOR.tools.setTimeout( function(){ switchLockRatio( this, 'check' ); }, 0, this );
		...
		var onImgLoadErrorEvent = function()
		{
			...
			// Set Error image.
			var noimage = OMEDITOR.getUrl( editor.skinPath + 'images/noimage.png' );
		...
		var numbering = function( id )
			{
				return OMEDITOR.tools.getNextId() + '_' + id;
			},
		...
		return {
			...
			onShow : function()
			{
				...
				previewPreloader = new OMEDITOR.dom.element( 'img', editor.document );
				this.preview = OMEDITOR.document.getById( previewImageId );
				...
				if ( !OMEDITOR.tools.trim( this.getValueOf( 'info', 'txtUrl' ) ) )
				{
									...
									[
										{
											...
											onChange : function()
											{
												...
												if ( newUrl.length > 0 )	//Prevent from load before onShow
												{
													...
													// Show loader
													var loader = OMEDITOR.document.getById( imagePreviewLoaderId );
													if ( loader )
											...
											validate : OMEDITOR.dialog.validate.notEmpty( editor.lang.image.urlMissing )
										},
										{
											type : 'button',
											id : 'browse',
											...
											[
													...
													[
														{
															...
															commit : function( type, element, internalCommit )
															{
																...
																{
																	if ( value )
																		element.setStyle( 'width', OMEDITOR.tools.cssLength( value ) );
																	else
														...
														},
														{
															...
															commit : function( type, element, internalCommit )
															{
																...
																{
																	if ( value )
																		element.setStyle( 'height', OMEDITOR.tools.cssLength( value ) );
																	else
												...
												},
												{
													...
													onLoad : function()
													{
														...
														var	resetButton = OMEDITOR.document.getById( btnResetSizeId ),
															ratioButton = OMEDITOR.document.getById( btnLockSizesId );
														if ( resetButton )
														{
											...
											[
												{
													...
													validate : OMEDITOR.dialog.validate.integer( editor.lang.image.validateBorder ),
													setup : function( type, element )
													{
														if ( type == IMAGE )
														{
													...
													commit : function( type, element, internalCommit )
													{
														...
														if ( type == IMAGE || type == PREVIEW )
														{
															if ( !isNaN( value ) )
															{
																element.setStyle( 'border-width', OMEDITOR.tools.cssLength( value ) );
												...
												},
												{
													...
													validate : OMEDITOR.dialog.validate.integer( editor.lang.image.validateHSpace ),
													setup : function( type, element )
													{
														if ( type == IMAGE )
														{
													...
													commit : function( type, element, internalCommit )
													{
														...
														if ( type == IMAGE || type == PREVIEW )
														{
															if ( !isNaN( value ) )
															{
																element.setStyle( 'margin-left', OMEDITOR.tools.cssLength( value ) );
																element.setStyle( 'margin-right', OMEDITOR.tools.cssLength( value ) );
															}
															else if ( !value && this.isChanged( ) )
															{
												...
												},
												{
													...
													validate : OMEDITOR.dialog.validate.integer( editor.lang.image.validateVSpace ),
													setup : function( type, element )
													{
														if ( type == IMAGE )
														{
													...
													commit : function( type, element, internalCommit )
													{
														...
														if ( type == IMAGE || type == PREVIEW )
														{
															if ( !isNaN( value ) )
															{
																element.setStyle( 'margin-top', OMEDITOR.tools.cssLength( value ) );
																element.setStyle( 'margin-bottom', OMEDITOR.tools.cssLength( value ) );
															}
															else if ( !value && this.isChanged( ) )
															{
									...
									[
										{
											...
											id : 'htmlPreview',
											style : 'width:95%;',
											html : '<div>' + OMEDITOR.tools.htmlEncode( editor.lang.common.preview ) +'<br>'+
					...
					[
						{
							...
							onChange : function ()
							{
								...
								if(editor.config.allowImageType){
									for (var i = 0; i < editor.config.allowImageType.length; i++) {
										if (editor.config.allowImageType[i] == ext) {
											allowSubmit = true;
											break; 
										}
									}
									if(!allowSubmit){
										alert(editor.lang.image.uploadTypeNoAllow + editor.config.allowImageType);
									}
									editor.allowImageUploadSubmit = allowSubmit;
								}
							}
						},
						{
							...
							onClick : function ()
							{
								if(!editor.allowImageUploadSubmit){
									alert(editor.lang.image.uploadTypeNoAllow + editor.config.allowImageType);
									return false;
								}
							}
	...
	OMEDITOR.dialog.add( 'image', function( editor )
		{
			return imageDialog( editor, 'image' );
		});

	OMEDITOR.dialog.add( 'imagebutton', function( editor )
		{
