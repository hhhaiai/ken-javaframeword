...
OMEDITOR.plugins.add( 'listblock',
{
	requires : [ 'panel' ],

	onLoad : function()
	{
		OMEDITOR.ui.panel.prototype.addListBlock = function( name, definition )
		{
			return this.addBlock( name, new OMEDITOR.ui.listBlock( this.getHolderElement(), definition ) );
		};

		OMEDITOR.ui.listBlock = OMEDITOR.tools.createClass(
			{
				base : OMEDITOR.ui.panel.block,
				...
				_ :
				{
					...
					getClick : function()
					{
						if ( !this._.click )
						{
							this._.click = OMEDITOR.tools.addFunction( function( value )
								{
									var marked = true;
				...
				proto :
				{
							...
							'<li id=', id, ' class=cke_panel_listItem role=presentation>' +
								'<a id="', id, '_option" _cke_focus=1 hidefocus=true' +
									...
									( OMEDITOR.env.ie ? 'onclick="return false;" onmouseup' : 'onclick' ) +		// #188
										'="OMEDITOR.tools.callFunction(', this._.getClick(), ',\'', value, '\'); return false;"',
									' role="option"' +
					...
					startGroup : function( title )
					{
						...
						var id = OMEDITOR.tools.getNextId();
