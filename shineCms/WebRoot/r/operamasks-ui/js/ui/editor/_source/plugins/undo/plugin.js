...
(function()
{
	OMEDITOR.plugins.add( 'undo',
	{
		...
		init : function( editor )
		{
			...
			{
				undoCommand.setState( undoManager.undoable() ? OMEDITOR.TRISTATE_OFF : OMEDITOR.TRISTATE_DISABLED );
				redoCommand.setState( undoManager.redoable() ? OMEDITOR.TRISTATE_OFF : OMEDITOR.TRISTATE_DISABLED );
			};
			 ...
			 * @name OMEDITOR.editor#updateUndo
			 * @example
			 * function()
			 * {
	...
	var Image = OMEDITOR.plugins.undo.Image = function( editor )
	{
		...
		OMEDITOR.env.ie && contents && ( contents = contents.replace( /\s+data-cke-expando=".*?"/g, '' ) );
			...
			if ( bookmarksA || bookmarksB )
			{
				...
				for ( var i = 0 ; i < bookmarksA.length ; i++ )
				{
					...
					if (
						...
						!OMEDITOR.tools.arrayCompare( bookmarkA.start, bookmarkB.start ) ||
						!OMEDITOR.tools.arrayCompare( bookmarkA.end, bookmarkB.end ) )
					{
						return false;
					}
				}
	...
	UndoManager.prototype =
	{
		 ...
		 * @param {OMEDITOR.dom.event} event
		 */
		type : function( event )
		{
 ...
 * @name OMEDITOR.config.undoStackSize
 * @type Number
 * @default 20
 * @example
 ...
 * @name OMEDITOR.editor#saveSnapshot
 * @event
 */
 ...
 * @name OMEDITOR.editor#beforeUndoImage
 * @since 3.5.3
 * @see OMEDITOR.editor#afterUndoImage
 * @event
 */
 ...
 * editor state at some point. It's saved into an undo store, so the editor is
 * able to recover the editor state on undo and redo operations.
 * @name OMEDITOR.editor#afterUndoImage
 * @since 3.5.3
 * @see OMEDITOR.editor#beforeUndoImage
 * @event
 */
