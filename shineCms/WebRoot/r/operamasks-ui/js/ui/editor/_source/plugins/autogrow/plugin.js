...
(function(){
	var resizeEditor = function( editor )
	{
		...
		if ( OMEDITOR.env.ie || OMEDITOR.env.gecko )
			newHeight = doc.getBody().$.scrollHeight + ( OMEDITOR.env.ie && OMEDITOR.env.quirks ? 0 : 24 );
		else
			...
			for ( var eventName in { contentDom:1, key:1, selectionChange:1, insertElement:1 } )
			{
				editor.on( eventName, function( evt )
				{
					...
					if ( evt.editor.mode == 'wysiwyg' &&
						...
						( !maximize || maximize.state != OMEDITOR.TRISTATE_ON ) )
					{
 ...
 * @name OMEDITOR.config.autoGrow_minHeight
 * @type Number
 * @default 200
 * @since 3.4
 * @example
 ...
 * @name OMEDITOR.config.autoGrow_maxHeight
 * @type Number
 * @default 0
 * @since 3.4
 * @example
