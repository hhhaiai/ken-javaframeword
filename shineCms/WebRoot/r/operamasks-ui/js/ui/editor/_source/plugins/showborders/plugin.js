...
(function()
{
	...
	var commandDefinition =
	{
		...
		refresh : function( editor )
		{
			if ( editor.document )
			{
				var funcName = ( this.state == OMEDITOR.TRISTATE_ON ) ? 'addClass' : 'removeClass';
				editor.document.getBody()[ funcName ]( 'cke_show_borders' );
			}
		}
	};

	OMEDITOR.plugins.add( 'showborders',
	{
		...
		init : function( editor )
		{
			...
			if ( editor.config.startupShowBorders !== false )
				command.setState( OMEDITOR.TRISTATE_ON );
			...
			editor.on( 'mode', function()
				{
					if ( command.state != OMEDITOR.TRISTATE_DISABLED )
			...
			editor.on( 'removeFormatCleanup', function( evt )
				{
					...
					if ( editor.getCommand( 'showborders' ).state == OMEDITOR.TRISTATE_ON &&
	...
	OMEDITOR.on( 'dialogDefinition', function( ev )
	{
		...
		if ( dialogName == 'table' || dialogName == 'tableProperties' )
		{
			...
			borderField.commit = OMEDITOR.tools.override( originalCommit, function( org )
			{
			...
			if ( classField )
			{
				classField.setup = OMEDITOR.tools.override( classField.setup, function( originalSetup )
					{
						return function()
							{
				...
				classField.commit = OMEDITOR.tools.override( classField.commit, function( originalCommit )
					{
 ...
 * @name OMEDITOR.config.startupShowBorders
 * @type Boolean
 * @default true
 * @example
