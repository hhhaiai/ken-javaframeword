...
OMEDITOR.plugins.add( 'pagebreak',
{
	init : function( editor )
	{
		// Register the command.
		editor.addCommand( 'pagebreak', OMEDITOR.plugins.pagebreakCmd );
		...
		var cssStyles = [
			'{' ,
				'background: url(' + OMEDITOR.getUrl( this.path + 'images/pagebreak.gif' ) + ') no-repeat center center;' ,
				'clear: both;' ,
		...
		OMEDITOR.env.opera && editor.on( 'contentDom', function()
		{
			...
			htmlFilter.addRules(
			{
				attributes : {
					'class' : function( value, element )
					{
						...
						if ( className != value )
						{
							var span = OMEDITOR.htmlParser.fragment.fromHtml( '<span style="display: none;">&nbsp;</span>' );
...
OMEDITOR.plugins.pagebreakCmd =
{
	exec : function( editor )
	{
		...
		var pagebreak = OMEDITOR.dom.element.createFromHtml(
			'<div style="' +
		...
		for ( var range, i = ranges.length - 1 ; i >= 0; i-- )
		{
			...
			if ( i == ranges.length - 1 )
			{
				...
				range.moveToPosition( pagebreak, OMEDITOR.POSITION_AFTER_END );
