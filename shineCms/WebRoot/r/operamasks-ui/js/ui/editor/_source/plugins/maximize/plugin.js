...
(function()
{
	...
	{
		if ( !formElement || formElement.type != OMEDITOR.NODE_ELEMENT || formElement.getName() != 'form' )
			return [];
		...
		for ( var i = 0 ; i < hijackNames.length ; i++ )
		{
			...
			if ( $node )
			{
				var hijackNode = new OMEDITOR.dom.element( $node );
	...
	{
		if ( !OMEDITOR.env.ie || OMEDITOR.env.version > 6 )
			return null;

		var shim = OMEDITOR.dom.element.createFromHtml( '<iframe frameborder="0" tabindex="-1"' +
					' src="javascript:' +
					   'void((function(){' +
						   'document.open();' +
						   ( OMEDITOR.env.isCustomDomain() ? 'document.domain=\'' + this.getDocument().$.domain + '\';' : '' ) +
						   'document.close();' +
					   '})())"' +
	...
	OMEDITOR.plugins.add( 'maximize',
	{
		init : function( editor )
		{
			var lang = editor.lang;
			var mainDocument = OMEDITOR.document,
			...
			var savedState = OMEDITOR.TRISTATE_OFF;
			...
			editor.addCommand( 'maximize',
				{
					...
					exec : function()
					{
						...
						else
						{
							...
							savedSelection = !OMEDITOR.env.ie && [ $textarea.selectionStart, $textarea.selectionEnd ];
							savedScroll = [ $textarea.scrollLeft, $textarea.scrollTop ];
						}

						if ( this.state == OMEDITOR.TRISTATE_OFF )		// Go fullscreen if the state is off.
						{
							...
							var styles =
								{
									overflow : OMEDITOR.env.webkit ? '' : 'hidden',		// #6896
									width : 0,
									height : 0
								};

							mainDocument.getDocumentElement().setStyles( styles );
							!OMEDITOR.env.gecko && mainDocument.getDocumentElement().setStyle( 'position', 'fixed' );
							!( OMEDITOR.env.gecko && OMEDITOR.env.quirks ) && mainDocument.getBody().setStyles( styles );
							...
							container.setStyle( 'position', OMEDITOR.env.gecko && OMEDITOR.env.quirks ? 'fixed' : 'absolute' );
							...
							OMEDITOR.env.gecko && refreshCursor( editor );

						}
						else if ( this.state == OMEDITOR.TRISTATE_ON )	// Restore from fullscreen if the state is on.
						{
						...
						if( button )
						{
							var label = ( this.state == OMEDITOR.TRISTATE_OFF )
			...
			editor.on( 'mode', function()
				{
					...
					command.setState( command.state == OMEDITOR.TRISTATE_DISABLED ? OMEDITOR.TRISTATE_DISABLED : savedState );
				}, null, null, 100 );
		}
	} );
})();
