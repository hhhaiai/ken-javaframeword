...
(function()
{
				...
				for ( var j = 0; j < eventNameList.length; j++ )
				{
					(function( eventName )
					{
						...
						if ( node.hasAttribute( 'on' + eventName ) )
						{
							...
							{
								var callFunc = /(return\s*)?OMEDITOR\.tools\.callFunction\(([^)]+)\)/.exec( inlineEventHandler ),
								...
								if ( callFuncArgs )
								{
									...
									for ( var i = 0; i < nums; i++ )
									{
										...
										callFuncArgs[ i ] = argName = OMEDITOR.tools.trim( callFuncArgs[ i ] );
									...
									var retval = OMEDITOR.tools.callFunction.apply( window, callFuncArgs );
	...
	OMEDITOR.ui.on( 'ready', function( evt )
		{
			var ui = evt.data;
			...
			else if ( ui instanceof OMEDITOR.dialog )
				convertInlineHandlers( ui._.element );
		});
})();

OMEDITOR.dom.document.prototype.write = OMEDITOR.tools.override( OMEDITOR.dom.document.prototype.write,
	function( original_write )
	{
		function appendElement( parent, tagName, fullTag, text )
		{
			var node = parent.append( tagName ),
				attrs = OMEDITOR.htmlParser.fragment.fromHtml( fullTag ).children[ 0 ].attributes;
				...
				if ( this.getBody() )
				{
					...
					html = html.replace( /<head>([\s\S]*)<\/head>/i,
						function( headHtml )
						{
							...
							var div = new OMEDITOR.dom.element( 'div', doc );
					...
					html.replace( /(<body[^>]*>)([\s\S]*)(?=$|<\/body>)/i,
						function( match, startTag, innerHTML )
						{
							...
							var attrs = OMEDITOR.htmlParser.fragment.fromHtml( startTag ).children[ 0 ].attributes;
