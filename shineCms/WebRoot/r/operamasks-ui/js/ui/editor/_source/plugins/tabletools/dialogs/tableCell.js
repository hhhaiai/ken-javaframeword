...
OMEDITOR.dialog.add( 'cellProperties', function( editor )
	{
		var langTable = editor.lang.table,
			langCell = langTable.cell,
			langCommon = editor.lang.common,
			validate = OMEDITOR.dialog.validate,
			widthPattern = /^(\d+(?:\.\d+)?)(px|%)$/,
			heightPattern = /^(\d+(?:\.\d+)?)px$/,
			bind = OMEDITOR.tools.bind,
		...
		return {
			...
			minWidth : OMEDITOR.env.ie && OMEDITOR.env.quirks? 450 : 410,
			minHeight : OMEDITOR.env.ie && ( OMEDITOR.env.ie7Compat || OMEDITOR.env.quirks )?  230 : 200,
			contents : [
				{
					id : 'info',
			...
			onShow : function()
			{
				this.cells = OMEDITOR.plugins.tabletools.getSelectedCells(
