...
(function()
{
	function getState( editor, path )
	{
		...
		return ( getAlignment( firstBlock, editor.config.useComputedState ) == this.value ) ?
			OMEDITOR.TRISTATE_ON :
			OMEDITOR.TRISTATE_OFF;
	}
			...
			for ( var i = ranges.length - 1 ; i >= 0 ; i-- )
			{
				...
				while ( ( block = iterator.getNextParagraph( enterMode == OMEDITOR.ENTER_P ? 'p' : 'div' ) ) )
				{
					...
					var className = cssClassName && ( block.$.className =
						OMEDITOR.tools.ltrim( block.$.className.replace( this.cssClassRegex, '' ) ) );

					var apply =
						( this.state == OMEDITOR.TRISTATE_OFF ) &&
	...
	OMEDITOR.plugins.add( 'justify',
	{
		init : function( editor )
		{
			...
			editor.on( 'selectionChange', OMEDITOR.tools.bind( onSelectionChange, left ) );
			editor.on( 'selectionChange', OMEDITOR.tools.bind( onSelectionChange, right ) );
			editor.on( 'selectionChange', OMEDITOR.tools.bind( onSelectionChange, center ) );
			editor.on( 'selectionChange', OMEDITOR.tools.bind( onSelectionChange, justify ) );
 ...
 * @name OMEDITOR.config.justifyClasses
 * @type Array
 * @default null
 * @example
