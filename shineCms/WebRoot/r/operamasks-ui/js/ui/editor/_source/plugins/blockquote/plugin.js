...
(function()
{
	...
	function noBlockLeft( bqBlock )
	{
		...
		{
			if ( child.type == OMEDITOR.NODE_ELEMENT && child.isBlockBoundary() )
				return false;
		}
		return true;
	}
		...
		exec : function( editor )
		{
			...
			if ( OMEDITOR.env.ie )
			{
				...
				if ( bookmarkStart && bookmarkStart.getParent().getName() == 'blockquote' )
				{
					...
					{
						if ( cursor.type == OMEDITOR.NODE_ELEMENT &&
			...
			var iterator = range.createIterator(),
				block;
			iterator.enlargeBr = editor.config.enterMode != OMEDITOR.ENTER_BR;

			if ( state == OMEDITOR.TRISTATE_OFF )
			{
				var paragraphs = [];
				...
				{
					var para = editor.document.createElement( editor.config.enterMode == OMEDITOR.ENTER_P ? 'p' : 'div' ),
						firstBookmark = bookmarks.shift();
					range.insertNode( para );
					para.append( new OMEDITOR.dom.text( '\ufeff', editor.document ) );
				...
				while ( tmp.length > 0 )
				{
					...
					{
						var docFrag = new OMEDITOR.dom.documentFragment( editor.document );
			...
			else if ( state == OMEDITOR.TRISTATE_ON )
			{
				var moveOutNodes = [],
					database = {};
				...
				while ( ( block = iterator.getNextParagraph() ) )
				{
					...
					if ( bqParent && bqChild && !bqChild.getCustomData( 'blockquote_moveout' ) )
					{
						...
						OMEDITOR.dom.element.setMarker( database, bqChild, 'blockquote_moveout', true );
					}
				}
				...
				while ( moveOutNodes.length > 0 )
				{
					...
					if ( !bqBlock.getCustomData( 'blockquote_processed' ) )
					{
						...
						OMEDITOR.dom.element.setMarker( database, bqBlock, 'blockquote_processed', true );
					}
				...
				if ( editor.config.enterMode == OMEDITOR.ENTER_BR )
				{
					var firstTime = true;
					while ( movedNodes.length )
					{
						...
						if ( node.getName() == 'div' )
						{
							docFrag = new OMEDITOR.dom.documentFragment( editor.document );
							var needBeginBr = firstTime && node.getPrevious() &&
									!( node.getPrevious().type == OMEDITOR.NODE_ELEMENT && node.getPrevious().isBlockBoundary() );
							if ( needBeginBr )
								docFrag.append( editor.document.createElement( 'br' ) );

							var needEndBr = node.getNext() &&
								!( node.getNext().type == OMEDITOR.NODE_ELEMENT && node.getNext().isBlockBoundary() );
							while ( node.getFirst() )
	...
	OMEDITOR.plugins.add( 'blockquote',
	{
