...
(function()
{
		...
		return {
			...
			exec : function( editor )
			{
				if ( editor.focusManager.hasFocus )
				{
					...
					{
						var resultRange = new OMEDITOR.dom.range( editor.document ),
								next = OMEDITOR.tools.tryThese( function()
								{
						...
						if ( ! ( next || backward ) )
						{
							...
							var newRow = new OMEDITOR.dom.element( table.insertRow( -1 ), editor.document );

							for ( var i = 0, count = cells.length ; i < count; i++ )
							{
								var newCell = newRow.append( new OMEDITOR.dom.element(
										cells[ i ], editor.document ).clone( false, false ) );
								!OMEDITOR.env.ie && newCell.appendBogus();
							}
						...
						{
							next = new OMEDITOR.dom.element( next );
	...
	OMEDITOR.plugins.add( 'tab',
	{
		...
		init : function( editor )
		{
			...
			if ( tabTools )
			{
				editor.on( 'key', function( ev )
				{
					if ( ev.data.keyCode == 9 && editor.execCommand( 'selectNextCell' ) ||	// TAB
							ev.data.keyCode == ( OMEDITOR.SHIFT + 9 ) && editor.execCommand( 'selectPreviousCell' ) )	// SHIFT+TAB
						ev.cancel();
				});
			}

			if ( OMEDITOR.env.webkit || OMEDITOR.env.gecko )
			{
				editor.on( 'key', function( ev )
					{
						...
						if ( keyCode == ( OMEDITOR.SHIFT + 9 ) )	// SHIFT+TAB
						{
			...
			editor.addCommand( 'blur', OMEDITOR.tools.extend( blurCommand, meta ) );
			editor.addCommand( 'blurBack', OMEDITOR.tools.extend( blurBackCommand, meta ) );
 ...
 * element.focusNext();
 */
OMEDITOR.dom.element.prototype.focusNext = function( ignoreChildren, indexToUse )
{
	var $ = this.$,
	...
	if ( curTabIndex <= 0 )
	{
		...
		element = this.getNextSourceNode( ignoreChildren, OMEDITOR.NODE_ELEMENT );
		...
		while ( element )
		{
			...
			element = element.getNextSourceNode( false, OMEDITOR.NODE_ELEMENT );
		}
	}
	else
	{
		...
		while ( ( element = element.getNextSourceNode( false, OMEDITOR.NODE_ELEMENT ) ) )
		{
			if ( !passedCurrent )
			{
				if ( !enteredCurrent && element.equals( this ) )
				{
					...
					if ( ignoreChildren )
					{
						if ( !( element = element.getNextSourceNode( true, OMEDITOR.NODE_ELEMENT ) ) )
							break;
						passedCurrent = 1;
					}
				}
 ...
 */
OMEDITOR.dom.element.prototype.focusPrevious = function( ignoreChildren, indexToUse )
{
	var $ = this.$,
		curTabIndex = ( indexToUse === undefined ? this.getTabIndex() : indexToUse ),
		passedCurrent, enteredCurrent,
	...
	while ( ( element = element.getPreviousSourceNode( false, OMEDITOR.NODE_ELEMENT ) ) )
	{
		if ( !passedCurrent )
		{
			if ( !enteredCurrent && element.equals( this ) )
			{
				...
				if ( ignoreChildren )
				{
					if ( !( element = element.getPreviousSourceNode( true, OMEDITOR.NODE_ELEMENT ) ) )
						break;
					passedCurrent = 1;
				}
			}
 ...
 * @name OMEDITOR.config.tabSpaces
 * @type Number
 * @default 0
 * @example
 * config.tabSpaces = 4;
 ...
 * </ul>
 * @name OMEDITOR.config.enableTabKeyTools
 * @type Boolean
 * @default true
 * @example
...
//	[ OMEDITOR.ALT + 38 /*Arrow Up*/, 'selectPreviousCell' ],
//	[ OMEDITOR.ALT + 40 /*Arrow Down*/, 'selectNextCell' ]
// );
