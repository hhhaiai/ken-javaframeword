...
(function()
{
	var pxUnit = OMEDITOR.tools.cssLength,
		needsIEHacks = OMEDITOR.env.ie && ( OMEDITOR.env.ie7Compat || OMEDITOR.env.quirks || OMEDITOR.env.version < 7 );

	function getWidth( el )
	{
		return OMEDITOR.env.ie ? el.$.clientWidth : parseInt( el.getComputedStyle( 'width' ), 10 );
	}
	...
	function buildTableColumnPillars( table )
	{
		...
		var tbody = new OMEDITOR.dom.element( table.$.tBodies[ 0 ] ),
		...
		for ( var i = 0, len = $tr.cells.length ; i < len ; i++ )
		{
			...
			var td = new OMEDITOR.dom.element( $tr.cells[ i ] ),
				nextTd = $tr.cells[ i + 1 ] && new OMEDITOR.dom.element( $tr.cells[ i + 1 ] );
	...
	function columnResizer( editor )
	{
		...
		function resizeStart()
		{
			...
			var columnIndex = pillar.index,
				map = OMEDITOR.tools.buildTableMap( pillar.table ),
				leftColumnCells = [],
				rightColumnCells = [],
			...
			for ( var i = 0, len = map.length ; i < len ; i++ )
			{
				...
				leftCell	= leftCell && new OMEDITOR.dom.element( leftCell );
				rightCell	= rightCell && new OMEDITOR.dom.element( rightCell );
		...
		resizer = OMEDITOR.dom.element.createFromHtml(
	...
	function clearPillarsCache( evt )
	{
		...
		if ( evt.name == 'mouseout' )
		{
			...
			var dest = new OMEDITOR.dom.element( evt.data.$.relatedTarget || evt.data.$.toElement );
	...
	OMEDITOR.plugins.add( 'tableresize',
	{
		requires : [ 'tabletools' ],
