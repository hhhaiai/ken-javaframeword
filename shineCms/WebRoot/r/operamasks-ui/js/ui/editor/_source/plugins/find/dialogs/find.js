...
(function()
{
	...
	{
		return node.type == OMEDITOR.NODE_TEXT && node.getLength() > 0 && ( !isReplace || !node.isReadOnly() );
	}
	...
	*/
	...
	{
		return !( node.type == OMEDITOR.NODE_ELEMENT && node.isBlockBoundary(
			OMEDITOR.tools.extend( {}, OMEDITOR.dtd.$empty, OMEDITOR.dtd.$nonEditable ) ) );
	}
	...
	var findDialog = function( editor, startupPage )
	{
		...
		var highlightStyle = new OMEDITOR.style( OMEDITOR.tools.extend( { fullMatch : true, childRule : function(){ return 0; } },
		 ...
		 * @param { OMEDITOR.dom.range } range
		...
		var characterWalker = function( range , matchWord )
		{
			...
			if ( range.startContainer.type == OMEDITOR.NODE_TEXT )
			{
		...
		characterRange.prototype = {
			/**
			 * Translate this range to {@link OMEDITOR.dom.range}
			 */
			toDomRange : function()
			{
		...
		function getSearchRange( isDefault )
		{
			...
			{
				searchRange = new OMEDITOR.dom.range();
				searchRange.setStartAt( body, OMEDITOR.POSITION_AFTER_START );
			}
			searchRange.setEndAt( body, OMEDITOR.POSITION_BEFORE_END );
			return searchRange;
		}
		...
		return {
			title : lang.title,
			resizable : OMEDITOR.DIALOG_RESIZE_NONE,
			minWidth : 350,
			minHeight : 170,
			buttons : [ OMEDITOR.dialog.cancelButton ],		// Cancel button only.
			contents : [
				{
					id : 'find',
					label : lang.find,
			...
			onLoad : function()
			{
				...
				this.selectPage = OMEDITOR.tools.override( this.selectPage, function( originalFunc )
					{
						return function( pageId )
						{
							...
							{
								patternFieldInput = OMEDITOR.document
	...
	OMEDITOR.dialog.add( 'find', function( editor )
		{
			return findDialog( editor, 'find' );
		});

	OMEDITOR.dialog.add( 'replace', function( editor )
		{
