...
(function()
{
	...
	function checkSelectionChange()
	{
		try
		{
			...
			var currentPath = new OMEDITOR.dom.elementPath( firstElement );
	...
	function checkSelectionChangeTimeout()
	{
		...
		checkSelectionChangeTimer = OMEDITOR.tools.setTimeout( checkSelectionChangeTimeoutExec, 200, this );
	}
	...
	function checkSelectionChangeTimeoutExec()
	{
		...
		if ( checkSelectionChangeTimeoutPending )
		{
			...
			// with Firefox.
			OMEDITOR.tools.setTimeout( checkSelectionChange, 0, this );
	...
	function rangeRequiresFix( range )
	{
		...
		{
			return node && node.type == OMEDITOR.NODE_ELEMENT
		...
		if ( start.type == OMEDITOR.NODE_TEXT )
			return false;
		...
		return !OMEDITOR.tools.trim( start.getHtml() ) ? isInlineCt( start ) || singletonBlock( start )
	...
	var selectAllCmd =
	{
		...
		readOnly : OMEDITOR.env.ie || OMEDITOR.env.webkit,
	...
	OMEDITOR.plugins.add( 'selection',
	{
		init : function( editor )
		{
				...
				editor.on( 'key', function( e )
					{
						...
						{
							case 13 :	// ENTER
							case OMEDITOR.SHIFT + 13 :	// SHIFT-ENTER
							case 37 :	// LEFT-ARROW
							case 39 :	// RIGHT-ARROW
							case 8 :	// BACKSPACE
			...
			editor.on( 'contentDom', function()
				{
					...
					if ( OMEDITOR.env.ie )
					{
						...
						// not editable. (#5634)
						if ( OMEDITOR.env.ie && ( OMEDITOR.env.ie7Compat || OMEDITOR.env.version < 8 || OMEDITOR.env.quirks ) )
						{
						...
						function saveSelection( testIt )
						{
							if ( saveEnabled )
							{
								...
								if ( testIt && nativeSel && nativeSel.type == 'None' )
								{
									...
									{
										OMEDITOR.tools.setTimeout( saveSelection, 50, this, true );
										return;
									}
								}
			...
			OMEDITOR.env.ie9Compat && editor.on( 'destroy', function()
			{
	 ...
	 * @returns {OMEDITOR.dom.selection} A selection object or null if not on
	 *		WYSIWYG mode or no selection is available.
	 * @example
	 * var selection = OMEDITOR.instances.editor1.<b>getSelection()</b>;
	 * alert( selection.getType() );
	 */
	OMEDITOR.editor.prototype.getSelection = function()
	{
		return this.document && this.document.getSelection();
	};

	OMEDITOR.editor.prototype.forceNextSelectionCheck = function()
	{
	 ...
	 * @returns {OMEDITOR.dom.selection} A selection object.
	 * @example
	 * var selection = OMEDITOR.instances.editor1.document.<b>getSelection()</b>;
	 * alert( selection.getType() );
	 */
	OMEDITOR.dom.document.prototype.getSelection = function()
	{
		var sel = new OMEDITOR.dom.selection( this );
	 ...
	 * @constant
	 * @example
	 * if ( editor.getSelection().getType() == OMEDITOR.SELECTION_NONE )
	 ...
	 * @constant
	 * @example
	 * if ( editor.getSelection().getType() == OMEDITOR.SELECTION_TEXT )
	 ...
	 * @constant
	 * @example
	 * if ( editor.getSelection().getType() == OMEDITOR.SELECTION_ELEMENT )
	 ...
	 * @example
	 */
	OMEDITOR.dom.selection = function( document )
	{
	...
	OMEDITOR.dom.selection.prototype =
	{
		/**
		 ...
		 * available:
		 * <ul>
		 *		<li>{@link OMEDITOR.SELECTION_NONE} (1): No selection.</li>
		 *		<li>{@link OMEDITOR.SELECTION_TEXT} (2): Text is selected or
		 *			collapsed selection.</li>
		 *		<li>{@link OMEDITOR.SELECTION_ELEMENT} (3): A element
		 *			selection.</li>
		 * </ul>
		 * @function
		 * @returns {Number} One of the following constant values:
		 *		{@link OMEDITOR.SELECTION_NONE}, {@link OMEDITOR.SELECTION_TEXT} or
		 *		{@link OMEDITOR.SELECTION_ELEMENT}.
		 * @example
		 * if ( editor.getSelection().<b>getType()</b> == OMEDITOR.SELECTION_TEXT )
		 *     alert( 'Text is selected' );
		 */
		getType :
			OMEDITOR.env.ie ?
				function()
				{
					...
					var type = OMEDITOR.SELECTION_NONE;
					...
					try
					{
						...
						if ( ieType == 'Text' )
							type = OMEDITOR.SELECTION_TEXT;

						if ( ieType == 'Control' )
							type = OMEDITOR.SELECTION_ELEMENT;
			...
			:
				function()
				{
					...
					var type = OMEDITOR.SELECTION_TEXT;

					var sel = this.getNative();

					if ( !sel )
						type = OMEDITOR.SELECTION_NONE;
		...
		/**
		 * Retrieve the {@link OMEDITOR.dom.range} instances that represent the current selection.
		...
		getRanges : (function()
		{
			var func = OMEDITOR.env.ie ?
				( function()
				{
					function getNodeIndex( node ) { return new OMEDITOR.dom.node( node ).getIndex(); }
					...
					var getBoundaryInformation = function( range, start )
					{
						...
						while ( startIndex <= endIndex )
						{
							...
							else
							{
								...
								if ( OMEDITOR.env.ie9Compat && child.tagName == 'BR' )
								{
						...
						if ( index == -1 || index == siblings.length - 1 && position < 0 )
						{
							...
							if ( !distance )
							{
								...
								if ( child.nodeType == OMEDITOR.NODE_ELEMENT )
					...
					return function()
					{
						...
						if ( type == OMEDITOR.SELECTION_TEXT )
						{
							range = new OMEDITOR.dom.range( this.document );

							var boundaryInfo = getBoundaryInformation( nativeRange, true );
							range.setStart( new OMEDITOR.dom.node( boundaryInfo.container ), boundaryInfo.offset );

							boundaryInfo = getBoundaryInformation( nativeRange );
							range.setEnd( new OMEDITOR.dom.node( boundaryInfo.container ), boundaryInfo.offset );

							// Correct an invalid IE range case on empty list item. (#5850)
							if ( range.endContainer.getPosition( range.startContainer ) & OMEDITOR.POSITION_PRECEDING
						...
						else if ( type == OMEDITOR.SELECTION_ELEMENT )
						{
							var retval = [];
							...
							for ( var i = 0 ; i < nativeRange.length ; i++ )
							{
								...
								range.setStart( new OMEDITOR.dom.node( parentElement ), j );
								range.setEnd( new OMEDITOR.dom.node( parentElement ), j + 1 );
								retval.push( range );
							}
			...
			:
				function()
				{
					...
					// tranform the native ranges in OMEDITOR.dom.range
					// instances.
					...
					if ( !sel.rangeCount )
					{
						range = new OMEDITOR.dom.range( doc );
					...
					for ( var i = 0 ; i < sel.rangeCount ; i++ )
					{
						...
						range.setStart( new OMEDITOR.dom.node( nativeRange.startContainer ), nativeRange.startOffset );
						range.setEnd( new OMEDITOR.dom.node( nativeRange.endContainer ), nativeRange.endOffset );
						ranges.push( range );
					}
					return ranges;
				};
			...
			return function( onlyEditables )
			{
				...
				if ( cache.ranges && !onlyEditables )
					...
					cache.ranges = new OMEDITOR.dom.rangeList( func.call( this ) );
					...
					for ( var i = 0; i < ranges.length; i++ )
					{
						...
						walker.evaluator = function( node )
						{
								...
								&& node.isReadOnly() )
							{
								...
								if ( !( node.getPosition( walkerRange.endContainer ) & OMEDITOR.POSITION_CONTAINS ) )
								{
		 ...
		 * @returns {OMEDITOR.dom.element} The element at the beginning of the
		 *		selection.
		 * @example
		...
		getStartElement : function()
		{
			...
			switch ( this.getType() )
			{
				case OMEDITOR.SELECTION_ELEMENT :
					return this.getSelectedElement();

				case OMEDITOR.SELECTION_TEXT :
					...
					if ( range )
					{
						if ( !range.collapsed )
						{
							...
							if ( !node || node.type != OMEDITOR.NODE_ELEMENT )
								node = range.startContainer;
							else
							{
								var child = node.getFirst();
								while (  child && child.type == OMEDITOR.NODE_ELEMENT )
								{
									node = child;
			...
			return cache.startElement = ( node ? new OMEDITOR.dom.element( node ) : null );
		},

		/**
		 * Gets the current selected element.
		 * @returns {OMEDITOR.dom.element} The selected element. Null if no
		...
		getSelectedElement : function()
		{
			...
			var node = OMEDITOR.tools.tryThese(
				...
				function()
				{
					...
					for ( var i = 2; i && !( ( enclosed = range.getEnclosedNode() )
						&& ( enclosed.type == OMEDITOR.NODE_ELEMENT )
						&& styleObjectElements[ enclosed.getName() ]
						&& ( selected = enclosed ) ); i-- )
					{
						// Then check any deep wrapped element, e.g. [<b><i><img /></i></b>]
						range.shrink( OMEDITOR.SHRINK_ELEMENT );
					}

					return  selected.$;
				});

			return cache.selectedElement = ( node ? new OMEDITOR.dom.element( node ) : null );
		},
		...
		getSelectedText : function()
		{
			...
			if ( this.getType() == OMEDITOR.SELECTION_TEXT )
				text = OMEDITOR.env.ie ? nativeSel.createRange().text : nativeSel.toString();
		...
		/**
		 *  Make the current selection of type {@link OMEDITOR.SELECTION_ELEMENT} by enclosing the specified element.
		 * @param element
		 */
		selectElement : function( element )
		{
			if ( this.isLocked )
			{
				var range = new OMEDITOR.dom.range( this.document );
				range.setStartBefore( element );
				range.setEndAfter( element );

				this._.cache.selectedElement = element;
				this._.cache.startElement = element;
				this._.cache.ranges = new OMEDITOR.dom.rangeList( range );
				this._.cache.type = OMEDITOR.SELECTION_ELEMENT;

				return;
			}

			range = new OMEDITOR.dom.range( element.getDocument() );
			range.setStartBefore( element );
			range.setEndAfter( element );
			range.select();
		 ...
		 * @param {OMEDITOR.dom.range} ranges
		 */
		selectRanges : function( ranges )
		{
			if ( this.isLocked )
			{
				...
				this._.cache.ranges = new OMEDITOR.dom.rangeList( ranges );
				this._.cache.type = OMEDITOR.SELECTION_TEXT;
			...
			else
			{
				...
				if ( ranges.length )
				{
					...
					OMEDITOR.env.webkit && removeFillingChar( this.document );
				}
				...
				for ( var i = 0 ; i < ranges.length ; i++ )
				{
					...
					if ( i < ranges.length -1 )
					{
						...
						{
							between.shrink( OMEDITOR.NODE_ELEMENT, true );
					...
					// element. (#4657)
					if ( range.collapsed &&
						( OMEDITOR.env.opera || ( OMEDITOR.env.gecko && OMEDITOR.env.version < 10900 ) ) &&
						startContainer.type == OMEDITOR.NODE_ELEMENT &&
							...
							&& rangeRequiresFix( range ) )
					{
						...
						if ( next && !fillingChar.getPrevious() && next.type == OMEDITOR.NODE_ELEMENT && next.getName() == 'br' )
						{
							removeFillingChar( this.document );
							range.moveToPosition( next, OMEDITOR.POSITION_BEFORE_START );
						}
						else
							range.moveToPosition( fillingChar, OMEDITOR.POSITION_AFTER_END );
					}
		 ...
		 * by calling the {@link OMEDITOR.dom.range.prototype.createBookmark} method,
		...
		/**
		 *  Create bookmark for every single of this selection range (from #getRanges)
		 * by calling the {@link OMEDITOR.dom.range.prototype.createBookmark2} method,
		 * with extra cares to avoid interferon among those ranges. Same arguments are
		 * received as with the underlay range method.
		 */
...
( function()
{
	var notWhitespaces = OMEDITOR.dom.walker.whitespaces( true ),
			fillerTextRegex = /\ufeff|\u00a0/,
			nonCells = { table:1,tbody:1,tr:1 };

	OMEDITOR.dom.range.prototype.select =
		OMEDITOR.env.ie ?
			// V2
			function( forceExpand )
			{
				...
				if ( this.startContainer.type == OMEDITOR.NODE_ELEMENT && this.startContainer.getName() in nonCells
					|| this.endContainer.type == OMEDITOR.NODE_ELEMENT && this.endContainer.getName() in nonCells )
				{
					this.shrink( OMEDITOR.NODE_ELEMENT, true );
				}
				...
				if ( collapsed )
				{
					...
					this.moveToPosition( dummySpan, OMEDITOR.POSITION_BEFORE_START );
					dummySpan.remove();
				}
				else
				{
