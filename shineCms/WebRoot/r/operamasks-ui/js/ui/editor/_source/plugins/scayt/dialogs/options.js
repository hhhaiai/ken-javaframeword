...
OMEDITOR.dialog.add( 'scaytcheck', function( editor )
{
	var firstLoad = true,
		captions,
		doc = OMEDITOR.document,
		editorName = editor.name,
		tags = OMEDITOR.plugins.scayt.getUiTabs( editor ),
		i,
		contents = [],
		userDicActive = 0,
		dic_buttons = [
	...
	var scayt_control = OMEDITOR.plugins.scayt.getScayt( editor );
	...
	var init_with_captions = function()
	{
		...
		var createOption = function( option, list )
		{
			...
			var radio = OMEDITOR.dom.element.createFromHtml( '<input id="cke_option' +
					option + '" type="radio" ' +
