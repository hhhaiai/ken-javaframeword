...
(function()
{
	var cssLength = OMEDITOR.tools.cssLength;
	...
	function getPreviousVisibleTab()
	{
		var tabId = this._.currentTabId,
			...
			tabIndex = OMEDITOR.tools.indexOf( this._.tabIdList, tabId ) + length;
	...
	function getNextVisibleTab()
	{
		var tabId = this._.currentTabId,
			length = this._.tabIdList.length,
			tabIndex = OMEDITOR.tools.indexOf( this._.tabIdList, tabId );
	...
	function clearOrRecoverTextInputValue( container, isRecover )
	{
		...
		{
			var item = new OMEDITOR.dom.element( inputs[ i ] );
	 ...
	 * @constructor
	 * @example
	 * var dialogObj = new OMEDITOR.dialog( editor, 'smiley' );
	 */
	OMEDITOR.dialog = function( editor, dialogName )
	{
		// Load the dialog definition.
		var definition = OMEDITOR.dialog._.dialogDefinitions[ dialogName ],
			defaultDefinition = OMEDITOR.tools.clone( defaultDialogDefinition ),
			buttonsOrder = editor.config.dialog_buttonsOrder || 'OS',
			dir = editor.lang.dir;

			if ( ( buttonsOrder == 'OS' && OMEDITOR.env.mac ) ||    // The buttons in MacOS Apps are in reverse order (#4750)
		...
		definition = OMEDITOR.tools.extend( definition( editor ), defaultDefinition );

		// Clone a functionally independent copy for this dialog.
		definition = OMEDITOR.tools.clone( definition );

		// Create a complex definition object, extending it with the API
		// functions.
		definition = new definitionObject( this, definition );

		var doc = OMEDITOR.document;
		...
		var startStyles = {
				position : OMEDITOR.env.ie6Compat ? 'absolute' : 'fixed',
				top : 0,
				visibility : 'hidden'
		};
		...
		this.definition = definition = OMEDITOR.fire( 'dialogDefinition',
			{
				name : dialogName,
		...
		function focusKeydownHandler( evt )
		{
			...
			if ( me != OMEDITOR.dialog._.currentTop )
				return;
			...
			processed = 0;
			if ( keystroke == 9 || keystroke == OMEDITOR.SHIFT + 9 )
			{
				var shiftPressed = ( keystroke == OMEDITOR.SHIFT + 9 );
			...
			else if ( keystroke == OMEDITOR.ALT + 121 && !me._.tabBarMode && me.getPageCount() > 1 )
			{
		...
		this.on( 'iframeAdded', function( evt )
			{
				var doc = new OMEDITOR.dom.document( evt.data.iframe.$.contentWindow.document );
		...
		this.on( 'show', function()
			{
				...
				else if ( !this._.hasFocus )
				{
					 ...
					 */
					if ( this._.editor.mode == 'wysiwyg' && OMEDITOR.env.ie )
					{
		...
		// Insert the title.
		( new OMEDITOR.dom.text( definition.title, OMEDITOR.document ) ).appendTo( this.parts.title );
		...
		// Insert buttons.
		var buttonsHtml = [],
			buttons = OMEDITOR.dialog._.uiElementBuilders.hbox.build( this,
				{
					type : 'hbox',
	...
	OMEDITOR.dialog.prototype =
	{
		...
		resize : (function()
		{
			return function( width, height )
			{
				...
				if ( this._.editor.lang.dir == 'rtl' && this._.position )
					this._.position.x = OMEDITOR.document.getWindow().getViewPaneSize().width -
		...
		move : (function()
		{
			...
			return function( x, y, save )
			{
				...
				if ( !isFixed )
				{
					var scrollPosition = OMEDITOR.document.getWindow().getScrollPosition();
					x += scrollPosition.x;
					y += scrollPosition.y;
				}
				...
				if ( rtl )
				{
					var dialogSize = this.getSize(),
						viewPaneSize = OMEDITOR.document.getWindow().getViewPaneSize();
		 ...
		 */
		getPosition : function(){ return OMEDITOR.tools.extend( {}, this._.position ); },
		...
		show : function()
		{
			...
			if ( !( element.getParent() && element.getParent().equals( OMEDITOR.document.getBody() ) ) )
				element.appendTo( OMEDITOR.document.getBody() );
			else
			...
			// Set z-index.
			if ( OMEDITOR.dialog._.currentZIndex === null )
				OMEDITOR.dialog._.currentZIndex = this._.editor.config.baseFloatZIndex;
			this._.element.getFirst().setStyle( 'z-index', OMEDITOR.dialog._.currentZIndex += 10 );

			// Maintain the dialog ordering and dialog cover.
			// Also register key handlers if first dialog.
			if ( OMEDITOR.dialog._.currentTop === null )
			{
				OMEDITOR.dialog._.currentTop = this;
				this._.parentDialog = null;
				showCover( this._.editor );

				element.on( 'keydown', accessKeyDownHandler );
				element.on( OMEDITOR.env.opera ? 'keypress' : 'keyup', accessKeyUpHandler );
			...
			{
				this._.parentDialog = OMEDITOR.dialog._.currentTop;
		...
		layout : function()
		{
			var viewSize = OMEDITOR.document.getWindow().getViewPaneSize(),
		 ...
		 * @returns {OMEDITOR.dialog} The current dialog object.
		 */
		foreach : function( fn )
		{
		...
		/**
		 * Calls the {@link OMEDITOR.dialog.definition.uiElement#setup} method of each of the UI elements, with the arguments passed through it.
		...
		/**
		 * Calls the {@link OMEDITOR.dialog.definition.uiElement#commit} method of each of the UI elements, with the arguments passed through it.
			...
			this.foreach( function( widget )
				{
					...
					if ( OMEDITOR.env.ie && this._.currentFocusIndex == widget.focusIndex )
		...
		hide : function()
		{
			...
			while( OMEDITOR.dialog._.currentTop != this )
				OMEDITOR.dialog._.currentTop.hide();
			...
			OMEDITOR.dialog._.currentTop = this._.parentDialog;

			// Deduct or clear the z-index.
			if ( !this._.parentDialog )
			{
				OMEDITOR.dialog._.currentZIndex = null;

				// Remove access key handlers.
				element.removeListener( 'keydown', accessKeyDownHandler );
				element.removeListener( OMEDITOR.env.opera ? 'keypress' : 'keyup', accessKeyUpHandler );
				...
				if ( editor.mode == 'wysiwyg' && OMEDITOR.env.ie )
				{
				...
				OMEDITOR.dialog._.currentZIndex -= 10;
		...
		{
			var pageHtml = [],
				titleHtml = contents.label ? ' title="' + OMEDITOR.tools.htmlEncode( contents.label ) + '"' : '',
				elements = contents.elements,
				vbox = OMEDITOR.dialog._.uiElementBuilders.vbox.build( this,
						{
							type : 'vbox',
			...
			var page = OMEDITOR.dom.element.createFromHtml( pageHtml.join( '' ) );
			page.setAttribute( 'role', 'tabpanel' );

			var env = OMEDITOR.env;
			var tabId = 'cke_' + contents.id + '_' + OMEDITOR.tools.getNextNumber(),
				 tab = OMEDITOR.dom.element.createFromHtml( [
		...
		selectPage : function( id )
		{
			...
			if ( OMEDITOR.env.ie6Compat || OMEDITOR.env.ie7Compat )
			{
			...
			this._.currentTabIndex = OMEDITOR.tools.indexOf( this._.tabIdList, id );
		},
		 ...
		 * @returns {OMEDITOR.dom.element} The &lt;span&gt; element containing this dialog.
		 * @example
		 ...
		 * @returns {OMEDITOR.ui.dialog.uiElement} The dialog UI element.
		 */
		 ...
		 * @example
		 * @returns {OMEDITOR.ui.dialog.button} The button object.
		 */
		 ...
		 * @returns {OMEDITOR.editor} Parent editor instances.
		 */
		 ...
		 * @returns {OMEDITOR.dom.element} The element that was selected, or null.
		 */
		 ...
		 *
		 * @param {OMEDITOR.dom.element} element
		 * @param {Number} [index]
		 */
	...
	OMEDITOR.tools.extend( OMEDITOR.dialog,
		/**
		 * @lends OMEDITOR.dialog
		 */
		{
			/**
			 * Registers a dialog.
			 ...
			 * return an object conforming to {@link OMEDITOR.dialog.definition}.
			 * @see OMEDITOR.dialog.definition
			 * @example
			 * // Full sample plugin, which does not only register a dialog window but also adds an item to the context menu.
			 * // To open the dialog window, choose "Open dialog" in the context menu.
			 * OMEDITOR.plugins.add( 'myplugin',
			 * {
			 * 	init: function( editor )
			 * 	{
			 * 		editor.addCommand( 'mydialog',new OMEDITOR.dialogCommand( 'mydialog' ) );
			 *
			 ...
			 * 			{
			 *  				return { 'My Dialog' : OMEDITOR.TRISTATE_OFF };
			 * 			});
			 * 		}
			 *
			 * 		<strong>OMEDITOR.dialog.add</strong>( 'mydialog', function( api )
			 * 		{
			 * 			// OMEDITOR.dialog.definition
			 ...
			 * 					}
			 * 				],
			 * 				buttons : [ OMEDITOR.dialog.okButton, OMEDITOR.dialog.cancelButton ],
			 * 				onOk : function() {
			 * 					// "this" is now a OMEDITOR.dialog object.
			 ...
			 * } );
			 *
			 * OMEDITOR.replace( 'editor1', { extraPlugins : 'myplugin' } );
			 */
			...
			getCurrent : function()
			{
				return OMEDITOR.dialog._.currentTop;
			},
			...
			okButton : (function()
			{
				...
				{
					return OMEDITOR.tools.extend( function( editor ){ return retval( editor, override ); },
							{ type : 'button' }, true );
				};
				return retval;
			})(),
	...
	OMEDITOR.event.implementOn( OMEDITOR.dialog );
	OMEDITOR.event.implementOn( OMEDITOR.dialog.prototype, true );

	var defaultDialogDefinition =
	{
		resizable : OMEDITOR.DIALOG_RESIZE_BOTH,
		minWidth : 600,
		minHeight : 400,
		buttons : [ OMEDITOR.dialog.okButton, OMEDITOR.dialog.cancelButton ]
	};
	 ...
	 * @constructor
	 * @name OMEDITOR.dialog.definitionObject
	 * @extends OMEDITOR.dialog.definition
	 * @example
	...
	var definitionObject = function( dialog, dialogDefinition )
	{
		...
		OMEDITOR.tools.extend( this, dialogDefinition );
	};

	definitionObject.prototype =
	/** @lends OMEDITOR.dialog.definitionObject.prototype */
	{
		/**
		 * Gets a content definition.
		 * @param {String} id The id of the content definition.
		 * @returns {OMEDITOR.dialog.definition.content} The content definition
		 *		matching id.
		 */
		 ...
		 * @returns {OMEDITOR.dialog.definition.button} The button definition
		 *		matching id.
		 */
		getButton : function( id )
		{
		 ...
		 * @param {OMEDITOR.dialog.definition.content} contentDefinition The
		 *		content definition.
		 * @param {String} [nextSiblingId] The id of an existing content
		 *		definition which the new content definition will be inserted
		 *		before. Omit if the new content definition is to be inserted as
		 *		the last item.
		 * @returns {OMEDITOR.dialog.definition.content} The inserted content
		 *		definition.
		 */
		 ...
		 * @param {OMEDITOR.dialog.definition.button} buttonDefinition The
		 *		button definition.
		 * @param {String} [nextSiblingId] The id of an existing button
		 *		definition which the new button definition will be inserted
		 *		before. Omit if the new button definition is to be inserted as
		 *		the last item.
		 * @returns {OMEDITOR.dialog.definition.button} The inserted button
		 *		definition.
		 */
		 ...
		 * @returns {OMEDITOR.dialog.definition.content} The removed content
		 *		definition.
		 */
		 ...
		 * @returns {OMEDITOR.dialog.definition.button} The removed button
		 *		definition.
		 */
	 ...
	 * OMEDITOR.dialog.definitionObject.
	 * @constructor
	 * @name OMEDITOR.dialog.definition.contentObject
	 * @example
	 * OMEDITOR.on( 'dialogDefinition', function( evt )
	 * 	{
	 * 		var definition = evt.data.definition;
	...
	function contentObject( dialog, contentDefinition )
	{
		...
		OMEDITOR.tools.extend( this, contentDefinition );
	}

	contentObject.prototype =
	/** @lends OMEDITOR.dialog.definition.contentObject.prototype */
	{
		/**
		 * Gets a UI element definition under the content definition.
		 * @param {String} id The id of the UI element definition.
		 * @returns {OMEDITOR.dialog.definition.uiElement}
		 */
		get : function( id )
		{
		 ...
		 * @param {OMEDITOR.dialog.definition.uiElement} elementDefinition The
		 *		UI elemnet definition to be added.
		 * @param {String} nextSiblingId The id of an existing UI element
		 *		definition which the new UI element definition will be inserted
		 *		before. Omit if the new button definition is to be inserted as
		 *		the last item.
		 * @returns {OMEDITOR.dialog.definition.uiElement} The element
		 *		definition inserted.
		 */
	...
	function initDragAndDrop( dialog )
	{
		...
		{
			OMEDITOR.document.removeListener( 'mousemove', mouseMoveHandler );
			OMEDITOR.document.removeListener( 'mouseup', mouseUpHandler );
		...
		dialog.parts.title.on( 'mousedown', function( evt )
			{
				...
				OMEDITOR.document.on( 'mousemove', mouseMoveHandler );
				OMEDITOR.document.on( 'mouseup', mouseUpHandler );
	...
	function initResizeHandles( dialog )
	{
		...
		if ( resizable == OMEDITOR.DIALOG_RESIZE_NONE )
			return;
		...
		var mouseDownFn = OMEDITOR.tools.addFunction( function( $event )
		{
			...
			if ( iframeDialog )
			{
				dialogCover = OMEDITOR.dom.element.createFromHtml( '<div class="cke_dialog_resize_cover" style="height: 100%; position: absolute; width: 100%;"></div>' );
				content.append( dialogCover );
			}

			// Calculate the offset between content and chrome size.
			wrapperHeight = startSize.height - dialog.parts.contents.getSize( 'height',  ! ( OMEDITOR.env.gecko || OMEDITOR.env.opera || OMEDITOR.env.ie && OMEDITOR.env.quirks ) );
			wrapperWidth = startSize.width - dialog.parts.contents.getSize( 'width', 1 );

			origin = { x : $event.screenX, y : $event.screenY };

			viewSize = OMEDITOR.document.getWindow().getViewPaneSize();
		...
		{
			var direction = '';
			if ( resizable == OMEDITOR.DIALOG_RESIZE_WIDTH )
				direction = ' cke_resizer_horizontal';
			else if ( resizable == OMEDITOR.DIALOG_RESIZE_HEIGHT )
				direction = ' cke_resizer_vertical';
			var resizer = OMEDITOR.dom.element.createFromHtml( '<div' +
					' class="cke_resizer' + direction + ' cke_resizer_' + editor.lang.dir + '"' +
					' title="' + OMEDITOR.tools.htmlEncode( editor.lang.resize ) + '"' +
					' onmousedown="OMEDITOR.tools.callFunction(' + mouseDownFn + ', event )"></div>' );
			dialog.parts.footer.append( resizer, 1 );
		});
		editor.on( 'destroy', function() { OMEDITOR.tools.removeFunction( mouseDownFn ); } );
		...
		function mouseMoveHandler( evt )
		{
			...
			if ( ( resizable == OMEDITOR.DIALOG_RESIZE_WIDTH || resizable == OMEDITOR.DIALOG_RESIZE_BOTH ) )
				width = Math.max( def.minWidth || 0, internalWidth - wrapperWidth );

			if ( resizable == OMEDITOR.DIALOG_RESIZE_HEIGHT || resizable == OMEDITOR.DIALOG_RESIZE_BOTH )
	...
	function showCover( editor )
	{
		...
		{
			var html = [
					'<div tabIndex="-1" style="position: ', ( OMEDITOR.env.ie6Compat ? 'absolute' : 'fixed' ),
					'; z-index: ', baseFloatZIndex,
					'; top: 0px; left: 0px; ',
					( !OMEDITOR.env.ie6Compat ? 'background-color: ' + backgroundColorStyle : '' ),
			...
			coverElement = OMEDITOR.dom.element.createFromHtml( html.join( '' ) );
			...
			coverElement.appendTo( OMEDITOR.document.getBody() );
		...
		var scrollFunc = function()
		{
			var pos = win.getScrollPosition(),
				cursor = OMEDITOR.dialog._.currentTop;
			coverElement.setStyles(
					{
						left : pos.x + 'px',
						top : pos.y + 'px'
		...
		if ( !( OMEDITOR.env.mac && OMEDITOR.env.webkit ) )
			coverElement.focus();
	...
	(function()
	{
		OMEDITOR.ui.dialog =
		{
			 ...
			 * @constructor
			 * @param {OMEDITOR.dialog} dialog Parent dialog object.
			 * @param {OMEDITOR.dialog.definition.uiElement} elementDefinition Element
			 * definition. Accepted fields:
			 * <ul>
			 * 	<li><strong>id</strong> (Required) The id of the UI element. See {@link
			 * 	OMEDITOR.dialog#getContentElement}</li>
			 ...
			 * 	refer to {@link OMEDITOR.ui.dialog.uiElement#registerEvents} and
			 * 	{@link OMEDITOR.ui.dialog.uiElement#eventProcessors} for more
			 * 	information.</li>
			 * </ul>
			...
			uiElement : function( dialog, elementDefinition, htmlList, nodeNameArg, stylesArg, attributesArg, contentsArg )
			{
				...
				var nodeName = ( nodeNameArg.call ? nodeNameArg( elementDefinition ) : nodeNameArg ) || 'div',
					...
					domId = this.domId = attributes.id || OMEDITOR.tools.getNextId() + '_uiElement',
				...
				for ( i in attributes )
					html.push( i + '="' + OMEDITOR.tools.htmlEncode( attributes[i] ) + '" ');
				...
				{
						this.setValue = OMEDITOR.tools.override( this.setValue, function( org )
						{
				...
				{
						this.getValue = OMEDITOR.tools.override( this.getValue, function( org )
						{
				...
				// Add events.
				OMEDITOR.event.implementOn( this );
				...
				// definition.
				OMEDITOR.tools.extend( this, elementDefinition );
			},

			/**
			 * Horizontal layout box for dialog UI elements, auto-expends to available width of container.
			 * @constructor
			 * @extends OMEDITOR.ui.dialog.uiElement
			 * @param {OMEDITOR.dialog} dialog
			 * Parent dialog object.
			 * @param {Array} childObjList
			 * Array of {@link OMEDITOR.ui.dialog.uiElement} objects inside this
			 * container.
			 ...
			 * @param {OMEDITOR.dialog.definition.uiElement} elementDefinition
			...
			hbox : function( dialog, childObjList, childHtmlList, htmlList, elementDefinition )
			{
				...
				var innerHTML = function()
				{
					...
					for ( i = 0 ; i < childHtmlList.length ; i++ )
					{
						...
						if ( OMEDITOR.env.ie && OMEDITOR.env.quirks && children[ i ].align )
				...
				OMEDITOR.ui.dialog.uiElement.call(
					this,
					dialog,
			 ...
			 * @constructor
			 * @extends OMEDITOR.ui.dialog.hbox
			 * @param {OMEDITOR.dialog} dialog
			 * Parent dialog object.
			 * @param {Array} childObjList
			 * Array of {@link OMEDITOR.ui.dialog.uiElement} objects inside this
			...
			vbox : function( dialog, childObjList, childHtmlList, htmlList, elementDefinition )
			{
				...
				var innerHTML = function()
				{
					...
					html.push( '"' );
					html.push( 'align="', OMEDITOR.tools.htmlEncode(
				...
				OMEDITOR.ui.dialog.uiElement.call( this, dialog, elementDefinition || { type : 'vbox' }, htmlList, 'div', null, { role : 'presentation' }, innerHTML );
			}
		};
	})();

	OMEDITOR.ui.dialog.uiElement.prototype =
	{
		/**
		 * Gets the root DOM element of this dialog UI object.
		 * @returns {OMEDITOR.dom.element} Root DOM element of UI object.
		 * @example
		 * uiElement.getElement().hide();
		 */
		getElement : function()
		{
			return OMEDITOR.document.getById( this.domId );
		},
		 ...
		 * element.
		 * @returns {OMEDITOR.dom.element} The element where the user input values.
		 * @example
		 ...
		 * @returns {OMEDITOR.dialog} Parent dialog object.
		 * @example
		 ...
		 * @returns {OMEDITOR.dialog.uiElement} The current UI element.
		 * @example
		 ...
		 *  	of {@link OMEDITOR.ui.dialog.uiElement#getInputElement}).
		 *  </li>
		 * </ol>
		 * This function is only called at UI element instantiation, but can
		 * be overridded in child classes if they require more flexibility.
		 * @param {OMEDITOR.dialog.definition.uiElement} definition The UI element
		 * definition.
		 * @returns {OMEDITOR.dialog.uiElement} The current UI element.
		 * @example
		 */
		 ...
		 * {@link OMEDITOR.ui.dialog.uiElement#getInputElement} at UI element
		 ...
		 * @type Object
		 * @example
		 * // This connects the 'click' event in OMEDITOR.ui.dialog.button to onClick
		 * // handlers in the UI element's definitions.
		 * OMEDITOR.ui.dialog.button.eventProcessors = OMEDITOR.tools.extend( {},
		 * &nbsp;&nbsp;OMEDITOR.ui.dialog.uiElement.prototype.eventProcessors,
		 ...
		 * @param {OMEDITOR.dialog} dialog The parent dialog object.
	...
	OMEDITOR.ui.dialog.hbox.prototype = OMEDITOR.tools.extend( new OMEDITOR.ui.dialog.uiElement,
		/**
		 * @lends OMEDITOR.ui.dialog.hbox.prototype
		 */
		{
			/**
			 ...
			 * @returns {Array|OMEDITOR.ui.dialog.uiElement} Array of all UI elements in the container
	...
	OMEDITOR.ui.dialog.vbox.prototype = new OMEDITOR.ui.dialog.hbox();
		...
		var commonBuilder = {
			build : function( dialog, elementDefinition, output )
			{
				...
				for ( var i = 0 ; ( i < children.length && ( child = children[i] ) ) ; i++ )
				{
					...
					childObjList.push( OMEDITOR.dialog._.uiElementBuilders[ child.type ].build( dialog, child, childHtml ) );
				}
				return new OMEDITOR.ui.dialog[elementDefinition.type]( dialog, childObjList, childHtmlList, output, elementDefinition );
			}
		};

		OMEDITOR.dialog.addUIElement( 'hbox', commonBuilder );
		OMEDITOR.dialog.addUIElement( 'vbox', commonBuilder );
	})();

	/**
	 * Generic dialog command. It opens a specific dialog when executed.
	 * @constructor
	 * @augments OMEDITOR.commandDefinition
	 * @param {string} dialogName The name of the dialog to open when executing
	 *		this command.
	 * @example
	 * // Register the "link" command, which opens the "link" dialog.
	 * editor.addCommand( 'link', <b>new OMEDITOR.dialogCommand( 'link' )</b> );
	 */
	OMEDITOR.dialogCommand = function( dialogName )
	{
		this.dialogName = dialogName;
	};

	OMEDITOR.dialogCommand.prototype =
	{
		/** @ignore */
		...
		editorFocus : OMEDITOR.env.ie || OMEDITOR.env.webkit
	};
		...
		OMEDITOR.dialog.validate =
		{
			functions : function()
			{
				...
				return function()
				{
					...
					var msg = undefined,
						relation = OMEDITOR.VALIDATE_AND,
						functions = [], i;
					...
					var passed = ( relation == OMEDITOR.VALIDATE_AND ? true : false );
					for ( i = 0 ; i < functions.length ; i++ )
					{
						if ( relation == OMEDITOR.VALIDATE_AND )
			...
			{
				return this.functions( function( val ){ return cssLengthRegex.test( OMEDITOR.tools.trim( val ) ); }, msg );
			},

			'htmlLength' : function( msg )
			{
				return this.functions( function( val ){ return htmlLengthRegex.test( OMEDITOR.tools.trim( val ) ); }, msg );
			},
	...
	OMEDITOR.on( 'instanceDestroyed', function( evt )
	{
		// Remove dialog cover on last instance destroy.
		if ( OMEDITOR.tools.isEmpty( OMEDITOR.instances ) )
		{
			var currentTopDialog;
			while ( ( currentTopDialog = OMEDITOR.dialog._.currentTop ) )
	...
	// Extend the OMEDITOR.editor class with dialog specific functions.
	OMEDITOR.tools.extend( OMEDITOR.editor.prototype,
		/** @lends OMEDITOR.editor.prototype */
		{
			 ...
			 * @see OMEDITOR.dialog.add
			 * @example
			 * OMEDITOR.instances.editor1.openDialog( 'smiley' );
			 * @returns {OMEDITOR.dialog} The dialog object corresponding to the dialog displayed. null if the dialog name is not registered.
			 */
			openDialog : function( dialogName, callback )
			{
				if ( this.mode == 'wysiwyg' && OMEDITOR.env.ie )
				{
					var selection = this.getSelection();
					selection && selection.lock();
				}

				var dialogDefinitions = OMEDITOR.dialog._.dialogDefinitions[ dialogName ],
				...
				if ( typeof dialogDefinitions == 'function' && dialogSkin._isLoaded )
				{
					...
					var dialog = storedDialogs[ dialogName ] ||
						( storedDialogs[ dialogName ] = new OMEDITOR.dialog( this, dialogName ) );
				...
				{
					hideCover();
					throw new Error( '[OMEDITOR.dialog.openDialog] Dialog "' + dialogName + '" failed when loading definition.' );
				}
				...
				{
					var dialogDefinition = OMEDITOR.dialog._.dialogDefinitions[ dialogName ],
							skin = me.skin.dialog;
					...
					if ( typeof dialogDefinition != 'function' )
						OMEDITOR.dialog._.dialogDefinitions[ dialogName ] = 'failed';
				...
				if ( typeof dialogDefinitions == 'string' )
				{
					...
					OMEDITOR.scriptLoader.load( OMEDITOR.getUrl( dialogDefinitions ), onDialogFileLoaded, null, 0, 1 );
				}

				OMEDITOR.skins.load( this, 'dialog', onDialogFileLoaded );
 ...
 * string.
 * @name OMEDITOR.config.dialog_backgroundCoverColor
 * @type String
 * @default 'white'
 * @example
 ...
 * range [0.0, 1.0].
 * @name OMEDITOR.config.dialog_backgroundCoverOpacity
 * @type Number
 * @default 0.5
 * @example
 ...
 * @name OMEDITOR.config.dialog_startupFocusTab
 * @type Boolean
 * @default false
 * @example
 ...
 * measured in pixels.
 * @name OMEDITOR.config.dialog_magnetDistance
 * @type Number
 * @default 20
 * @example
 ...
 * </ul>
 * @name OMEDITOR.config.dialog_buttonsOrder
 * @type String
 * @default 'OS'
 * @since 3.5
 * @example
 ...
 * @name OMEDITOR.config.removeDialogTabs
 * @type String
 * @since 3.5
 * @default ''
 * @example
 ...
 * not get fired.</p>
 * @name OMEDITOR#dialogDefinition
 * @event
 * @param {OMEDITOR.dialog.definition} data The dialog defination that
 *		is being loaded.
 * @param {OMEDITOR.editor} editor The editor instance that will use the
 *		dialog.
 */

/**
 * Fired when a tab is going to be selected in a dialog
 * @name OMEDITOR.dialog#selectPage
 * @event
 ...
 * @name OMEDITOR.dialog#show
 * @event
 */

/**
 * Fired when a dialog is shown
 * @name OMEDITOR.editor#dialogShow
 * @event
 */

/**
 * Fired when a dialog is hidden
 * @name OMEDITOR.dialog#hide
 * @event
 */

/**
 * Fired when a dialog is hidden
 * @name OMEDITOR.editor#dialogHide
 * @event
 */

/**
 * Fired when a dialog is being resized. The event is fired on
 * both the 'OMEDITOR.dialog' object and the dialog instance
 * since 3.5.3, previously it's available only in the global object.
 * @name OMEDITOR.dialog#resize
 * @since 3.5
 * @event
 * @param {OMEDITOR.dialog} dialog The dialog being resized (if
