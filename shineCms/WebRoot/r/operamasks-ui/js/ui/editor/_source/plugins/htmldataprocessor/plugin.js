...
(function()
{
	...
	function lastNoneSpaceChild( block )
	{
		...
		while (  last && last.type == OMEDITOR.NODE_TEXT && !OMEDITOR.tools.trim( last.value ) )
	...
	function trimFillers( block, fromSource )
	{
		...
		if ( lastChild )
		{
			if ( ( fromSource || !OMEDITOR.env.ie ) && lastChild.type == OMEDITOR.NODE_ELEMENT && lastChild.name == 'br' )
				children.pop();
			if ( lastChild.type == OMEDITOR.NODE_TEXT && tailNbspRegex.test( lastChild.value ) )
				children.pop();
		}
	}
	...
	function blockNeedsExtension( block, fromSource, extendEmptyBlock )
	{
        ...
        // (#6248)
        if ( fromSource && OMEDITOR.env.ie &&
                ( document.documentMode > 7
                || block.name in OMEDITOR.dtd.tr
                || block.name in OMEDITOR.dtd.$listItem ) )
            return false;

		var lastChild = lastNoneSpaceChild( block );

		return !lastChild || lastChild &&
				( lastChild.type == OMEDITOR.NODE_ELEMENT && lastChild.name == 'br'
	...
	function getBlockExtension( isOutput, emptyBlockFiller )
	{
		return function( node )
		{
			...
			{
				if ( isOutput || OMEDITOR.env.ie )
					node.add( new OMEDITOR.htmlParser.text( '\xa0' ) );
				else
					node.add( new OMEDITOR.htmlParser.element( 'br', {} ) );
			}
		};
	}
	...
	var blockLikeTags = OMEDITOR.tools.extend( {}, dtd.$block, dtd.$listItem, dtd.$tableContent );
	...
	var defaultHtmlFilterRules =
		{
			...
			elements :
			{
				...
				table : function( element )
				{
					...
					children.sort( function ( node1, node2 )
								   {
									   return node1.type == OMEDITOR.NODE_ELEMENT && node2.type == node1.type ?
											OMEDITOR.tools.indexOf( tableOrder, node1.name )  > OMEDITOR.tools.indexOf( tableOrder, node2.name ) ? 1 : -1 : 0;
								   } );
				},
				...
				pre : function( element ) { OMEDITOR.env.ie && trimFillers( element ); },
				...
				style : function( element )
				{
					...
					child && child.value && ( child.value = OMEDITOR.tools.trim( child.value ));
			...
			attributes :
			{
				'class' : function( value, element )
				{
					...
					return OMEDITOR.tools.ltrim( value.replace( /(?:^|\s+)cke_[^\s]*/g, '' ) ) || false;
				}
			}
		};
	...
	OMEDITOR.plugins.add( 'htmldataprocessor',
	{
		requires : [ 'htmlwriter' ],

		init : function( editor )
		{
			var dataProcessor = editor.dataProcessor = new OMEDITOR.htmlDataProcessor( editor );
		...
		onLoad : function()
		{
			! ( 'fillEmptyBlocks' in OMEDITOR.config ) && ( OMEDITOR.config.fillEmptyBlocks = 1 );
		}
	});

	OMEDITOR.htmlDataProcessor = function( editor )
	{
		this.editor = editor;

		this.writer = new OMEDITOR.htmlWriter();
		this.dataFilter = new OMEDITOR.htmlParser.filter();
		this.htmlFilter = new OMEDITOR.htmlParser.filter();
	};

	OMEDITOR.htmlDataProcessor.prototype =
	{
		toHtml : function( data, fixForBody )
		{
			...
			// structure.
			var div = new OMEDITOR.dom.element( 'div' );
			...
			var fragment = OMEDITOR.htmlParser.fragment.fromHtml( data, fixForBody ),
		...
		toDataFormat : function( html, fixForBody )
		{
			var writer = this.writer,
				fragment = OMEDITOR.htmlParser.fragment.fromHtml( html, fixForBody );
 ...
 * @name OMEDITOR.config.forceSimpleAmpersand
 * @name OMEDITOR.config.forceSimpleAmpersand
 * @type Boolean
 * @default false
 * @example
 ...
 * it'll be passed a {@link OMEDITOR.htmlParser.element} to decide whether adding the filler text
 * by expecting a boolean return value.
 * @name OMEDITOR.config.fillEmptyBlocks
 * @since 3.5
 * @type Boolean
 * @default true
 * @example
