...
(function()
{
	...
	var notWhitespaceEval = OMEDITOR.dom.walker.whitespaces( true );

	// Elements that could blink the cursor anchoring beside it, like hr, page-break. (#6554)
	function nonEditable( element )
	{
		return element.isBlockBoundary() && OMEDITOR.dtd.$empty[ element.getName() ];
	}
	...
	function doInsertHtml( data )
	{
		...
		{
			var path = new OMEDITOR.dom.elementPath( range.startContainer );
			if ( path.block )
			{
				var nodes = OMEDITOR.htmlParser.fragment.fromHtml( data, false ).children;
				for ( var i = 0, count = nodes.length; i < count; i++ )
				{
					if ( nodes[ i ]._.isBlockLike )
					{
						range.splitBlock( this.enterMode == OMEDITOR.ENTER_DIV ? 'div' : 'p' );
		...
		if ( OMEDITOR.env.ie )
		{
			...
			if ( $sel.type == 'Control' )
				$sel.clear();
			else if ( selection.getType() == OMEDITOR.SELECTION_TEXT )
			{
				...
				if ( endContainer &&
						endContainer.type == OMEDITOR.NODE_ELEMENT &&
						endContainer.getAttribute( 'contenteditable' ) == 'false' &&
						range.checkBoundaryOfElement( endContainer, OMEDITOR.END ) )
				{
	...
	function doInsertText( text )
	{
		var selection = this.getSelection(),
			mode = selection.getStartElement().hasAscendant( 'pre', true ) ?
				   OMEDITOR.ENTER_BR : this.config.enterMode,
			isEnterBrMode = mode == OMEDITOR.ENTER_BR;

		var html = OMEDITOR.tools.htmlEncode( text.replace( /\r\n|\r/g, '\n' ) );
		...
		var paragraphTag = mode == OMEDITOR.ENTER_P ? 'p' : 'div';
		...
		if ( !( isEnterBrMode || OMEDITOR.env.ie ) )
		{
			html = html.replace( new RegExp( '<br>(?=</' + paragraphTag + '>)' ), function( match )
			{
				return OMEDITOR.tools.repeat( match, 2 );
			} );
		}

		// Inline styles have to be inherited in Firefox.
		if ( OMEDITOR.env.gecko || OMEDITOR.env.webkit )
		{
			var path = new OMEDITOR.dom.elementPath( selection.getStartElement() ),
				context = [];

			for ( var i = 0; i < path.elements.length; i++ )
			{
				var tag = path.elements[ i ].getName();
				if ( tag in OMEDITOR.dtd.$inline )
					context.unshift( path.elements[ i ].getOuterHtml().match( /^<.*?>/) );
				else if ( tag in OMEDITOR.dtd.$block )
					break;
			}
	...
	function doInsertElement( element )
	{
		var selection = this.getSelection(),
				...
				isBlock = OMEDITOR.dtd.$block[ elementName ];
		...
		for ( var i = ranges.length - 1 ; i >= 0 ; i-- )
		{
				...
				if ( !range.checkReadOnly() )
				{
					...
					if ( isBlock )
					{
						while ( ( current = range.getCommonAncestor( 0, 1 ) )
								&& ( dtd = OMEDITOR.dtd[ current.getName() ] )
								&& !( dtd && dtd [ elementName ] ) )
						{
							// Split up inline elements.
							if ( current.getName() in OMEDITOR.dtd.span )
			...
			if ( lastElement )
			{
				range.moveToPosition( lastElement, OMEDITOR.POSITION_AFTER_END );
				...
				if ( isBlock )
				{
					var next = lastElement.getNext( notWhitespaceEval ),
						nextName = next && next.type == OMEDITOR.NODE_ELEMENT && next.getName();

					// Check if it's a block element that accepts text.
					if ( nextName && OMEDITOR.dtd.$block[ nextName ] && OMEDITOR.dtd[ nextName ]['#'] )
	...
	var isNotWhitespace = OMEDITOR.dom.walker.whitespaces( true ),
		isNotBookmark = OMEDITOR.dom.walker.bookmark( false, true );
	...
	function isNbsp( node )
	{
		return node.type == OMEDITOR.NODE_TEXT
			   && OMEDITOR.tools.trim( node.getText() ).match( /^(?:&nbsp;|\xa0)$/ );
	}
	...
	isNotWhitespace = OMEDITOR.dom.walker.whitespaces( true );
	...
	function activateEditing( editor )
	{
		...
		if ( !bodyChildsNum
			|| bodyChildsNum == 1
				&& bodyFirstChild.type == OMEDITOR.NODE_ELEMENT
				&& bodyFirstChild.hasAttribute( '_moz_editor_bogus_node' ) )
		{
			...
			var nativeRange = new OMEDITOR.dom.range( doc );
			nativeRange.setStartAt( body , OMEDITOR.POSITION_AFTER_START );
			nativeRange.select();
		}
	}
	...
	function onSelectionChangeFixBody( evt )
	{
		...
		if ( OMEDITOR.env.gecko )
		{
			...
			if ( pathBlock
					...
					&& !( lastNode && lastNode.type == OMEDITOR.NODE_ELEMENT && lastNode.isBlockBoundary() )
					&& !pathBlock.is( 'pre' )
					&& !pathBlock.getBogus() )
			{
				...
				&& !path.block )
		{
			editor.fire( 'updateSnapshot' );
			restoreDirty( editor );
			OMEDITOR.env.ie && restoreSelection( selection );

			var fixedBlock = range.fixBlock( true,
					editor.config.enterMode == OMEDITOR.ENTER_DIV ? 'div' : 'p'  );
			...
			if ( isBlankParagraph( fixedBlock ) )
			{
				...
				if ( element &&
					 element.type == OMEDITOR.NODE_ELEMENT &&
					 !nonEditable( element ) )
				{
		...
		var testRange = new OMEDITOR.dom.range( editor.document );
		testRange.moveToElementEditEnd( editor.document.getBody() );
		var testPath = new OMEDITOR.dom.elementPath( testRange.startContainer );
		if ( !testPath.blockLimit.is( 'body') )
		{
			...
			var paddingBlock;
			if ( enterMode != OMEDITOR.ENTER_BR )
				paddingBlock = body.append( editor.document.createElement( enterMode == OMEDITOR.ENTER_P ? 'p' : 'div' ) );
			else
				paddingBlock = body;
	...
	OMEDITOR.plugins.add( 'wysiwygarea',
	{
		requires : [ 'editingblock' ],

		init : function( editor )
		{
			var fixForBody = ( editor.config.enterMode != OMEDITOR.ENTER_BR && editor.config.autoParagraph !== false )
				? editor.config.enterMode == OMEDITOR.ENTER_DIV ? 'div' : 'p' : false;
			...
			editor.on( 'editingBlockReady', function()
				{
					...
					var createIFrame = function( data )
					{
						...
						iframe = OMEDITOR.dom.element.createFromHtml( '<iframe' +
  							' style="width:100%;height:100%"' +
  							' frameBorder="0"' +
  							' title="' + frameLabel + '"' +
  							' src="' + src + '"' +
							' tabIndex="' + ( OMEDITOR.env.webkit? -1 : editor.tabIndex ) + '"' +
  							' allowTransparency="true"' +
  							'></iframe>' );

						// Running inside of Firefox chrome the load event doesn't bubble like in a normal page (#5689)
						if ( document.location.protocol == 'chrome:' )
							OMEDITOR.event.useCapture = true;
						...
						iframe.on( 'load', function( ev )
							{
								...
								OMEDITOR.env.air && contentDomReady( doc.getWindow().$ );
							});

						// Reset adjustment back to default (#5689)
						if ( document.location.protocol == 'chrome:' )
							OMEDITOR.event.useCapture = false;
					...
					contentDomReadyHandler = OMEDITOR.tools.addFunction( contentDomReady );
					var activationScript =
						'<script id="cke_actscrpt" type="text/javascript" data-cke-temp="1">' +
							( isCustomDomain ? ( 'document.domain="' + document.domain + '";' ) : '' ) +
							'window.parent.OMEDITOR.tools.callFunction( ' + contentDomReadyHandler + ', window );' +
						'</script>';
					...
					function contentDomReady( domWindow )
					{
						...
						editable && OMEDITOR.env.gecko && OMEDITOR.tools.setTimeout( activateEditing, 0, null, editor );

						domWindow	= editor.window	= new OMEDITOR.dom.window( domWindow );
						domDocument	= editor.document	= new OMEDITOR.dom.document( domDocument );
						...
						OMEDITOR.env.ie && domDocument.on( 'click', function( evt )
						{
							var element = evt.data.getTarget();
							if ( element.is( 'input' ) )
							{
						...
						if ( OMEDITOR.env.gecko )
						{
							domDocument.on( 'mouseup', function( ev )
							{
								if ( ev.data.$.button == 2 )
								{
									...
									{
										var range = new OMEDITOR.dom.range( domDocument );
						...
						if ( editable &&
								OMEDITOR.env.ie && domDocument.$.compatMode == 'CSS1Compat'
								|| OMEDITOR.env.gecko
								|| OMEDITOR.env.opera )
						{
						...
						var focusTarget = OMEDITOR.env.ie ? iframe : domWindow;
						...
						focusTarget.on( 'focus', function()
							{
								...
								if ( editable && OMEDITOR.env.gecko && OMEDITOR.env.version >= 10900 )
									blinkCursor();
								else if ( OMEDITOR.env.opera )
									doc.getBody().focus();
						...
						if ( OMEDITOR.env.ie )
						{
							...
							editor.config.enterMode != OMEDITOR.ENTER_P
						...
						setTimeout( function()
							{
								...
								if ( editor.config.disableObjectResizing )
								{
									...
									catch(e)
									{
										...
										editor.document.getBody().on( OMEDITOR.env.ie ? 'resizestart' : 'resize', function( evt )
										{
											evt.data.preventDefault();
										});
									}
					...
					editor.addMode( 'wysiwyg',
						{
							load : function( holderElement, data, isSnapshot )
							{
								...
								if ( OMEDITOR.env.ie && OMEDITOR.env.quirks )
							...
							loadData : function( data )
							{
								...
								!fullPage && ( headExtra =
									OMEDITOR.tools.buildStyleHtml( editor.config.contentsCss ) +
									headExtra );
							...
							focus : function()
							{
								...
								else if ( OMEDITOR.env.opera && editor.document )
								{
								...
								else if ( !OMEDITOR.env.opera && win )
								{
									// AIR needs a while to focus when moving from a link.
									OMEDITOR.env.air ? setTimeout( function () { win.focus(); }, 0 ) : win.focus();
									editor.selectionChange();
								}
							}
						});
			...
			if ( OMEDITOR.document.$.documentMode >= 8 )
			{
				editor.addCss( 'html.CSS1Compat [contenteditable=false]{ min-height:0 !important;}' );

				var selectors = [];
				for ( var tag in OMEDITOR.dtd.$removeEmpty )
			...
			if ( OMEDITOR.env.gecko || OMEDITOR.env.ie || OMEDITOR.env.opera )
			{
				var focusGrabber;
				editor.on( 'uiReady', function()
				{
					focusGrabber = editor.container.append( OMEDITOR.dom.element.createFromHtml(
				...
				{
					OMEDITOR.tools.removeFunction( contentDomReadyHandler );
			...
			editor.on( 'insertElement', function ( evt )
			{
				...
				if ( element.type == OMEDITOR.NODE_ELEMENT
			...
			else
			{
				...
				body.setAttribute( 'onpageshow', ( currentHandler ? currentHandler + ';' : '') +
							'event.persisted && (function(){' +
								'var allInstances = OMEDITOR.instances, editor, doc;' +
 ...
 */
OMEDITOR.config.disableObjectResizing = false;
 ...
 */
OMEDITOR.config.disableNativeTableHandles = true;

/**
 * Disables the built-in words spell checker if browser provides one.<br /><br />
 *
 * <strong>Note:</strong> Although word suggestions provided by browsers (natively) will not appear in OMEditor's default context menu,
 * users can always reach the native context menu by holding the <em>Ctrl</em> key when right-clicking if {@link OMEDITOR.config.browserContextMenuOnCtrl}
 ...
 */
OMEDITOR.config.disableNativeSpellChecker = true;
 ...
 */
OMEDITOR.config.ignoreEmptyParagraph = true;

/**
 * Fired when data is loaded and ready for retrieval in an editor instance.
 * @name OMEDITOR.editor#dataReady
 * @event
 */
 ...
 * @name OMEDITOR.config.autoParagraph
 * @since 3.6
 * @type Boolean
 * @default true
 * @example
 ...
 * @name OMEDITOR.editor#ariaWidget
 * @event
