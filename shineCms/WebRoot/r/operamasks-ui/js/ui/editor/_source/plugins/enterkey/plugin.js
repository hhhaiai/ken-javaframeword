...
(function()
{
	OMEDITOR.plugins.add( 'enterkey',
	{
		...
		init : function( editor )
		{
			...
			keystrokes[ 13 ] = 'enter';
			keystrokes[ OMEDITOR.SHIFT + 13 ] = 'shiftEnter';
		}
	});

	OMEDITOR.plugins.enterkey =
	{
		enterBlock : function( editor, mode, range, forceMode )
		{
			...
			var atBlockStart = range.checkStartOfBlock(),
				...
				path = new OMEDITOR.dom.elementPath( range.startContainer ),
				block = path.block;
			...
			else if ( block && OMEDITOR.dtd.$captionBlock[ block.getName() ] )
			{
				enterBr( editor, mode, range, forceMode );
				return;
			}
			...
			var blockTag = ( mode == OMEDITOR.ENTER_DIV ? 'div' : 'p' );
			...
			if ( !isStartOfBlock && !isEndOfBlock )
			{
				...
				if ( nextBlock.is( 'li' )
					 && ( node = nextBlock.getFirst( OMEDITOR.dom.walker.invisible( true ) ) )
					 && node.is && node.is( 'ul', 'ol' ) )
					( OMEDITOR.env.ie ? doc.createText( '\xa0' ) : doc.createElement( 'br' ) ).insertBefore( node );
			...
			else
			{
				...
				if ( elementPath )
				{
					for ( var i = 0, len = elementPath.elements.length ; i < len ; i++ )
					{
						...
						if ( OMEDITOR.dtd.$removeEmpty[ element.getName() ] )
						{
				...
				// empty table cell).
				if ( OMEDITOR.env.ie && isStartOfBlock && ( !isEndOfBlock || !previousBlock.getChildCount() ) )
				{
		...
		enterBr : function( editor, mode, range, forceMode )
		{
			...
			var elementPath = new OMEDITOR.dom.elementPath( editor.getSelection().getStartElement() );
			...
			if ( !forceMode && isEndOfBlock && headerTagRegex.test( startBlockTag ) )
			{
				...
				else
				{
					...
					range.setStartAt( startBlock.getNext(), OMEDITOR.env.ie ? OMEDITOR.POSITION_BEFORE_START : OMEDITOR.POSITION_AFTER_START );
				}
			}
			else
			{
				...
				if ( isPre && !OMEDITOR.env.gecko )
					lineBreak = doc.createText( OMEDITOR.env.ie ? '\r' : '\n' );
				else
				...
				if ( OMEDITOR.env.ie )
					range.setStartAt( lineBreak, OMEDITOR.POSITION_AFTER_END );
				else
				{
					// A text node is required by Gecko only to make the cursor blink.
					...
					range.setStartAt( lineBreak.getNext(), OMEDITOR.POSITION_AFTER_START );
	...
	var plugin = OMEDITOR.plugins.enterkey,
