...
OMEDITOR.plugins.add( 'colorbutton',
{
	...
	init : function( editor )
	{
		...
		{
			var colorBoxId = OMEDITOR.tools.getNextId() + '_colorBox';
			editor.ui.add( name, OMEDITOR.UI_PANELBUTTON,
				{
					label : title,
					title : title,
					...
					onOpen : function()
					{
						var selection = editor.getSelection(),
							...
							path = new OMEDITOR.dom.elementPath( block ),
							color;
		...
		function renderColors( panel, type, colorBoxId )
		{
			...
			var clickFn = OMEDITOR.tools.addFunction( function( color, type )
				{
					if ( color == '?' )
					{
					...
					new OMEDITOR.style( config['colorButton_' + type + 'Style'], { color : 'inherit' } ).remove( editor.document );
					...
					if ( color )
					{
						...
						new OMEDITOR.style( colorStyle, { color : color } ).apply( editor.document );
					}
			...
			output.push(
				'<a class="cke_colorauto" _cke_focus=1 hidefocus=true' +
					' title="', lang.auto, '"' +
					' onclick="OMEDITOR.tools.callFunction(', clickFn, ',null,\'', type, '\');return false;"' +
				...
				output.push(
					'<td>' +
						'<a class="cke_colorbox" _cke_focus=1 hidefocus=true' +
							' title="', colorLabel, '"' +
							' onclick="OMEDITOR.tools.callFunction(', clickFn, ',\'', colorName, '\',\'', type, '\'); return false;"' +
					...
					'<tr>' +
						'<td colspan=8 align=center>' +
							'<a class="cke_colormore" _cke_focus=1 hidefocus=true' +
								' title="', lang.more, '"' +
								' onclick="OMEDITOR.tools.callFunction(', clickFn, ',\'?\',\'', type, '\');return false;"' +
 ...
 * @name OMEDITOR.config.colorButton_enableMore
 * @default true
 * @type Boolean
 * @example
 ...
 * @name OMEDITOR.config.colorButton_colors
 * @type String
 ...
 */
OMEDITOR.config.colorButton_colors =
 ...
 * @name OMEDITOR.config.colorButton_foreStyle
 * @type Object
 * @example
 ...
 *     };
 */
OMEDITOR.config.colorButton_foreStyle =
	{
		element		: 'span',
		styles		: { 'color' : '#(color)' },
 ...
 * @name OMEDITOR.config.colorButton_backStyle
 * @type Object
 * @example
 * // This is basically the default setting value.
 ...
 *     };
 */
OMEDITOR.config.colorButton_backStyle =
	{
		element		: 'span',
		styles		: { 'background-color' : '#(color)' }
	};
