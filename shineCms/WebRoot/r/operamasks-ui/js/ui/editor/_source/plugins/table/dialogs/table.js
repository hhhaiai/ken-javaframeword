...
(function()
{
	var defaultToPixel = OMEDITOR.tools.cssLength;
	...
	function tableDialog( editor, command )
	{
		var makeElement = function( name )
			{
				return new OMEDITOR.dom.element( name, editor.document );
			};
		...
		return {
			...
			minWidth : 310,
			minHeight : OMEDITOR.env.ie ? 310 : 280,
				...
				if ( command == 'tableProperties' )
				{
					...
					else if ( ranges.length > 0 )
					{
						...
						if ( OMEDITOR.env.webkit )
							ranges[ 0 ].shrink( OMEDITOR.NODE_ELEMENT );
			...
			onOk : function()
			{
				...
				if ( data.info )
				{
					...
					{
						var thead = new OMEDITOR.dom.element( table.$.createTHead() );
						...
						for ( i = 0 ; i < theRow.getChildCount() ; i++ )
						{
							...
							if ( th.type == OMEDITOR.NODE_ELEMENT && !th.data( 'cke-bookmark' ) )
							{
								th.renameNode( 'th' );
					...
					if ( table.$.tHead !== null && !( headers == 'row' || headers == 'both' ) )
					{
						...
						thead = new OMEDITOR.dom.element( table.$.tHead );
						tbody = table.getElementsByTag( 'tbody' ).getItem( 0 );
						...
						while ( thead.getChildCount() > 0 )
						{
							...
							for ( i = 0; i < theRow.getChildCount() ; i++ )
							{
								...
								if ( newCell.type == OMEDITOR.NODE_ELEMENT )
								{
					...
					if ( !this.hasColumnHeaders && ( headers == 'col' || headers == 'both' ) )
					{
						...
						{
							newCell = new OMEDITOR.dom.element( table.$.rows[ row ].cells[ 0 ] );
					...
					if ( ( this.hasColumnHeaders ) && !( headers == 'col' || headers == 'both' ) )
					{
						for ( i = 0 ; i < table.$.rows.length ; i++ )
						{
							...
							{
								newCell = new OMEDITOR.dom.element( row.$.cells[0] );
								newCell.renameNode( 'td' );
								newCell.removeAttribute( 'scope' );
							}
						}
				...
				if ( !this._.selectedElement )
				{
					...
					setTimeout( function()
						{
							var firstCell = new OMEDITOR.dom.element( table.$.rows[ 0 ].cells[ 0 ] );
							var range = new OMEDITOR.dom.range( editor.document );
							range.moveToPosition( firstCell, OMEDITOR.POSITION_AFTER_START );
							range.select( 1 );
						}, 0 );
				}
									...
									[
										{
											...
											validate : function()
											{
												var pass = true,
													value = this.getValue();
												pass = pass && OMEDITOR.dialog.validate.integer()( value )
													&& value > 0;
												if ( !pass )
												{
										...
										},
										{
											...
											validate : OMEDITOR.dialog.validate['number']( editor.lang.table.invalidBorder ),
											setup : function( selectedTable )
											{
								...
								},
								{
									...
									[
											...
											[
												{
													...
													validate : OMEDITOR.dialog.validate.cssLength( editor.lang.common.invalidCssLength.replace( '%1', editor.lang.common.width ) ),
													onChange : function()
													{
											...
											[
												{
													...
													'default' : '',
													getValue : defaultToPixel,
													validate : OMEDITOR.dialog.validate.cssLength( editor.lang.common.invalidCssLength.replace( '%1', editor.lang.common.height ) ),
													onChange : function()
													{
														var styles = this.getDialog().getContentElement( 'advanced', 'advStyles' );
										...
										},
										{
											...
											'default' : 1,
											validate : OMEDITOR.dialog.validate.number( editor.lang.table.invalidCellSpacing ),
											setup : function( selectedTable )
											{
										...
										},
										{
											...
											'default' : 1,
											validate : OMEDITOR.dialog.validate.number( editor.lang.table.invalidCellPadding ),
											setup : function( selectedTable )
											{
							...
							[
								{
									...
									setup : function( selectedTable )
									{
										...
										if ( nodeList.count() > 0 )
										{
											...
											var firstElementChild = caption.getFirst( OMEDITOR.dom.walker.nodeType( OMEDITOR.NODE_ELEMENT ) );
											...
											caption = OMEDITOR.tools.trim( caption.getText() );
									...
									commit : function( data, table )
									{
										...
										if ( caption )
										{
											...
											{
												captionElement = new OMEDITOR.dom.element( 'caption', editor.document );
												if ( table.getChildCount() )
													captionElement.insertBefore( table.getFirst() );
												else
													captionElement.appendTo( table );
											}
											captionElement.append( new OMEDITOR.dom.text( caption, editor.document ) );
										}
	...
	OMEDITOR.dialog.add( 'table', function( editor )
		{
			return tableDialog( editor, 'table' );
		} );
	OMEDITOR.dialog.add( 'tableProperties', function( editor )
		{
