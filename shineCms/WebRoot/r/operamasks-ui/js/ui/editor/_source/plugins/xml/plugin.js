...
/**
 * @fileOverview Defines the {@link OMEDITOR.xml} class, which represents a
 *		loaded XML document.
 */

(function()
{
	OMEDITOR.plugins.add( 'xml', {});
	 ...
	 * @example
	 * var xml = <b>new OMEDITOR.xml( '<books><book title="My Book" /></books>' )</b>;
	 */
	OMEDITOR.xml = function( xmlObjectOrData )
	{
		var baseXml = null;
	...
	OMEDITOR.xml.prototype =
	{
		 ...
		 * var xml = new OMEDITOR.xml( '<list><item id="test1" /><item id="test2" /></list>' );
		...
		selectSingleNode : function( xpath, contextNode )
		{
			...
			{
				if ( OMEDITOR.env.ie || contextNode.selectSingleNode )	// IE
		...
		selectNodes : function( xpath, contextNode )
		{
			...
			if ( contextNode || ( contextNode = baseXml ) )
			{
				if ( OMEDITOR.env.ie || contextNode.selectNodes )		// IE
