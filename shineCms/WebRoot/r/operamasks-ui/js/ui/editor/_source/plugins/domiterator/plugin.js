...
OMEDITOR.plugins.add( 'domiterator' );

(function()
{
	/**
	 * @name OMEDITOR.dom.iterator
	 */
	...
	var beginWhitespaceRegex = /^[\r\n\t ]+$/,
		...
		bookmarkGuard = OMEDITOR.dom.walker.bookmark( false, true ),
		whitespacesGuard = OMEDITOR.dom.walker.whitespaces( true ),
	...
	iterator.prototype = {
		getNextParagraph : function( blockTag )
		{
			...
			if ( !this._.lastNode )
			{
				...
				range.shrink( OMEDITOR.NODE_ELEMENT, true );
				...
				range.enlarge( this.forceBrBreak && !touchPre || !this.enlargeBr ?
							   OMEDITOR.ENLARGE_LIST_ITEM_CONTENTS : OMEDITOR.ENLARGE_BLOCK_CONTENTS );

				var walker = new OMEDITOR.dom.walker( range ),
					ignoreBookmarkTextEvaluator = OMEDITOR.dom.walker.bookmark( true, true );
				...
				// next block.(#3887)
				if ( this._.lastNode &&
						this._.lastNode.type == OMEDITOR.NODE_TEXT &&
						!OMEDITOR.tools.trim( this._.lastNode.getText() ) &&
						this._.lastNode.getParent().isBlockBoundary() )
				{
					var testRange = new OMEDITOR.dom.range( range.document );
					testRange.moveToPosition( this._.lastNode, OMEDITOR.POSITION_AFTER_END );
					if ( testRange.checkEndOfBlock() )
					{
						var path = new OMEDITOR.dom.elementPath( testRange.endContainer );
			...
			while ( currentNode )
			{
				...
				var includeNode = ( currentNode.type != OMEDITOR.NODE_ELEMENT ),
				...
				if ( !includeNode )
				{
							...
							!parentPre && { br : 1 } ) )
					{
						...
						if ( range )
						{
							range.setEndAt( currentNode, OMEDITOR.POSITION_BEFORE_START );
					...
					else
					{
						...
						if ( currentNode.getFirst() )
						{
							...
							if ( !range )
							{
								range = new OMEDITOR.dom.range( this.range.document );
								range.setStartAt( currentNode, OMEDITOR.POSITION_BEFORE_START );
							}
				...
				else if ( currentNode.type == OMEDITOR.NODE_TEXT )
				{
				...
				if ( range && !closeRange )
				{
					while ( !currentNode.getNext( skipGuard ) && !isLast )
					{
								...
								&& !parentPre && { br : 1 } ) )
						{
							...
							range.setEndAt( parentNode, OMEDITOR.POSITION_BEFORE_END );
							break;
						}
				...
				if ( includeNode )
					range.setEndAt( currentNode, OMEDITOR.POSITION_AFTER_END );
			...
			if ( !block )
			{
				...
				var startPath = new OMEDITOR.dom.elementPath( range.startContainer );
			...
			if ( removePreviousBr )
			{
				...
				if ( previousSibling && previousSibling.type == OMEDITOR.NODE_ELEMENT )
				{
			...
			if ( removeLastBr )
			{
				...
				if ( lastChild && lastChild.type == OMEDITOR.NODE_ELEMENT && lastChild.getName() == 'br' )
				{
	...
	OMEDITOR.dom.range.prototype.createIterator = function()
	{
