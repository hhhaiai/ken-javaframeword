 ...
 * states are {@link OMEDITOR.TRISTATE_ON} and {@link OMEDITOR.TRISTATE_OFF}.
 * @param {OMEDITOR.style} style The style to be watched.
 * @param {Function} callback The function to be called.
 * @example
 * // Create a style object for the &lt;b&gt; element.
 * var style = new OMEDITOR.style( { element : 'b' } );
 * var editor = OMEDITOR.instances.editor1;
 * editor.attachStyleStateChange( style, function( state )
 *     {
 *         if ( state == OMEDITOR.TRISTATE_ON )
 *             alert( 'The current state for the B element is ON' );
 *         else
 *             alert( 'The current state for the B element is OFF' );
 *     });
 */
OMEDITOR.editor.prototype.attachStyleStateChange = function( style, callback )
{
		...
		this.on( 'selectionChange', function( ev )
			{
				...
				for ( var i = 0 ; i < styleStateChangeCallbacks.length ; i++ )
				{
					...
					// callback.
					var currentState = callback.style.checkActive( ev.data.path ) ? OMEDITOR.TRISTATE_ON : OMEDITOR.TRISTATE_OFF;
...
(function()
{
	...
	var notBookmark = OMEDITOR.dom.walker.bookmark( 0, 1 ),
		nonWhitespaces = OMEDITOR.dom.walker.whitespaces( 1 );

	OMEDITOR.style = function( styleDefinition, variablesValues )
	{
		if ( variablesValues )
		{
			styleDefinition = OMEDITOR.tools.clone( styleDefinition );
		...
		this.type =
			blockElements[ element ] ?
				OMEDITOR.STYLE_BLOCK
			: objectElements[ element ] ?
				OMEDITOR.STYLE_OBJECT
			:
				OMEDITOR.STYLE_INLINE;

		// If the 'element' property is an object with a set of possible element, it will be applied like an object style: only to existing elements
		if ( typeof this.element == 'object' )
			this.type = OMEDITOR.STYLE_OBJECT;
	...
	OMEDITOR.style.prototype =
	{
		...
		applyToRange : function( range )
		{
			return ( this.applyToRange =
						this.type == OMEDITOR.STYLE_INLINE ?
							applyInlineStyle
						: this.type == OMEDITOR.STYLE_BLOCK ?
							applyBlockStyle
						: this.type == OMEDITOR.STYLE_OBJECT ?
							applyObjectStyle
		...
		checkActive : function( elementPath )
		{
			switch ( this.type )
			{
				case OMEDITOR.STYLE_BLOCK :
					return this.checkElementRemovable( elementPath.block || elementPath.blockLimit, true );

				case OMEDITOR.STYLE_OBJECT :
				case OMEDITOR.STYLE_INLINE :
					...
					for ( var i = 0, element ; i < elements.length ; i++ )
					{
						...
						if ( this.type == OMEDITOR.STYLE_INLINE
							  && ( element == elementPath.block || element == elementPath.blockLimit ) )
							continue;

						if( this.type == OMEDITOR.STYLE_OBJECT )
						{
		...
		buildPreview : function( label )
		{
			...
			var cssStyle = OMEDITOR.style.getStyleText( styleDefinition );
			if ( cssStyle )
	...
	OMEDITOR.style.getStyleText = function( styleDefinition )
	{
	...
	function applyInlineStyle( range )
	{
		...
		if ( range.collapsed )
		{
			...
			range.moveToPosition( collapsedElement, OMEDITOR.POSITION_BEFORE_END );
		...
		var dtd = OMEDITOR.dtd[ elementName ] || ( isUnknownElement = true, OMEDITOR.dtd.span );

		// Expand the range.
		range.enlarge( OMEDITOR.ENLARGE_ELEMENT, 1 );
		range.trim();
		...
		if ( currentNode.getPosition( lastNode ) == OMEDITOR.POSITION_FOLLOWING )
			currentNode = 0;
		...
		while ( currentNode )
		{
			...
			else
			{
				...
				var nodeName = nodeType == OMEDITOR.NODE_ELEMENT ? currentNode.getName() : null;
				...
				if ( !nodeName || ( dtd[ nodeName ]
					...
					&& ( currentNode.getPosition( lastNode ) | OMEDITOR.POSITION_PRECEDING | OMEDITOR.POSITION_IDENTICAL | OMEDITOR.POSITION_IS_CONTAINED ) == ( OMEDITOR.POSITION_PRECEDING + OMEDITOR.POSITION_IDENTICAL + OMEDITOR.POSITION_IS_CONTAINED )
					&& ( !def.childRule || def.childRule( currentNode ) ) ) )
				{
					...
					if ( currentParent
						&& ( ( currentParent.getDtd() || OMEDITOR.dtd.span )[ elementName ] || isUnknownElement )
						&& ( !def.parentRule || def.parentRule( currentParent ) ) )
					{
						...
						if ( !styleRange && ( !nodeName || !OMEDITOR.dtd.$removeEmpty[ nodeName ] || ( currentNode.getPosition( lastNode ) | OMEDITOR.POSITION_PRECEDING | OMEDITOR.POSITION_IDENTICAL | OMEDITOR.POSITION_IS_CONTAINED ) == ( OMEDITOR.POSITION_PRECEDING + OMEDITOR.POSITION_IDENTICAL + OMEDITOR.POSITION_IS_CONTAINED ) ) )
						{
							styleRange = new OMEDITOR.dom.range( document );
							styleRange.setStartBefore( currentNode );
						}

						// Non element nodes, readonly elements, or empty
						// elements can be added completely to the range.
						if ( nodeType == OMEDITOR.NODE_TEXT || nodeIsReadonly || ( nodeType == OMEDITOR.NODE_ELEMENT && !currentNode.getChildCount() ) )
						{
							...
							while ( ( applyStyle = !includedNode.getNext( notBookmark ) )
								...
								&& ( parentNode.getPosition( firstNode ) | OMEDITOR.POSITION_FOLLOWING | OMEDITOR.POSITION_IDENTICAL | OMEDITOR.POSITION_IS_CONTAINED ) == ( OMEDITOR.POSITION_FOLLOWING + OMEDITOR.POSITION_IDENTICAL + OMEDITOR.POSITION_IS_CONTAINED )
			...
			if ( applyStyle && styleRange && !styleRange.collapsed )
			{
				...
				{
					styleNode = new OMEDITOR.dom.element( 'span' );
					styleRange.extractContents().appendTo( styleNode );
					styleRange.insertNode( styleNode );
					removeFromInsideElement( this, styleNode );
	...
	function removeInlineStyle( range )
	{
		...
		if ( range.collapsed )
		{
			...
			var startPath = new OMEDITOR.dom.elementPath( startNode.getParent() ),
					...
					&& ( element = startPath.elements[i] ) ; i++ )
			{
				...
				if ( this.checkElementRemovable( element ) )
				{
					...
					if ( range.collapsed && (
						 range.checkBoundaryOfElement( element, OMEDITOR.END ) ||
						 ( isStart = range.checkBoundaryOfElement( element, OMEDITOR.START ) ) ) )
					{
		...
		else
		{
			...
			{
				var startPath = new OMEDITOR.dom.elementPath( startNode.getParent() ),
					endPath = new OMEDITOR.dom.elementPath( endNode.getParent() ),
					breakStart = null,
					breakEnd = null;
			...
			while ( !currentNode.equals( endNode ) )
			{
				...
				if ( currentNode.type == OMEDITOR.NODE_ELEMENT && this.checkElementRemovable( currentNode ) )
				{
					 ...
					 */
					if ( nextNode.type == OMEDITOR.NODE_ELEMENT && nextNode.contains( startNode ) )
					{
						breakNodes();
	...
	function removeObjectStyle( range )
	{
		...
		var styles = OMEDITOR.style.getStyleText( def );
	...
	function applyBlockStyle( range )
	{
		...
		if ( this._.enterMode )
			iterator.enlargeBr = ( this._.enterMode != OMEDITOR.ENTER_BR );
	...
	function removeBlockStyle( range )
	{
		...
		while ( ( block = iterator.getNextParagraph() ) )
		{
			if ( this.checkElementRemovable( block ) )
			{
				...
				if ( block.is( 'pre' ) )
				{
					var newBlock = this._.enterMode == OMEDITOR.ENTER_BR ?
								null : range.document.createElement(
									this._.enterMode == OMEDITOR.ENTER_P ? 'p' : 'div' );
	...
	function fromPres( preHtmls, newBlock )
	{
		...
		if ( preHtmls.length > 1 )
			docFrag = new OMEDITOR.dom.documentFragment( newBlock.getDocument() );
			 ...
			 blockHtml = replace(  blockHtml, /^[ \t]+|[ \t]+$/g, function( match, offset, s )
					{
						if ( match.length == 1 )	// one space, preserve it
							...
							return OMEDITOR.tools.repeat( '&nbsp;', match.length - 1 ) + ' ';
						else				// end of block
							return ' ' + OMEDITOR.tools.repeat( '&nbsp;', match.length - 1 );
					} ) ;
	...
	function removeFromElement( style, element )
	{
		var def = style._.definition,
			attributes = OMEDITOR.tools.extend( {}, def.attributes, getOverrides( style )[ element.getName() ] ),
			styles = def.styles,
			// If the style is only about the element itself, we have to remove the element.
			removeEmpty = OMEDITOR.tools.isEmpty( attributes ) && OMEDITOR.tools.isEmpty( styles );
		...
		if ( removeEmpty )
		{
			!OMEDITOR.dtd.$block[ element.getName() ] || style._.enterMode == OMEDITOR.ENTER_BR && !element.hasAttributes() ?
				removeNoAttribsElement( element ) :
				element.renameNode( style._.enterMode == OMEDITOR.ENTER_P ? 'p' : 'div' );
		}
	}
	...
	function removeNoAttribsElement( element )
	{
		...
		{
			if ( OMEDITOR.dtd.$block[ element.getName() ] )
			{
				var previous = element.getPrevious( nonWhitespaces ),
						next = element.getNext( nonWhitespaces );

				if ( previous && ( previous.type == OMEDITOR.NODE_TEXT || !previous.isBlockBoundary( { br : 1 } ) ) )
					element.append( 'br', 1 );
				if ( next && ( next.type == OMEDITOR.NODE_TEXT || !next.isBlockBoundary( { br : 1 } ) ) )
					element.append( 'br' );
			...
			else
			{
				...
				if ( firstChild )
				{
					...
					firstChild.type == OMEDITOR.NODE_ELEMENT && firstChild.mergeSiblings();

					if ( lastChild && !firstChild.equals( lastChild )
							&& lastChild.type == OMEDITOR.NODE_ELEMENT )
	...
	function getElement( style, targetDocument, element )
	{
		...
		// Create the element.
		el = new OMEDITOR.dom.element( elementName, targetDocument );
	...
	function setupElement( el, style )
	{
		var def = style._.definition,
			attributes = def.attributes,
			styles = OMEDITOR.style.getStyleText( def );
	...
	function getAttributesForComparison( styleDefinition )
	{
		...
		var styleText = OMEDITOR.style.getStyleText( styleDefinition );
		if ( styleText )
		{
			if ( !attribs[ 'style' ] )
				length++;
	...
	function getOverrides( style )
	{
		...
		if ( definition )
		{
			...
			if ( !OMEDITOR.tools.isArray( definition ) )
	...
	{
		var temp = new OMEDITOR.dom.element( 'span' );
	...
	function normalizeCssText( unparsedCssText, nativeNormalize )
	{
		...
		styleText = styleText.replace( /(font-family:)(.*?)(?=;|$)/, function ( match, prop, val )
		{
			...
			for ( var i = 0; i < names.length; i++ )
				names[ i ] = OMEDITOR.tools.trim( names[ i ].replace( /["']/g, '' ) );
...
OMEDITOR.styleCommand = function( style )
{
	this.style = style;
};

OMEDITOR.styleCommand.prototype.exec = function( editor )
{
	editor.focus();
...
/**
 * Manages styles registration and loading. See also {@link OMEDITOR.config.stylesSet}.
 * @namespace
 ...
 * OMEDITOR.stylesSet.add( 'default',
 * [
 * 	// Block Styles
 ...
 * ]);
 */
OMEDITOR.stylesSet = new OMEDITOR.resourceManager( '', 'stylesSet' );

// Backward compatibility (#5025).
OMEDITOR.addStylesSet = OMEDITOR.tools.bind( OMEDITOR.stylesSet.add, OMEDITOR.stylesSet );
OMEDITOR.loadStylesSet = function( name, url, callback )
	{
		OMEDITOR.stylesSet.addExternal( name, url, '' );
		OMEDITOR.stylesSet.load( name, callback );
	};
 ...
 */
OMEDITOR.editor.prototype.getStylesSet = function( callback )
{
	if ( !this._.stylesDefinitions )
	{
		...
		var	partsStylesSet = configStyleSet.split( ':' ),
			...
			pluginPath = OMEDITOR.plugins.registered.styles.path;

		OMEDITOR.stylesSet.addExternal( styleSetName,
				externalPath ?
					partsStylesSet.slice( 1 ).join( ':' ) :
					pluginPath + 'styles/' + styleSetName + '.js', '' );

		OMEDITOR.stylesSet.load( styleSetName, function( stylesSet )
			{
 ...
 * @name OMEDITOR.style.includeReadonly
 * @type Boolean
 * @default false
 * @since 3.5
 */
  ...
  * @name OMEDITOR.config.disableReadonlyStyling
  * @type Boolean
  * @default false
  * @since 3.5
  */
 ...
 * @name OMEDITOR.config.stylesSet
 * @type String|Array
 * @default 'default'
 * @since 3.3
 * @example
