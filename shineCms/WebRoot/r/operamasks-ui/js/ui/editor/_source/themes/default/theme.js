...
OMEDITOR.themes.add( 'default', (function()
{
	var hiddenSkins = {};
	...
	function checkSharedSpace( editor, spaceName )
	{
		...
		element = element && OMEDITOR.document.getById( element );
		...
		if ( element )
		{
			...
			var html =
				...
				'<span class="' + OMEDITOR.env.cssClass + '">' +
				'<span class="cke_wrapper cke_' + editor.lang.dir + '">' +
				'<span class="cke_editor">' +
				'<div class="cke_' + spaceName + '">' +
				'</div></span></span></span></span></span>';

			var mainContainer = element.append( OMEDITOR.dom.element.createFromHtml( html, element.getDocument() ) );
			...
			editor.on( 'focus', function()
				{
					...
					{
						if ( sibling.type == OMEDITOR.NODE_ELEMENT
	...
	return /** @lends OMEDITOR.theme */ {
		build : function( editor, themePath )
		{
			...
			if ( !element || elementMode == OMEDITOR.ELEMENT_MODE_NONE )
				return;

			if ( elementMode == OMEDITOR.ELEMENT_MODE_REPLACE )
				element.hide();
			...
			var container = OMEDITOR.dom.element.createFromHtml( [
				'<span' +
					' id="cke_', name, '"' +
					' class="', editor.skinClass, ' ', editor.id, ' cke_editor_', name, '"' +
					' dir="', editor.lang.dir, '"' +
					' title="', ( OMEDITOR.env.gecko ? ' ' : '' ), '"' +
					' lang="', editor.langCode, '"' +
						( OMEDITOR.env.webkit? ' tabindex="' + tabIndex + '"' : '' ) +
					' role="application"' +
					' aria-labelledby="cke_', name, '_arialbl"' +
					( style ? ' style="' + style + '"' : '' ) +
					'>' +
					'<span id="cke_', name, '_arialbl" class="cke_voice_label">' + editor.lang.editor + '</span>' +
					'<span class="' , OMEDITOR.env.cssClass, '" role="presentation">' +
			 ...
			 * @name OMEDITOR.editor.prototype.container
		...
		{
			var baseIdNumber = OMEDITOR.tools.getNextNumber();

			var element = OMEDITOR.dom.element.createFromHtml( [
					'<div class="', editor.id, '_dialog cke_editor_', editor.name.replace('.', '\\.'), '_dialog cke_skin_', editor.skinName,
						...
						'>' +
						'<table class="cke_dialog', ' ' + OMEDITOR.env.cssClass,
						...
						//Hide the container when loading skins, later restored by skin css.
						( OMEDITOR.env.ie ? '' : '<style>.cke_dialog{visibility:hidden;}</style>' ),
		...
		destroy : function( editor )
		{
			...
			if ( element )
			{
				...
				editor.elementMode == OMEDITOR.ELEMENT_MODE_REPLACE && element.show();
 ...
 * @returns {OMEDITOR.dom.element} The element that represents the space.
 * @example
 * // Hide the bottom space in the UI.
 * var bottom = editor.getThemeSpace( 'bottom' );
 * bottom.setStyle( 'display', 'none' );
 */
OMEDITOR.editor.prototype.getThemeSpace = function( spaceName )
{
	var spacePrefix = 'cke_' + spaceName;
	var space = this._[ spacePrefix ] ||
		( this._[ spacePrefix ] = OMEDITOR.document.getById( spacePrefix + '_' + this.name ) );
	return space;
};
 ...
 */
OMEDITOR.editor.prototype.resize = function( width, height, isContentHeight, resizeInner )
{
	var container = this.container,
		contents = OMEDITOR.document.getById( 'cke_contents_' + this.name ),
	...
	OMEDITOR.env.webkit && outer.setStyle( 'display', 'none' );
 ...
 * @returns {OMEDITOR.dom.element} The resizable element.
 * @example
 */
OMEDITOR.editor.prototype.getResizable = function( forContents )
{
	return forContents ? OMEDITOR.document.getById( 'cke_contents_' + this.name ) : this.container;
};
 ...
 * display.
 * @name OMEDITOR.config.sharedSpaces
 * @type Object
 * @default undefined
 * @example
 ...
 * the {@link OMEDITOR.editor.prototype.resize} method.
 * @name OMEDITOR.editor#resize
 * @event
 */
