...
OMEDITOR.dialog.add( 'link', function( editor )
{
	var plugin = OMEDITOR.plugins.link;
	...
	var parseLink = function( editor, element )
	{
		...
		if ( element )
		{
			...
			OMEDITOR.plugins.link.synAnchorSelector ?
		...
		{
			var anchorList = new OMEDITOR.dom.nodeList( editor.document.$.anchors );
		...
		if ( OMEDITOR.plugins.link.fakeAnchor )
		{
			...
			{
				if ( ( item = OMEDITOR.plugins.link.tryRestoreFakeAnchor( editor, imgs.getItem( i ) ) ) )
								...
								[
									...
									},
									{
										...
										validate : function()
										{
											...
											var func = OMEDITOR.dialog.validate.notEmpty( linkLang.noUrl );
					...
					},
					{
						...
						[
							...
							},
							{
								...
								html : '<div role="label" tabIndex="-1">' + OMEDITOR.tools.htmlEncode( linkLang.noAnchors ) + '</div>',
						...
						[
							{
								...
								validate : function()
								{
									...
									var func = OMEDITOR.dialog.validate.notEmpty( linkLang.noEmail );
									return func.apply( this );
								},
								setup : function( data )
								{
		...
		onOk : function()
		{
			...
			switch ( data.type || 'url' )
			{
				case 'url':
					var protocol = ( data.url && data.url.protocol != undefined ) ? data.url.protocol : 'http://',
						url = ( data.url && OMEDITOR.tools.trim( data.url.url ) ) || '';
			...
			if ( !this._.selectedElement )
			{
				...
				if ( ranges.length == 1 && ranges[0].collapsed )
				{
					...
					var text = new OMEDITOR.dom.text( data.type == 'email' ?
				...
				style.type = OMEDITOR.STYLE_INLINE;		// need to override... dunno why.
			...
			else
			{
				...
				if ( data.adv && data.adv.advName && OMEDITOR.plugins.link.synAnchorSelector )
 ...
 * @name OMEDITOR.config.emailProtection
 * @since 3.1
 * @type String
 * @default '' (empty string = disabled)
 * @example
 * // href="mailto:tester@omeditor.com?subject=subject&body=body"
 * config.emailProtection = '';
 * @example
 * // href="<a href=\"javascript:void(location.href=\'mailto:\'+String.fromCharCode(116,101,115,116,101,114,64,99,107,101,100,105,116,111,114,46,99,111,109)+\'?subject=subject&body=body\')\">e-mail</a>"
 * config.emailProtection = 'encode';
 * @example
 * // href="javascript:mt('tester','omeditor.com','subject','body')"
