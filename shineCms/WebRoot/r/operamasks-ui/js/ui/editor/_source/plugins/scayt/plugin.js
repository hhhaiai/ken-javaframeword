...
(function()
{
	...
	var onEngineLoad = function()
	{
		...
		var createInstance = function()	// Create new instance every time Document is created.
		{
			...
			oParams.assocApp  = 'OMEDITOR.' + OMEDITOR.version + '@' + OMEDITOR.revision;
			...
			{
				( new OMEDITOR.dom.element( node, scayt_control.document ) ).mergeSiblings();
			} );
		...
		editor.on( 'contentDomUnload', function()
			{
				// Remove scripts.
				var scripts = OMEDITOR.document.getElementsByTag( 'script' ),
		...
		var undoImagePrototype = OMEDITOR.plugins.undo.Image.prototype;
		undoImagePrototype.equals = OMEDITOR.tools.override( undoImagePrototype.equals, function( org )
		{
...
OMEDITOR.plugins.scayt =
	{
		...
		loadEngine : function( editor )
		{
			...
			if ( OMEDITOR.env.gecko && OMEDITOR.env.version < 10900 || OMEDITOR.env.opera || OMEDITOR.env.air )
			...
			if ( this.engineLoaded === true )
				...
				return OMEDITOR.on( 'scaytReady', function(){ onEngineLoad.apply( editor ); } );	// Use function(){} to avoid rejection as duplicate.

			OMEDITOR.on( 'scaytReady', onEngineLoad, editor );
			OMEDITOR.on( 'scaytReady', function()
				{
			...
			if( window.scayt == undefined )
			{
				OMEDITOR._djScaytConfig =
				{
					...
					[
						function()
						{
							OMEDITOR.fireOnce( 'scaytReady' );
						}
					],
					isDebug: false
				};
				// Append javascript code.
				OMEDITOR.document.getHead().append(
					OMEDITOR.document.createElement( 'script',
						{
							attributes :
								{
	...
	var plugin = OMEDITOR.plugins.scayt;
	...
	var commandDefinition =
	{
		...
		exec: function( editor )
		{
			if ( plugin.isScaytReady( editor ) )
			{
				...
				this.setState( isEnabled ? OMEDITOR.TRISTATE_OFF : OMEDITOR.TRISTATE_ON );
	...
	OMEDITOR.plugins.add( 'scayt',
	{
		...
		init : function( editor )
		{
			...
			// Add Options dialog.
			OMEDITOR.dialog.add( commandName, OMEDITOR.getUrl( this.path + 'dialogs/options.js' ) );
			...
			editor.addMenuItems( uiMenuItems );
				...
				editor.ui.add( 'Scayt', OMEDITOR.UI_MENUBUTTON,
					{
						label : lang.title,
						title : OMEDITOR.env.opera ? lang.opera_title : lang.title,
						...
						onMenu : function()
						{
							...
							return {
								scaytToggle  : OMEDITOR.TRISTATE_OFF,
								scaytOptions : isEnabled && uiTabs[0] ? OMEDITOR.TRISTATE_OFF : OMEDITOR.TRISTATE_DISABLED,
								scaytLangs   : isEnabled && uiTabs[1] ? OMEDITOR.TRISTATE_OFF : OMEDITOR.TRISTATE_DISABLED,
								scaytDict    : isEnabled && uiTabs[2] ? OMEDITOR.TRISTATE_OFF : OMEDITOR.TRISTATE_DISABLED,
								scaytAbout   : isEnabled && uiTabs[3] ? OMEDITOR.TRISTATE_OFF : OMEDITOR.TRISTATE_DISABLED
							};
						}
					});
				...
				editor.contextMenu.addListener( function( element, selection )
					{
						...
						for ( var i = 0, l = items_suggestion.length; i < l; i += 1 )
						{
							...
							if ( i < maxSuggestions )
							{
								...
								_r[ commandName ] = OMEDITOR.TRISTATE_OFF;
								mainSuggestions[ commandName ] = OMEDITOR.TRISTATE_OFF;
							}
							else if ( moreSuggestionsUnable == 'on' )
							{
								addButtonCommand( editor, 'button_' + commandName, items_suggestion[i],
									commandName, exec, 'scayt_moresuggest', i + 1 );
								moreSuggestions[ commandName ] = OMEDITOR.TRISTATE_OFF;
						...
						if ( moreSuggestionsUnableAdded )
						{
							...
							mainSuggestions[ 'scayt_moresuggest' ] = OMEDITOR.TRISTATE_OFF;
						}
						...
						if ( in_array( 'all', contextCommands )  || in_array( 'ignore', contextCommands)  )
						{
							...
							mainSuggestions[ 'scayt_ignore' ] = OMEDITOR.TRISTATE_OFF;
						}
						...
						if ( in_array( 'all', contextCommands )  || in_array( 'ignoreall', contextCommands ) )
						{
							...
							mainSuggestions['scayt_ignore_all'] = OMEDITOR.TRISTATE_OFF;
						}
						...
						if ( in_array( 'all', contextCommands )  || in_array( 'add', contextCommands ) )
						{
							...
							mainSuggestions['scayt_add_word'] = OMEDITOR.TRISTATE_OFF;
						}
			...
			var showInitialState = function()
				{
					...
					if ( !OMEDITOR.env.opera && !OMEDITOR.env.air )
						command.setState( plugin.isScaytEnabled( editor ) ? OMEDITOR.TRISTATE_ON : OMEDITOR.TRISTATE_OFF );
					else
						command.setState( OMEDITOR.TRISTATE_DISABLED );
				};
 ...
 * @name OMEDITOR.config.scayt_autoStartup
 * @type Boolean
 * @default false
 * @example
 ...
 * </ul>
 * @name OMEDITOR.config.scayt_maxSuggestions
 * @type Number
 * @default 5
 * @example
 ...
 * @name OMEDITOR.config.scayt_customerid
 * @type String
 * @default ''
 * @example
 ...
 * @name OMEDITOR.config.scayt_moreSuggestions
 * @type String
 * @default 'on'
 * @example
 ...
 * </ul>
 * @name OMEDITOR.config.scayt_contextCommands
 * @type String
 * @default 'all'
 * @example
 ...
 * @name OMEDITOR.config.scayt_sLang
 * @type String
 * @default 'en_US'
 * @example
 ...
 * @name OMEDITOR.config.scayt_uiTabs
 * @type String
 * @default '1,1,1'
 * @example
 ...
 * Further details at http://wiki.spellchecker.net/doku.php?id=3rd:wysiwyg:fomeditor:wscckf3l .
 * @name OMEDITOR.config.scayt_srcUrl
 * @type String
 * @default ''
 * @example
 ...
 * @name OMEDITOR.config.scayt_customDictionaryIds
 * @type String
 * @default ''
 * @example
 ...
 * @name OMEDITOR.config.scayt_userDictionaryName
 * @type String
 * @default ''
 * @example
 ...
 * </ul>
 *
 * @name OMEDITOR.config.scayt_contextMenuItemsOrder
 * @type String
