...
(function()
{
	...
	OMEDITOR.plugins.add( 'pastefromword',
	{
					...
					var isLazyLoad = this.loadFilterRules( function()
						{
							...
							if ( isLazyLoad )
							 ...
							 {
								data[ 'html' ] = OMEDITOR.cleanWord( mswordHtml, editor );
							}
						});
		...
		loadFilterRules : function( callback )
		{
			...
			var isLoaded = OMEDITOR.cleanWord;

			if ( isLoaded )
				callback();
			else
			{
				var filterFilePath = OMEDITOR.getUrl(
						OMEDITOR.config.pasteFromWordCleanupFile
						|| ( this.path + 'filter/default.js' ) );

				// Load with busy indicator.
				OMEDITOR.scriptLoader.load( filterFilePath, callback, null, true );
			}
 ...
 * MS Word.
 * @name OMEDITOR.config.pasteFromWordPromptCleanup
 * @since 3.1
 * @type Boolean
 * @default undefined
 * @example
 ...
 * in the page.
 * @name OMEDITOR.config.pasteFromWordCleanupFile
 * @since 3.1
 * @type String
 * @default 'default'
 * @example
 * // Load from 'pastefromword' plugin 'filter' sub folder (custom.js file).
 * OMEDITOR.config.pasteFromWordCleanupFile = 'custom';
 */
