...
OMEDITOR.dialog.add( 'uicolor', function( editor )
{
	var dialog, picker, pickerContents,
		// Actual UI color value.
		uiColor = editor.getUiColor(),
		pickerId = 'cke_uicolor_picker' + OMEDITOR.tools.getNextNumber();
	...
	pickerContents =
	{
		...
		onLoad : function( event )
		{
			...
			var inputs = new OMEDITOR.dom.nodeList( picker.getElementsByTagName( 'input' ) );
	...
	return {
									...
									[
										{
											...
											onChange : function()
											{
												...
												if ( color )
												{
													...
													OMEDITOR.document.getById( 'predefinedPreview' ).setStyle( 'background', color );
												}
												else
													OMEDITOR.document.getById( 'predefinedPreview' ).setStyle( 'background', '' );
											},
											onShow : function()
											{
		...
		buttons : [ OMEDITOR.dialog.okButton ]
	};
} );
