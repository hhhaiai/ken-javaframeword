...
(function()
{
	OMEDITOR.on( 'dialogDefinition', function( ev )
	{
	...
	var dtd =  OMEDITOR.dtd,
		blockLikeTags = OMEDITOR.tools.extend( { table:1 }, dtd.$block, dtd.$listItem, dtd.$tableContent, dtd.$list );
	...
	OMEDITOR.BBCodeParser = function()
	{
		this._ =
		{
	...
	OMEDITOR.BBCodeParser.prototype =
	{
		parse : function( bbcode )
		{
			...
			while ( ( parts = this._.bbcPartsRegex.exec( bbcode ) ) )
			{
				...
				if ( parts[ 1 ] )
				{
					...
					this.onTagOpen( tagName, attribs, OMEDITOR.dtd.$empty[ tagName ] );
				}
				// Closing tag
				else if ( parts[ 3 ] )
	...
	/**
	 * Creates a {@link OMEDITOR.htmlParser.fragment} from an HTML string.
	 * @param {String} source The HTML to be parsed, filling the fragment.
	 * @param {Number} [fixForBody=false] Wrap body with specified element if needed.
	 * @returns OMEDITOR.htmlParser.fragment The fragment created.
	 * @example
	 * var fragment = OMEDITOR.htmlParser.fragment.fromHtml( '<b>Sample</b> Text' );
	 * alert( fragment.children[0].name );  "b"
	 * alert( fragment.children[1].value );  " Text"
	 */
	OMEDITOR.htmlParser.fragment.fromBBCode = function( source )
	{
		var parser = new OMEDITOR.BBCodeParser(),
			fragment = new OMEDITOR.htmlParser.fragment(),
			pendingInline = [],
			pendingBrs = 0,
			...
			if ( pendingInline.length > 0 )
			{
				for ( var i = 0 ; i < pendingInline.length ; i++ )
				{
					var pendingElement = pendingInline[ i ],
						...
						pendingDtd = OMEDITOR.dtd[ pendingName ],
						currentDtd = currentNode.name && OMEDITOR.dtd[ currentNode.name ];

					if ( ( !currentDtd || currentDtd[ pendingName ] ) && ( !newTagName || !pendingDtd || pendingDtd[ newTagName ] || !OMEDITOR.dtd[ newTagName ] ) )
					{
		...
		function checkPendingBrs( tagName, closing )
		{
			var len = currentNode.children.length,
				...
				lineBreakPrevious = previous && previous.type == OMEDITOR.NODE_ELEMENT && BBCodeWriter.getRule( tagnameMap[ previous.name ], 'breakAfterClose' ),
			...
			while ( pendingBrs && pendingBrs-- )
				currentNode.children.push( previous = new OMEDITOR.htmlParser.element( 'br' ) );
		}
		...
		{
			var element = new OMEDITOR.htmlParser.element( tagName, attributes );

			// This is a tag to be removed if empty, so do not add it immediately.
			if ( OMEDITOR.dtd.$removeEmpty[ tagName ] )
			{
			...
			var currentDtd = currentName
				&& ( OMEDITOR.dtd[ currentName ]
					|| ( currentNode._.isBlockLike ? OMEDITOR.dtd.div : OMEDITOR.dtd.span ) );
			...
			if ( currentDtd && !currentDtd[ tagName ] )
			{
				...
				else if ( tagName in OMEDITOR.dtd.$listItem )
				{
					parser.onTagOpen( 'ul', {} );
		...
		{
			var currentDtd = OMEDITOR.dtd[ currentNode.name ];
			if ( !currentDtd || currentDtd[ '#' ] )
			{
				...
				text.replace(/([\r\n])|[^\r\n]*/g, function( piece, lineBreak )
				{
					...
					else if ( piece.length )
					{
						...
						{
							addElement( new OMEDITOR.htmlParser.text( piece.substring( lastIndex, index ) ), currentNode );
							addElement( new OMEDITOR.htmlParser.element( 'smiley', { 'desc': smileyReverseMap[ match ] } ), currentNode );
							lastIndex = index + match.length;
						});

						if ( lastIndex != piece.length )
							addElement( new OMEDITOR.htmlParser.text( piece.substring( lastIndex, piece.length ) ), currentNode );
					}
				});
			}
		};
		...
		// Parse it.
		parser.parse( OMEDITOR.tools.htmlEncode( source ) );
	...
	OMEDITOR.htmlParser.BBCodeWriter = OMEDITOR.tools.createClass(
	{
		$ : function()
		{
			this._ =
		...
		proto :
		{
			...
			setRules : function( tagName, rules )
			{
				...
				if ( currentRules )
					OMEDITOR.tools.extend( currentRules, rules, true );
				else
	...
	var BBCodeWriter = new OMEDITOR.htmlParser.BBCodeWriter();

	OMEDITOR.plugins.add( 'bbcode',
	  {
		  ...
		  init : function( editor )
		  {
			  ...
			  {
				  var fragment = OMEDITOR.htmlParser.fragment.fromBBCode( code ),
						  writer = new OMEDITOR.htmlParser.basicWriter();

				  fragment.writeHtml( writer, dataFilter );
				  return writer.getHtml( true );
			  }

			  var dataFilter = new OMEDITOR.htmlParser.filter();
			  dataFilter.addRules(
			  {
				  elements :
				  {
					  ...
					  {
						  var quoted = new OMEDITOR.htmlParser.element( 'div' );
						  quoted.children = element.children;
						  element.children = [ quoted ];
						  var citeText = element.attributes.cite;
						  if ( citeText )
						  {
							  var cite = new OMEDITOR.htmlParser.element( 'cite' );
							  cite.add( new OMEDITOR.htmlParser.text( citeText.replace( /^"|"$/g, '' ) ) );
					  ...
					  'smiley' : function( element )
					  {
							...
							var description = element.attributes.desc,
								image = config.smiley_images[ OMEDITOR.tools.indexOf( config.smiley_descriptions, description ) ],
								src = OMEDITOR.tools.htmlEncode( config.smiley_path + image );
					...
					$ : function( element )
					{
						...
						else if ( tagName == 'a' )
						{
							if ( ( value = attributes.href ) )
							{
								if ( value.indexOf( 'mailto:' ) !== -1 )
								{
									...
									element.children = [ new OMEDITOR.htmlParser.text( value.replace( 'mailto:', '' ) ) ];
									value = '';
								}
								else
								{
									...
									if ( singleton
											&& singleton.type == OMEDITOR.NODE_TEXT
						...
						else if ( tagName == 'img' )
						{
							...
							if ( src && src.indexOf( editor.config.smiley_path ) != -1 )
								return new OMEDITOR.htmlParser.text( smileyMap[ attributes.alt ] );
							else
								element.children = [ new OMEDITOR.htmlParser.text( src ) ];
						}
			  ...
			  editor.on( 'beforeSetMode', function( evt )
			  {
				  ...
				  wysiwyg.loadData = OMEDITOR.tools.override( wysiwyg.loadData, function( org )
				  {
