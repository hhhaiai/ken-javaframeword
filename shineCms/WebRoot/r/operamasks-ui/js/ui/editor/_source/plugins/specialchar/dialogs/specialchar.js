...
OMEDITOR.dialog.add( 'specialchar', function( editor )
{
	/**
	...
	var onChoice = function( evt )
	{
		...
		if ( evt.data )
			...
			target = new OMEDITOR.dom.element( evt );
	...
	var onClick = OMEDITOR.tools.addFunction( onChoice );
	...
	var onFocus = function( evt, target )
	{
		...
		if ( target.getName() == 'a' && ( value = target.getChild( 0 ).getHtml() ) )
		{
			...
			htmlPreview.setHtml( OMEDITOR.tools.htmlEncode( value ) );
	...
	var onKeydown = OMEDITOR.tools.addFunction( function( ev )
	{
	...
	return {
		...
		onLoad :  function()
		{
			...
			var charsTableLabel =  OMEDITOR.tools.getNextId() + '_specialchar_table_label';
			...
			while ( i < size )
			{
				...
				for ( var j = 0 ; j < columns ; j++, i++ )
				{
					if ( ( character = chars[ i ] ) )
					{
						...
						var charLabelId =  'cke_specialchar_label_' + i + '_' + OMEDITOR.tools.getNextNumber();
						...
						html.push(
							...
							' style="cursor: inherit; display: block; height: 1.25em; margin-top: 0.25em; text-align: center;" title="', OMEDITOR.tools.htmlEncode( charDesc ), '"' +
							' onkeydown="OMEDITOR.tools.callFunction( ' + onKeydown + ', event, this )"' +
							' onclick="OMEDITOR.tools.callFunction(' + onClick + ', this); return false;"' +
							' tabindex="-1">' +
