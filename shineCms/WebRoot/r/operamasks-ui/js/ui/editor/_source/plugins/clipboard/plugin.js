...
(function()
{
	...
	var execIECommand = function( editor, command )
	{
		...
		( OMEDITOR.env.version > 7 ? doc.$ : doc.$.selection.createRange() ) [ 'execCommand' ]( command );
	...
	var onKey = function( event )
	{
		...
		{
			// Paste
			case OMEDITOR.CTRL + 86 :		// CTRL+V
			case OMEDITOR.SHIFT + 45 :		// SHIFT+INS

				var body = this.document.getBody();

				// Simulate 'beforepaste' event for all none-IEs.
				if ( !OMEDITOR.env.ie && body.fire( 'beforepaste' ) )
					event.cancel();
			...
			// Cut
			case OMEDITOR.CTRL + 88 :		// CTRL+X
			case OMEDITOR.SHIFT + 46 :		// SHIFT+DEL
	...
	function getClipboardData( evt, mode, callback )
	{
		...
		var pastebin = new OMEDITOR.dom.element( mode == 'text' ? 'textarea' : OMEDITOR.env.webkit ? 'body' : 'div', doc );
		pastebin.setAttribute( 'id', 'cke_pastebin' );
		// Safari requires a filler node inside the div to have the content pasted into it. (#4882)
		OMEDITOR.env.webkit && pastebin.append( doc.createText( '\xa0' ) );
		...
		{
			range.setStartAt( pastebin, OMEDITOR.POSITION_AFTER_START );
			range.setEndAt( pastebin, OMEDITOR.POSITION_BEFORE_END );
			range.select( true );
		}
		...
		{
			mode == 'text' && OMEDITOR.env.gecko && editor.focusGrabber.focus();
			pastebin.remove();
	...
	function fixCut( editor )
	{
		...
		var control;
		if( ( sel.getType() == OMEDITOR.SELECTION_ELEMENT ) && ( control = sel.getSelectedElement() ) )
		{
	...
	function stateFromNamedCommand( command, editor )
	{
		...
		OMEDITOR.env.ie && ( depressBeforeEvent = 1 );

		var retval = OMEDITOR.TRISTATE_OFF;
		try { retval = editor.document.$.queryCommandEnabled( command ) ? OMEDITOR.TRISTATE_OFF : OMEDITOR.TRISTATE_DISABLED; }catch( er ){}
	...
	function setToolbarStates()
	{
		...
		this.getCommand( 'cut' ).setState( inReadOnly ? OMEDITOR.TRISTATE_DISABLED : stateFromNamedCommand( 'Cut', this ) );
		this.getCommand( 'copy' ).setState( stateFromNamedCommand( 'Copy', this ) );
		var pasteState = inReadOnly ? OMEDITOR.TRISTATE_DISABLED :
						OMEDITOR.env.webkit ? OMEDITOR.TRISTATE_OFF : stateFromNamedCommand( 'Paste', this );
		this.fire( 'pasteState', pasteState );
	}

	// Register the plugin.
	OMEDITOR.plugins.add( 'clipboard',
		{
			...
			init : function( editor )
			{
				...
				OMEDITOR.dialog.add( 'paste', OMEDITOR.getUrl( this.path + 'dialogs/paste.js' ) );
				...
				editor.on( 'contentDom', function()
				{
					...
					body.on( OMEDITOR.env.webkit ? 'paste' : 'beforepaste', function( evt )
						{
							...
							getClipboardData.call( editor, evt, eventData.mode, function ( data )
							{
								...
								if ( !( data = OMEDITOR.tools.trim( data.replace( /<span[^>]+data-cke-bookmark[^<]*?<\/span>/ig,'' ) ) ) )
									return;
					...
					editor.contextMenu.addListener( function( element, selection )
						{
							...
							return {
								...
								paste : !readOnly && ( OMEDITOR.env.webkit ? OMEDITOR.TRISTATE_OFF : stateFromNamedCommand( 'Paste', editor ) )
							};
						});
				}
			}
 ...
 * @name OMEDITOR.editor#pasteDialog
 * @event
 */
