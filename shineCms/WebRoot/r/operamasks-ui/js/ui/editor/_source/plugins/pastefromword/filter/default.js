...
(function()
{
	var fragmentPrototype = OMEDITOR.htmlParser.fragment.prototype,
		elementPrototype = OMEDITOR.htmlParser.element.prototype;
	 ...
	 * @param tagName
	 */
	OMEDITOR.dtd.parentOf = function( tagName )
	{
		var result = {};
		for ( var tag in this )
		{
	...
	function postProcessList( list )
	{
		var children = list.children,
			...
			stylesFilter = OMEDITOR.plugins.pastefromword.filters.stylesFilter;
	...
	var plugin = ( OMEDITOR.plugins.pastefromword =
	{
		utils :
		{
			...
			{
				var marker = new OMEDITOR.htmlParser.element( 'cke:listbullet' );
				marker.attributes = { 'cke:listsymbol' : bullet[ 0 ] };
				marker.add( new OMEDITOR.htmlParser.text( bulletText ) );
				return marker;
			},
			...
			resolveList : function( element )
			{
						...
						&& ( listMarker = listMarker[ 0 ] ) )
				{
						...
						attrs.style = plugin.filters.stylesFilter(
								[
									...
									[ ( /^margin(:?-left)?$/ ), null, function( margin )
									{
										...
										margin = OMEDITOR.tools.convertToPx( values[ 3 ] || values[ 1 ] || values [ 0 ] );
					...
					OMEDITOR.tools.extend( attrs, listMarker.attributes );
					return true;
				}
			...
			{
				var calculator = OMEDITOR.dom.element.createFromHtml(
								'<div style="position:absolute;left:-9999px;top:-9999px;"></div>',
								OMEDITOR.document );
				OMEDITOR.document.getBody().append( calculator );
			...
			listDtdParents : OMEDITOR.dtd.parentOf( 'ol' )
		},
		...
		filters :
		{
				...
				flattenList : function( element, level )
				{
					...
					for ( var i = 0; i < children.length; i++ )
					{
						...
						if ( child.name in OMEDITOR.dtd.$listItem )
						{
							...
							if ( last.name in OMEDITOR.dtd.$list )
							{
							...
							plugin.filters.stylesFilter(
								[
									[ 'tab-stops', null, function( val )
									{
										...
										margin && ( previousListItemMargin = OMEDITOR.tools.convertToPx( margin[ 0 ] ) );
									} ],
						...
						// Flatten sub list.
						else if ( child.name in OMEDITOR.dtd.$list )
						{
				...
				assembleList : function( element )
				{
					...
					for ( var i = 0; i < children.length; i++ )
					{
						...
						if ( 'cke:li' == child.name )
						{
							...
							if ( !list )
							{
								openedLists.push( list = new OMEDITOR.htmlParser.element( listType ) );
								list.add( listItem );
								children[ i ] = list;
							}
							else
					...
					return function( element )
						{
							var styleDef =
									variables ?
										new OMEDITOR.style( styleDefiniton, variables )._.definition
										: styleDefiniton;
							element.name = styleDef.element;
							OMEDITOR.tools.extend( element.attributes, OMEDITOR.tools.clone( styleDef.attributes ) );
							element.addStyle( OMEDITOR.style.getStyleText( styleDef ) );
						};
				},
				...
				styleMigrateFilter : function( styleDefinition, variableName )
				{
					...
					return function( value, element )
					{
						...
						var styleElement = new OMEDITOR.htmlParser.element( null ),
							variables = {};
		...
		getRules : function( editor )
		{
			var dtd = OMEDITOR.dtd,
				blockLike = OMEDITOR.tools.extend( {}, dtd.$block, dtd.$listItem, dtd.$tableContent ),
				...
				styleMigrateFilter = OMEDITOR.tools.bind( this.filters.styleMigrateFilter, this.filters ),
				...
				convertToPx = function( value )
					{
						value = OMEDITOR.tools.convertToPx( value );
			...
			return {
				...
				elements :
				{
					'^' : function( element )
					{
						...
						if ( OMEDITOR.env.gecko && ( applyStyleFilter = filters.applyStyleFilter ) )
									...
									.replace( /(.+?)\{(.+?)\}/g,
										function( rule, selectors, styleBlock )
										{
											...
											for ( var i = 0; i < length; i++ )
											{
												...
												OMEDITOR.tools.trim( selectors[ i ] )
															  .replace( /^(\w+)(\.[\w-]+)?$/g,
					...
					'p' : function( element )
					{
						...
						if ( /MsoListParagraph/.exec( element.attributes[ 'class' ] ) )
						{
							...
							{
								return node.type == OMEDITOR.NODE_TEXT && !containsNothingButSpaces( node.parent );
							});
						...
						if ( config.enterMode == OMEDITOR.ENTER_BR )
						{
							// We suffer from attribute/style lost in this situation.
							delete element.name;
							element.add( new OMEDITOR.htmlParser.element( 'br' ) );
						}
						else
							elementMigrateFilter( config[ 'format_' + ( config.enterMode == OMEDITOR.ENTER_P ? 'p' : 'div' ) ] )( element );
					},
					...
					'div' : function( element )
					{
						...
						{
							var attrs = element.attributes;
							singleChild.attributes = OMEDITOR.tools.extend( singleChild.attributes, attrs );
							attrs.style && singleChild.addStyle( attrs.style );

							var clearFloatDiv = new OMEDITOR.htmlParser.element( 'div' );
					...
					'font' : function( element )
					{
						...
						{
							OMEDITOR.tools.extend( parent.attributes,
									element.attributes );
					...
					'a' : function( element )
					{
						...
						if ( attrs && !attrs.href && attrs.name )
							delete element.name;
						else if ( OMEDITOR.env.webkit && attrs.href && attrs.href.match( /file:\/\/\/[\S]+#/i ) )
				...
				comment :
					!OMEDITOR.env.ie ?
						function( value, node )
						{
							...
							if ( OMEDITOR.env.gecko && imageInfo )
							{
								var img = OMEDITOR.htmlParser.fragment.fromHtml( imageInfo[ 0 ] ).children[ 0 ],
	...
	{
		this.dataFilter = new OMEDITOR.htmlParser.filter();
	};
	...
	pasteProcessor.prototype =
	{
		...
		{
			var fragment = OMEDITOR.htmlParser.fragment.fromHtml( data, false ),
	...
	OMEDITOR.cleanWord = function( data, editor )
	{
		...
		dataFilter.addRules( OMEDITOR.plugins.pastefromword.getRules( editor ) );
 ...
 * @name OMEDITOR.config.pasteFromWordRemoveFontStyles
 * @since 3.1
 * @type Boolean
 * @default true
 * @example
 ...
 * @name OMEDITOR.config.pasteFromWordNumberedHeadingToList
 * @since 3.1
 * @type Boolean
 * @default false
 * @example
 ...
 * {@link OMEDITOR.config.pasteFromWordRemoveFontStyles} setting instead.
 * @name OMEDITOR.config.pasteFromWordRemoveStyles
 * @since 3.1
 * @type Boolean
 * @default true
 * @example
