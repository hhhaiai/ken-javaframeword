...
OMEDITOR.plugins.add( 'menu',
{
	beforeInit : function( editor )
	{
		 ...
		 * @name OMEDITOR.editor.prototype.addMenuGroup
		 ...
		 * @name OMEDITOR.editor.prototype.addMenuItem
		 * @param {String} name The menu item name.
		 * @param {OMEDITOR.menu.definition} definition The menu item definition.
		 */
		editor.addMenuItem = function( name, definition )
			{
				if ( groupsOrder[ definition.group ] )
					menuItems[ name ] = new OMEDITOR.menuItem( this, name, definition );
			};

		/**
		 * Adds one or more items from the specified definition array to the editor context menu.
		 * @name OMEDITOR.editor.prototype.addMenuItems
		 * @param {Array} definitions List of definitions for each menu item as if {@link OMEDITOR.editor.addMenuItem} is called.
		 */
		 ...
		 * @name OMEDITOR.editor.prototype.getMenuItem
		 * @param {String} name The name of the desired menu item.
		 * @return {OMEDITOR.menu.definition}
		 */
		 ...
		 * @name OMEDITOR.editor.prototype.removeMenuItem
		 * @param {String} name The name of the desired menu item.
		 * @since 3.6.1
		 */
...
(function()
{
	OMEDITOR.menu = OMEDITOR.tools.createClass(
	{
		$ : function( editor, definition )
		{
			...
			var panelDefinition = OMEDITOR.tools.extend( {}, definition.panel,
			{
		...
		_ :
		{
			...
			showSubMenu : function( index )
			{
				...
				{
					menu = this._.subMenu = new OMEDITOR.menu( this.editor,
								   OMEDITOR.tools.extend( {}, this._.definition, { level : this._.level + 1 }, true ) );
					menu.parent = this;
					menu._.onClick = OMEDITOR.tools.bind( this._.onClick, this );
				}
		...
		proto :
		{
			...
			show : function( offsetParent, corner, offsetX, offsetY )
			{
				...
				if ( !panel )
				{
					panel = this._.panel = new OMEDITOR.ui.floatPanel( this.editor,
						OMEDITOR.document.getBody(),
						this._.panelDefinition,
						this._.level );

					panel.onEscape = OMEDITOR.tools.bind( function( keystroke )
					{
					...
					panel.onHide = OMEDITOR.tools.bind( function()
					{
					...
					keys[ OMEDITOR.SHIFT + 9 ]	= 'prev';	// SHIFT + TAB
					keys[ ( editor.lang.dir == 'rtl' ? 37 : 39 ) ]= OMEDITOR.env.ie ? 'mouseup' : 'click';  // ARROW-RIGHT/ARROW-LEFT(rtl)
					keys[ 32 ]	= OMEDITOR.env.ie ? 'mouseup' : 'click';					// SPACE
					OMEDITOR.env.ie && ( keys[ 13 ] = 'mouseup' );		// Manage ENTER, since onclick is blocked in IE (#8041).
					...
					this._.itemOverFn = OMEDITOR.tools.addFunction( function( index )
						{
							clearTimeout( this._.showSubTimeout );
							this._.showSubTimeout = OMEDITOR.tools.setTimeout( this._.showSubMenu, editor.config.menu_subMenuDelay || 400, this, [ index ] );
						},
						this );

					this._.itemOutFn = OMEDITOR.tools.addFunction( function( index )
						{
							clearTimeout( this._.showSubTimeout );
						},
						this );

					this._.itemClickFn = OMEDITOR.tools.addFunction( function( index )
						{
							var item = this.items[ index ];

							if ( item.state == OMEDITOR.TRISTATE_DISABLED )
							{
								this.hide();
								return;
							}
	...
	OMEDITOR.menuItem = OMEDITOR.tools.createClass(
	{
		...
		proto :
		{
			render : function( menu, index, output )
			{
				var id = menu.id + String( index ),
					state = ( typeof this.state == 'undefined' ) ? OMEDITOR.TRISTATE_OFF : this.state;

				var classes = ' cke_' + (
					state == OMEDITOR.TRISTATE_ON ? 'on' :
					state == OMEDITOR.TRISTATE_DISABLED ? 'disabled' :
					'off' );
			...
			output.push(
				...
				'<a id="', id, '"' +
					...
					( state == OMEDITOR.TRISTATE_DISABLED ? 'aria-disabled="true"' : '' ) +
					( state == OMEDITOR.TRISTATE_ON ? 'aria-pressed="true"' : '' ) );
				...
				output.push(
//					' onkeydown="return OMEDITOR.ui.button._.keydown(', index, ', event);"' +
					' onmouseover="OMEDITOR.tools.callFunction(', menu._.itemOverFn, ',', index, ');"' +
					' onmouseout="OMEDITOR.tools.callFunction(', menu._.itemOutFn, ',', index, ');" ' +
					( OMEDITOR.env.ie ? 'onclick="return false;" onmouseup' : 'onclick' ) +		// #188
						'="OMEDITOR.tools.callFunction(', menu._.itemClickFn, ',', index, '); return false;"' +
					'>' +
						'<span class="cke_icon_wrapper"><span class="cke_icon"' +
							( this.icon ? ' style="background-image:url(' + OMEDITOR.getUrl( this.icon ) + ');background-position:0 ' + offset + 'px;"'
							: '' ) +
							'></span></span>' +
