		...
		toolbarFocus :
		{
			...
			exec : function( editor )
			{
				if ( editor.toolbox )
				{
					...
					if ( OMEDITOR.env.ie || OMEDITOR.env.air )
		...
		init : function( editor )
		{
				...
				switch ( keystroke )
				{
					...
					case 38 :					// UP-ARROW
						...
						else
						{
							endFlag = 1;
							// Send a SHIFT + TAB.
							itemKeystroke( item, OMEDITOR.SHIFT + 9 );
							endFlag = 0;
						}
			...
			editor.on( 'themeSpace', function( event )
				{
					if ( event.data.space == editor.config.toolbarLocation )
					{
						...
						var labelId = OMEDITOR.tools.getNextId();
							...
							for ( var i = 0 ; i < items.length ; i++ )
							{
								...
								if ( item )
								{
									...
									if ( !toolbarObj )
									{
										...
										toolbarId = OMEDITOR.tools.getNextId();
						...
						{
							var collapserFn = OMEDITOR.tools.addFunction(
								function()
								{
							...
							editor.on( 'destroy', function () {
									OMEDITOR.tools.removeFunction( collapserFn );
								});

							var collapserId = OMEDITOR.tools.getNextId();
							...
							editor.addCommand( 'toolbarCollapse',
								{
									...
									exec : function( editor )
									{
										var collapser = OMEDITOR.document.getById( collapserId ),
							...
							output.push( '" onclick="OMEDITOR.tools.callFunction(' + collapserFn + ')">',
			...
			editor.on( 'destroy', function()
			{
				...
				for ( ; index < toolbars.length; index++ )
				{
					...
					{
						instance = items[ i ];
						if ( instance.clickFn ) OMEDITOR.tools.removeFunction( instance.clickFn );
						if ( instance.keyDownFn ) OMEDITOR.tools.removeFunction( instance.keyDownFn );
					}
				}
			});
			...
			editor.ui.add( '-', OMEDITOR.UI_SEPARATOR, {} );
			editor.ui.addHandler( OMEDITOR.UI_SEPARATOR,
			{
				create: function()
				{
					return {
...
OMEDITOR.UI_SEPARATOR = 'separator';
 ...
 * @type String
 * @default 'top'
 * @see OMEDITOR.config.theme
 * @example
 * config.toolbarLocation = 'bottom';
 */
OMEDITOR.config.toolbarLocation = 'top';
 ...
 * {@link OMEDITOR.config.toolbar} setting, so you instruct the editor which
 ...
 */
OMEDITOR.config.toolbarCanCollapse = true;

/**
 * Whether the toolbar must start expanded when the editor is loaded.
 * @name OMEDITOR.config.toolbarStartupExpanded
 * @type Boolean
 * @default true
 * @example
 ...
 * toolbar groups.
 * @name OMEDITOR.config.toolbarGroupCycling
 * @since 3.6
 * @type Boolean
 * @default true
 * @example
