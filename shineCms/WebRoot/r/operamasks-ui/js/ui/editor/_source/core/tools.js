...
/**
 * @fileOverview Defines the {@link OMEDITOR.tools} object, which contains
 *		utility functions.
 */
...
(function()
{
	...
	OMEDITOR.tools =
	{
		 ...
		 * var d = [ 1, 'a', 3, 4 ];
		 *
		 * alert( OMEDITOR.tools.arrayCompare( a, b ) );  // false
		 * alert( OMEDITOR.tools.arrayCompare( a, c ) );  // true
		 * alert( OMEDITOR.tools.arrayCompare( a, d ) );  // false
		 */
		 ...
		 *             }
		 *     };
		 * var clone = OMEDITOR.tools.clone( obj );
		 * clone.name = 'Paul';
		 ...
		 * OMEDITOR.tools.extend( myObject,
		 *     {
		 *         prop2 : true,
		 *         prop3 : true
		 *     } );
		 ...
		 * @example
		 * alert( OMEDITOR.tools.isArray( [] ) );      // "true"
		 * alert( OMEDITOR.tools.isArray( 'Test' ) );  // "false"
		 */
		 ...
		 * @example
		 * alert( OMEDITOR.tools.cssStyleToDomStyle( 'background-color' ) );  // "backgroundColor"
		 * alert( OMEDITOR.tools.cssStyleToDomStyle( 'float' ) );             // "cssFloat"
		 */
		 ...
		 * @example
		 * alert( OMEDITOR.tools.htmlEncode( 'A > B & C < D' ) );  // "A &amp;gt; B &amp;amp; C &amp;lt; D"
		 */
		htmlEncode : function( text )
		{
			var standard = function( text )
			{
				var span = new OMEDITOR.dom.element( 'span' );
				span.setText( text );
				return span.getHtml();
			};
		 ...
		 * alert( OMEDITOR.tools.htmlEncodeAttr( element.getAttribute( 'title' ) );  // "&gt;a &quot; b &lt;"
		 */
		...
		/**
		 * Gets a unique number for this OMEDITOR execution session. It returns
		 * progressive numbers starting at 1.
		 * @function
		 * @returns {Number} A unique number.
		 * @example
		 * alert( OMEDITOR.tools.<b>getNextNumber()</b> );  // "1" (e.g.)
		 * alert( OMEDITOR.tools.<b>getNextNumber()</b> );  // "2"
		 */
		...
		/**
		 * Gets a unique ID for OMEditor's interface elements. It returns a
		 * string with the "cke_" prefix and a progressive number.
		 * @function
		 * @returns {String} A unique ID.
		 * @example
		 * alert( OMEDITOR.tools.<b>getNextId()</b> );  // "cke_1" (e.g.)
		 * alert( OMEDITOR.tools.<b>getNextId()</b> );  // "cke_2"
		 */
		getNextId : function()
		{
		 ...
		 * };
		 *
		 * example.myFunction = OMEDITOR.tools.override( example.myFunction, function( myFunctionOriginal )
		 *     {
		 ...
		 * @example
		 * alert( OMEDITOR.tools.trim( '  example ' );  // "example"
		 */
		trim : (function()
		{
		 ...
		 * @example
		 * alert( OMEDITOR.tools.ltrim( '  example ' );  // "example "
		 */
		ltrim : (function()
		{
			// We are not using \s because we don't want "non-breaking spaces" to be caught.
		 ...
		 * alert( OMEDITOR.tools.indexOf( letters, '0' ) );  "-1" because 0 !== '0'
		 * alert( OMEDITOR.tools.indexOf( letters, false ) );  "4" because 0 !== false
		 */
		indexOf :
		 ...
		 * }
		 *
		 * var newFunc = <b>OMEDITOR.tools.bind( alertText, obj )</b>;
			...
			if ( privates )
			{
				...
				$ = function()
				{
					...
					for ( var privateName in privates )
					{
						...
						_[ privateName ] =
							( typeof priv == 'function' ) ? OMEDITOR.tools.bind( priv, this ) : priv;
					}
		 ...
		 * OMEDITOR.tools.callFunction. This approach is specially useful to
		 * make DOM attribute function calls to JavaScript defined functions.
		 * @param {Function} fn The function to be executed on call.
		 * @param {Object} [scope] The object to have the context on "fn" execution.
		 * @returns {Number} A unique reference to be used in conjuction with
		 *		OMEDITOR.tools.callFunction.
		 * @example
		 * var ref = <b>OMEDITOR.tools.addFunction</b>(
		 *     function()
		 *     {
		 *         alert( 'Hello!');
		 *     });
		 * OMEDITOR.tools.callFunction( ref );  // Hello!
		 */
		...
		/**
		 * Removes the function reference created with {@see OMEDITOR.tools.addFunction}.
		 * @param {Number} ref The function reference created with
		 *		OMEDITOR.tools.addFunction.
		 */
		 ...
		 * @example
		 * var ref = OMEDITOR.tools.addFunction(
		 *     function()
		 *     {
		 *         alert( 'Hello!');
		 *     });
		 * <b>OMEDITOR.tools.callFunction( ref )</b>;  // Hello!
		 */
		...
		convertToPx : ( function ()
			{
				...
				return function( cssLength )
				{
					if ( !calculator )
					{
						calculator = OMEDITOR.dom.element.createFromHtml(
								'<div style="position:absolute;left:-9999px;' +
								'top:-9999px;margin:0px;padding:0px;border:0px;"' +
								'></div>', OMEDITOR.document );
		 ...
		 * @example
		 * var key = OMEDITOR.tools.genKey( 'key1', 'key2', 'key3' );
...
// PACKAGER_RENAME( OMEDITOR.tools )
