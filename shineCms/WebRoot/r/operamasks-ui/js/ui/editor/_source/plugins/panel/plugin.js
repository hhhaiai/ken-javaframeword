...
OMEDITOR.plugins.add( 'panel',
{
	beforeInit : function( editor )
	{
		editor.ui.addHandler( OMEDITOR.UI_PANEL, OMEDITOR.ui.panel.handler );
	}
});
...
OMEDITOR.ui.panel = function( document, definition )
{
...
/**
 * Transforms a rich combo definition in a {@link OMEDITOR.ui.richCombo}
 * instance.
 * @type Object
 * @example
 */
OMEDITOR.ui.panel.handler =
{
	create : function( definition )
	{
		return new OMEDITOR.ui.panel( definition );
	}
};

OMEDITOR.ui.panel.prototype =
{
	...
	getHolderElement : function()
	{
		...
		if ( !holder )
		{
			if ( this.forceIFrame || this.css.length )
			{
				...
				var onLoad = OMEDITOR.tools.addFunction( OMEDITOR.tools.bind( function( ev )
					{
						this.isLoaded = true;
						if ( this.onLoad )
							this.onLoad();
				...
				var data =
					...
					'<html dir="' + dir + '" class="' + className + '_container" lang="' + langCode + '">' +
						...
						'</head>' +
						'<body class="cke_' + dir + ' cke_panel_frame ' + OMEDITOR.env.cssClass + '" style="margin:0;padding:0"' +
						' onload="( window.OMEDITOR || window.parent.OMEDITOR ).tools.callFunction(' + onLoad + ');"></body>' +
						// It looks strange, but for FF2, the styles must go
						// after <body>, so it (body) becames immediatelly
						// available. (#3031)
						OMEDITOR.tools.buildStyleHtml( this.css ) +
					'<\/html>';
				...
				// Register the OMEDITOR global.
				win.$.OMEDITOR = OMEDITOR;

				// Arrow keys for scrolling is only preventable with 'keypress' event in Opera (#4534).
				doc.on( 'key' + ( OMEDITOR.env.opera? 'press':'down' ), function( evt )
					{
				...
				holder = doc.getBody();
				holder.unselectable();
				OMEDITOR.env.air && OMEDITOR.tools.callFunction( onLoad );
			}
			else
	...
	{
		block = this._.blocks[ name ] = block instanceof OMEDITOR.ui.panel.block ?  block
				: new OMEDITOR.ui.panel.block( this.getHolderElement(), block );
	...
	showBlock : function( name )
	{
		...
		OMEDITOR.fire( 'ariaWidget', holder );

		// Reset the focus index, so it will always go into the first one.
		block._.focusIndex = -1;

		this._.onKeyDown = block.onKeyDown && OMEDITOR.tools.bind( block.onKeyDown, block );
...
OMEDITOR.ui.panel.block = OMEDITOR.tools.createClass(
{
	$ : function( blockHolder, blockDefinition )
	{
		...
		// Copy all definition properties to this object.
		if ( blockDefinition )
			OMEDITOR.tools.extend( this, blockDefinition );
	...
	_ : {
		...
		markItem: function( index )
		{
			...
			if ( OMEDITOR.env.webkit || OMEDITOR.env.opera )
