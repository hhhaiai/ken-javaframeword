...
(function()
{
	...
	function loadValue( objectNode, embedNode, paramMap )
	{
		...
		var isCheckbox = ( this instanceof OMEDITOR.ui.dialog.checkbox );
	...
	function commitValue( objectNode, embedNode, paramMap )
	{
		...
		var isRemove = ( this.getValue() === '' ),
			isCheckbox = ( this instanceof OMEDITOR.ui.dialog.checkbox );
			...
			switch ( attrDef.type )
			{
				...
				case ATTRTYPE_PARAM:
					...
					else
					{
						...
						{
							var param = OMEDITOR.dom.element.createFromHtml( '<cke:param></cke:param>', objectNode.getDocument() );
	...
	OMEDITOR.dialog.add( 'flash', function( editor )
	{
		...
		var previewPreloader,
			previewAreaHtml = '<div>' + OMEDITOR.tools.htmlEncode( editor.lang.common.preview ) +'<br>' +
			'<div id="cke_FlashPreviewLoader' + OMEDITOR.tools.getNextNumber() + '" style="display:none"><div class="loading">&nbsp;</div></div>' +
			'<div id="cke_FlashPreviewBox' + OMEDITOR.tools.getNextNumber() + '" class="FlashPreviewBox"></div></div>';
		...
		return {
			...
			onShow : function()
			{
				...
				previewPreloader = new OMEDITOR.dom.element( 'embed', editor.document );
			...
			onOk : function()
			{
				...
				if ( !this.fakeImage )
				{
					if ( makeObjectTag )
					{
						objectNode = OMEDITOR.dom.element.createFromHtml( '<cke:object></cke:object>', editor.document );
						var attributes = {
					...
					if ( makeEmbedTag )
					{
						embedNode = OMEDITOR.dom.element.createFromHtml( '<cke:embed></cke:embed>', editor.document );
			...
			contents : [
									...
									[
										{
											...
											required : true,
											validate : OMEDITOR.dialog.validate.notEmpty( editor.lang.flash.validateSrc ),
											setup : loadValue,
											commit : commitValue,
											onLoad : function()
											{
												...
												updatePreview = function( src ){
													...
													dialog.preview.setHtml( '<embed height="100%" width="100%" src="'
														+ OMEDITOR.tools.htmlEncode( previewPreloader.getAttribute( 'src' ) )
							...
							[
								{
									...
									validate : OMEDITOR.dialog.validate.htmlLength( editor.lang.common.invalidHtmlLength.replace( '%1', editor.lang.common.width ) ),
									setup : loadValue,
									commit : commitValue
								},
								{
									...
									validate : OMEDITOR.dialog.validate.htmlLength( editor.lang.common.invalidHtmlLength.replace( '%1', editor.lang.common.height ) ),
									setup : loadValue,
									commit : commitValue
								},
								{
									...
									validate : OMEDITOR.dialog.validate.integer( editor.lang.flash.validateHSpace ),
									setup : loadValue,
									commit : commitValue
								},
								{
									...
									validate : OMEDITOR.dialog.validate.integer( editor.lang.flash.validateVSpace ),
									setup : loadValue,
									commit : commitValue
								}
							]
				...
				},
				{
					...
					[
						...
						{
							type : 'fieldset',
							label : OMEDITOR.tools.htmlEncode( editor.lang.flash.flashvars ),
							children :
							[
								{
									type : 'vbox',
