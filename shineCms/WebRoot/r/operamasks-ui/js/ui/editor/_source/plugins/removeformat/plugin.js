...
OMEDITOR.plugins.add( 'removeformat',
{
	requires : [ 'selection' ],

	init : function( editor )
	{
		editor.addCommand( 'removeFormat', OMEDITOR.plugins.removeformat.commands.removeformat );
	...
	commands :
	{
		removeformat :
		{
			exec : function( editor )
			{
				...
				var filter = OMEDITOR.plugins.removeformat.filter;
				...
				{
					if ( ! range.collapsed )
						range.enlarge( OMEDITOR.ENLARGE_ELEMENT );
					...
					if ( endNode )
					{
						...
						currentNode = startNode.getNextSourceNode( true, OMEDITOR.NODE_ELEMENT );
						...
						while ( currentNode )
						{
							...
							var nextNode = currentNode.getNextSourceNode( false, OMEDITOR.NODE_ELEMENT );
 ...
 * @since 3.3
 * @param {Function} func The function to be called, which will be passed a {OMEDITOR.dom.element} element to test.
 * @example
 *  // Don't remove empty span
 *  editor.addRemoveFormatFilter.push( function( element )
 *		{
 *			return !( element.is( 'span' ) && OMEDITOR.tools.isEmpty( element.getAttributes() ) );
 *		});
 */
OMEDITOR.editor.prototype.addRemoveFormatFilter = function( func )
{
 ...
 * @example
 */
OMEDITOR.config.removeFormatTags = 'b,big,code,del,dfn,em,font,i,ins,kbd,q,samp,small,span,strike,strong,sub,sup,tt,u,var';
 ...
 * @example
 */
OMEDITOR.config.removeFormatAttributes = 'class,style,lang,width,height,align,hspace,valign';

/**
 * Fired after an element was cleaned by the removeFormat plugin.
 * @name OMEDITOR.editor#removeFormatCleanup
 * @event
