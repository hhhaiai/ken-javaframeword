...
*/
OMEDITOR.dialog.add( 'button', function( editor )
{
	...
	return {
		...
		onOk : function()
		{
			...
			var fake = element ? OMEDITOR.htmlParser.fragment.fromHtml( element.getOuterHtml() ).children[ 0 ]
					: new OMEDITOR.htmlParser.element( 'input' );
			this.commitContent( fake );

			var writer = new OMEDITOR.htmlParser.basicWriter();
			fake.writeHtml( writer );
			var newElement = OMEDITOR.dom.element.createFromHtml( writer.getHtml(), editor.document );
