...
(function()
{
	...
	function parseClasses( aRules, skipSelectors, validSelectors )
	{
		...
		for ( var i = 0; i < aSelectors.length ; i++ )
		{
			...
			if ( validSelectors.test( selector ) && !skipSelectors.test( selector ) )
			{
				...
				if ( OMEDITOR.tools.indexOf( aClasses, selector ) == -1 )
	...
	{
		var styles = [],
			// It will hold all the rules of the applied stylesheets (except those internal to OMEditor)
			aRules = [],
			i;
	...
	OMEDITOR.plugins.add( 'stylesheetparser',
	{
		requires: [ 'styles' ],
		onLoad : function()
		{
			var obj = OMEDITOR.editor.prototype;
			obj.getStylesSet = OMEDITOR.tools.override( obj.getStylesSet,  function( org )
			{
 ...
 * @name OMEDITOR.config.stylesheetParser_skipSelectors
 * @type RegExp
 * @default /(^body\.|^\.)/i
 * @since 3.6
 * @see OMEDITOR.config.stylesheetParser_validSelectors
 * @example
 ...
 * @name OMEDITOR.config.stylesheetParser_validSelectors
 * @type RegExp
 * @default /\w+\.\w+/
 * @since 3.6
 * @see OMEDITOR.config.stylesheetParser_skipSelectors
 * @example
