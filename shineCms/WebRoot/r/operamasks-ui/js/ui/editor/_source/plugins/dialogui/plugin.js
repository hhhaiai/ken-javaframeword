...
(function()
{
	var initPrivateObject = function( elementDefinition )
	{
		...
		args.push( true );
		OMEDITOR.tools.extend.apply( OMEDITOR.tools, args );
		return this._;
	},
	textBuilder =
	{
		...
		{
			return new OMEDITOR.ui.dialog.textInput( dialog, elementDefinition, output );
		}
	},
	commonBuilder =
	{
		build : function( dialog, elementDefinition, output )
		{
			return new OMEDITOR.ui.dialog[elementDefinition.type]( dialog, elementDefinition, output );
		}
	},
	containerBuilder =
	{
	...
	commonEventProcessors = OMEDITOR.tools.extend( {}, OMEDITOR.ui.dialog.uiElement.prototype.eventProcessors,
		{
	...
	OMEDITOR.tools.extend( OMEDITOR.ui.dialog,
		/** @lends OMEDITOR.ui.dialog */
		{
			/**
			...
			labeledElement : function( dialog, elementDefinition, htmlList, contentHtml )
			{
				...
				_.labelId = OMEDITOR.tools.getNextId() + '_label';
				...
				var innerHTML = function()
				{
					...
					else
					{
							...
							[
								{
									...
									html : '<label class="cke_dialog_ui_labeled_label' + requiredClass + '"' +
										...
										' for="' + _.inputId + '"' +
										( elementDefinition.labelStyle ? ' style="' + elementDefinition.labelStyle + '"' : '' ) +'>' +
										   OMEDITOR.tools.htmlEncode( elementDefinition.label ) +
										'</span>'
								},
								{
									type : 'html',
						...
						OMEDITOR.dialog._.uiElementBuilders.hbox.build( dialog, hboxDefinition, html );
					}
					return html.join( '' );
				};
				OMEDITOR.ui.dialog.uiElement.call( this, dialog, elementDefinition, htmlList, 'div', null, { role : 'presentation' }, innerHTML );
			},
			 ...
			 * @constructor
			 * @example
			 * @extends OMEDITOR.ui.dialog.labeledElement
			 * @param {OMEDITOR.dialog} dialog
			 * Parent dialog object.
			 * @param {OMEDITOR.dialog.definition.uiElement} elementDefinition
			 * The element definition. Accepted fields:
			...
			textInput : function( dialog, elementDefinition, htmlList )
			{
				...
				var domId = this._.inputId = OMEDITOR.tools.getNextId() + '_textInput',
				...
				OMEDITOR.ui.dialog.labeledElement.call( this, dialog, elementDefinition, htmlList, innerHTML );
			},
			...
			textarea : function( dialog, elementDefinition, htmlList )
			{
				...
				initPrivateObject.call( this, elementDefinition );
				var me = this,
					domId = this._.inputId = OMEDITOR.tools.getNextId() + '_textarea',
					attributes = {};
				...
				var innerHTML = function()
				{
					...
					html.push( '>', OMEDITOR.tools.htmlEncode( me._['default'] ), '</textarea></div>' );
					return html.join( '' );
				};
				OMEDITOR.ui.dialog.labeledElement.call( this, dialog, elementDefinition, htmlList, innerHTML );
			},
			...
			checkbox : function( dialog, elementDefinition, htmlList )
			{
				...
				var innerHTML = function()
				{
					var myDefinition = OMEDITOR.tools.extend( {}, elementDefinition,
							{
								id : elementDefinition.id ? elementDefinition.id + '_checkbox' : OMEDITOR.tools.getNextId() + '_checkbox'
							}, true ),
						html = [];

					var labelId = OMEDITOR.tools.getNextId() + '_label';
					...
					_.checkbox = new OMEDITOR.ui.dialog.uiElement( dialog, myDefinition, html, 'input', null, attributes );
				...
				OMEDITOR.ui.dialog.uiElement.call( this, dialog, elementDefinition, htmlList, 'span', null, null, innerHTML );
			},
			...
			radio : function( dialog, elementDefinition, htmlList )
			{
				...
				var innerHTML = function()
				{
					var inputHtmlList = [], html = [],
						...
						commonName = elementDefinition.id ? elementDefinition.id + '_radio' : OMEDITOR.tools.getNextId() + '_radio';
					for ( var i = 0 ; i < elementDefinition.items.length ; i++ )
					{
						var item = elementDefinition.items[i],
							title = item[2] !== undefined ? item[2] : item[0],
							value = item[1] !== undefined ? item[1] : item[0],
							inputId = OMEDITOR.tools.getNextId() + '_radio_input',
							labelId = inputId + '_label',
							inputDefinition = OMEDITOR.tools.extend( {}, elementDefinition,
									{
										id : inputId,
										title : null,
										type : null
									}, true ),
							labelDefinition = OMEDITOR.tools.extend( {}, inputDefinition,
									{
										title : title
									}, true ),
							inputAttributes =
						...
						children.push( new OMEDITOR.ui.dialog.uiElement( dialog, inputDefinition, inputHtml, 'input', null, inputAttributes ) );
						inputHtml.push( ' ' );
						new OMEDITOR.ui.dialog.uiElement( dialog, labelDefinition, inputHtml, 'label', null, { id : labelId, 'for' : inputAttributes.id },
							   item[0] );
						inputHtmlList.push( inputHtml.join( '' ) );
					}
					new OMEDITOR.ui.dialog.hbox( dialog, [], inputHtmlList, html );
					return html.join( '' );
				};
			...
			button : function( dialog, elementDefinition, htmlList )
			{
				...
				var outerDefinition = OMEDITOR.tools.extend( {}, elementDefinition );
			...
			select : function( dialog, elementDefinition, htmlList )
			{
				...
				_.inputId = OMEDITOR.tools.getNextId() + '_select';
				/** @ignore */
				var innerHTML = function()
				{
					var myDefinition = OMEDITOR.tools.extend( {}, elementDefinition,
							{
								id : elementDefinition.id ? elementDefinition.id + '_select' : OMEDITOR.tools.getNextId() + '_select'
							}, true ),
						html = [],
						innerHTML = [],
					...
					for ( var i = 0, item ; i < elementDefinition.items.length && ( item = elementDefinition.items[i] ) ; i++ )
					{
						innerHTML.push( '<option value="',
							OMEDITOR.tools.htmlEncode( item[1] !== undefined ? item[1] : item[0] ).replace( /"/g, '&quot;' ), '" /> ',
							OMEDITOR.tools.htmlEncode( item[0] ) );
					}

					if ( typeof myDefinition.inputStyle != 'undefined' )
						myDefinition.style = myDefinition.inputStyle;

					_.select = new OMEDITOR.ui.dialog.uiElement( dialog, myDefinition, html, 'select', null, attributes, innerHTML.join( '' ) );
					return html.join( '' );
				};
			...
			file : function( dialog, elementDefinition, htmlList )
			{
				...
				var _ = OMEDITOR.tools.extend( initPrivateObject.call( this, elementDefinition ), { definition : elementDefinition, buttons : [] } );
				...
				var innerHTML = function()
				{
					_.frameId = OMEDITOR.tools.getNextId() + '_fileInput';
				...
				dialog.on( 'load', function()
					{
						var iframe = OMEDITOR.document.getById( _.frameId ),
			 ...
			 * @extends OMEDITOR.ui.dialog.button
			 * @example
			 * @constructor
			 * @param {OMEDITOR.dialog} dialog
			 * Parent dialog object.
			...
			fileButton : function( dialog, elementDefinition, htmlList )
			{
				...
				OMEDITOR.ui.dialog.button.call( this, dialog, myDefinition, htmlList );
			},
			...
			html : (function()
			{
				 ...
				 * @extends OMEDITOR.ui.dialog.uiElement
				 * @name OMEDITOR.ui.dialog.html
				 * @param {OMEDITOR.dialog} dialog Parent dialog object.
				 * @param {OMEDITOR.dialog.definition.uiElement} elementDefinition Element definition.
				 * Accepted fields:
				 * <ul>
				...
				return function( dialog, elementDefinition, htmlList )
				{
					...
					OMEDITOR.ui.dialog.uiElement.call( this, dialog, elementDefinition, myHtmlList, 'span', null, null, '' );
			...
			fieldset : function( dialog, childObjList, childHtmlList, htmlList, elementDefinition )
			{
				...
				OMEDITOR.ui.dialog.uiElement.call( this, dialog, elementDefinition, htmlList, 'fieldset', null, null, innerHTML );
			}

		}, true );

	OMEDITOR.ui.dialog.html.prototype = new OMEDITOR.ui.dialog.uiElement;

	OMEDITOR.ui.dialog.labeledElement.prototype = OMEDITOR.tools.extend( new OMEDITOR.ui.dialog.uiElement,
			/** @lends OMEDITOR.ui.dialog.labeledElement.prototype */
			{
				/**
				 * Sets the label text of the element.
				 * @param {String} label The new label text.
				 * @returns {OMEDITOR.ui.dialog.labeledElement} The current labeled element.
				 * @example
				 */
				setLabel : function( label )
				{
					var node = OMEDITOR.document.getById( this._.labelId );
					if ( node.getChildCount() < 1 )
						( new OMEDITOR.dom.text( label, OMEDITOR.document ) ).appendTo( node );
					else
	...
	OMEDITOR.ui.dialog.button.prototype = OMEDITOR.tools.extend( new OMEDITOR.ui.dialog.uiElement,
			/** @lends OMEDITOR.ui.dialog.button.prototype */
			{
				/**
				 ...
				 * @example
				 */
				eventProcessors : OMEDITOR.tools.extend( {}, OMEDITOR.ui.dialog.uiElement.prototype.eventProcessors,
					{
						/** @ignore */
	...
	OMEDITOR.ui.dialog.textInput.prototype = OMEDITOR.tools.extend( new OMEDITOR.ui.dialog.labeledElement,
			/** @lends OMEDITOR.ui.dialog.textInput.prototype */
			{
				/**
				 * Gets the text input DOM element under this UI object.
				 * @example
				 * @returns {OMEDITOR.dom.element} The DOM element of the text input.
				 */
				getInputElement : function()
				{
					return OMEDITOR.document.getById( this._.inputId );
				},
				 ...
				 * @returns {OMEDITOR.ui.dialog.textInput} The current UI element.
				 * @example
				...
				{
					!value && ( value = '' );
					return OMEDITOR.ui.dialog.uiElement.prototype.setValue.apply( this, arguments );
				},

				keyboardFocusable : true
			}, commonPrototype, true );

	OMEDITOR.ui.dialog.textarea.prototype = new OMEDITOR.ui.dialog.textInput();

	OMEDITOR.ui.dialog.select.prototype = OMEDITOR.tools.extend( new OMEDITOR.ui.dialog.labeledElement,
			/** @lends OMEDITOR.ui.dialog.select.prototype */
			{
				/**
				 * Gets the DOM element of the select box.
				 * @returns {OMEDITOR.dom.element} The &lt;select&gt; element of this UI
				 * element.
				 * @example
				 */
				getInputElement : function()
				 ...
				 * @example
				 * @returns {OMEDITOR.ui.dialog.select} The current select UI element.
				 */
				add : function( label, value, index )
				{
					var option = new OMEDITOR.dom.element( 'option', this.getDialog().getParentEditor().document ),
	...
	OMEDITOR.ui.dialog.checkbox.prototype = OMEDITOR.tools.extend( new OMEDITOR.ui.dialog.uiElement,
			/** @lends OMEDITOR.ui.dialog.checkbox.prototype */
			{
				/**
				 * Gets the checkbox DOM element.
				 * @example
				 * @returns {OMEDITOR.dom.element} The DOM element of the checkbox.
				 */
				getInputElement : function()
				{
	...
	OMEDITOR.ui.dialog.radio.prototype = OMEDITOR.tools.extend( new OMEDITOR.ui.dialog.uiElement,
			/** @lends OMEDITOR.ui.dialog.radio.prototype */
			{
				/**
	...
	OMEDITOR.ui.dialog.file.prototype = OMEDITOR.tools.extend( new OMEDITOR.ui.dialog.labeledElement,
			commonPrototype,
			/** @lends OMEDITOR.ui.dialog.file.prototype */
			{
				/**
				 * Gets the &lt;input&gt; element of this file input.
				 * @returns {OMEDITOR.dom.element} The file input element.
				 * @example
				 */
				getInputElement : function()
				{
					var frameDocument = OMEDITOR.document.getById( this._.frameId ).getFrameDocument();
					return frameDocument.$.forms.length > 0 ?
						new OMEDITOR.dom.element( frameDocument.$.forms[0].elements[0] ) :
						this.getElement();
				},

				/**
				 * Uploads the file in the file input.
				 * @returns {OMEDITOR.ui.dialog.file} This object.
				 * @example
				 */
				submit : function()
				{
				...
				{
					var _ = this._,
						frameElement = OMEDITOR.document.getById( _.frameId ),
					...
					if ( !callNumber )
					{
						callNumber = this.formLoadedNumber = OMEDITOR.tools.addFunction(
							function()
							{
						...
						unloadNumber = this.formUnloadNumber = OMEDITOR.tools.addFunction(
							function()
							{
						...
						this.getDialog()._.editor.on( 'destroy', function()
								{
									OMEDITOR.tools.removeFunction( callNumber );
									OMEDITOR.tools.removeFunction( unloadNumber );
								} );
					}
					...
					function generateFormField()
					{
						...
						var size = '';
						if ( elementDefinition.size )
							size = elementDefinition.size - ( OMEDITOR.env.ie  ? 7 : 0 );	// "Browse" button is bigger in IE.
						...
						frameDocument.$.write( [ '<html dir="' + langDir + '" lang="' + langCode + '"><head><title></title></head><body style="margin: 0; overflow: hidden; background: transparent;">',
								...
								OMEDITOR.tools.htmlEncode( elementDefinition.action ),
								'">',
								'<input type="file" name="',
								OMEDITOR.tools.htmlEncode( elementDefinition.id || 'cke_upload' ),
								'" size="',
								OMEDITOR.tools.htmlEncode( size > 0 ? size : "" ),
								'" />',
								'</form>',
								'</body></html>',
								'<script>window.parent.OMEDITOR.tools.callFunction(' + callNumber + ');',
								'window.onbeforeunload = function() {window.parent.OMEDITOR.tools.callFunction(' + unloadNumber + ')}</script>' ].join( '' ) );
	...
	OMEDITOR.ui.dialog.fileButton.prototype = new OMEDITOR.ui.dialog.button;

	OMEDITOR.ui.dialog.fieldset.prototype = OMEDITOR.tools.clone( OMEDITOR.ui.dialog.hbox.prototype );

	OMEDITOR.dialog.addUIElement( 'text', textBuilder );
	OMEDITOR.dialog.addUIElement( 'password', textBuilder );
	OMEDITOR.dialog.addUIElement( 'textarea', commonBuilder );
	OMEDITOR.dialog.addUIElement( 'checkbox', commonBuilder );
	OMEDITOR.dialog.addUIElement( 'radio', commonBuilder );
	OMEDITOR.dialog.addUIElement( 'button', commonBuilder );
	OMEDITOR.dialog.addUIElement( 'select', commonBuilder );
	OMEDITOR.dialog.addUIElement( 'file', commonBuilder );
	OMEDITOR.dialog.addUIElement( 'fileButton', commonBuilder );
	OMEDITOR.dialog.addUIElement( 'html', commonBuilder );
	OMEDITOR.dialog.addUIElement( 'fieldset', containerBuilder );
})();

/**
 * Fired when the value of the uiElement is changed
 * @name OMEDITOR.ui.dialog.uiElement#change
 * @event
 */
 ...
 * @name OMEDITOR.ui.dialog.fileButton#formLoaded
 * @event
 */
