...
OMEDITOR.dialog.add( 'smiley', function( editor )
{
	...
	var onKeydown = OMEDITOR.tools.addFunction( function( ev, element )
	{
		ev = new OMEDITOR.dom.event( ev );
		element = new OMEDITOR.dom.element( element );
	...
	var labelId = OMEDITOR.tools.getNextId() + '_smiley_emtions_label';
	var html =
	[
		'<div>' +
		...
		OMEDITOR.env.ie && OMEDITOR.env.quirks ? ' style="position:absolute;"' : '',
		'><tbody>'
	];
	...
	for ( i = 0 ; i < size ; i++ )
	{
		...
		var smileyLabelId = 'cke_smile_label_' + i + '_' + OMEDITOR.tools.getNextNumber();
		html.push(
			'<td class="cke_dark_background cke_centered" style="vertical-align: middle;">' +
				'<a href="javascript:void(0)" role="option"',
					...
					' class="cke_smile cke_hand" tabindex="-1" onkeydown="OMEDITOR.tools.callFunction( ', onKeydown, ', event, this );">',
					'<img class="cke_hand" title="', config.smiley_descriptions[i], '"' +
						' cke_src="', OMEDITOR.tools.htmlEncode( config.smiley_path + images[ i ] ), '" alt="', config.smiley_descriptions[i], '"',
						' src="', OMEDITOR.tools.htmlEncode( config.smiley_path + images[ i ] ), '"',
						// IE BUG: Below is a workaround to an IE image loading bug to ensure the image sizes are correct.
						( OMEDITOR.env.ie ? ' onload="this.setAttribute(\'width\', 2); this.removeAttribute(\'width\');" ' : '' ),
					'>' +
