/*
 * $Id: demo.js,v 1.37 2012/01/06 07:48:35 licongping Exp $
    ...
    var _privateMethods = {
        initNavTree : function() {
            ...
            for ( var i = 0, len = categorys.length; i < len; i++) {
                ...
                for ( var j = 0, cs = c.comps, len2 = cs.length; j < len2; j++) {
                    cComps[j] = '<dd><a href="#" __index="' + i + '#' + j + '" __name="'+cs[j].name+'" class="component"><span class="plus">+</span>' + cs[j].name + '</a>';
                    var samples = cs[j].samples;
                    cComps[j] += '<ul>';
                    for (var k = 0; k < samples.length; k++){
                    	var s = samples[k];
                    	cComps[j] += '<li><a href="#" __height="'+(s.height || 400)+'"  __href="'+s.url+ (s.java?'" __java="'+s.java+'">':'">')+s.title+'</a></li>';
                    }
                    cComps[j] += '</ul></dd>';
                }
                var dds = cComps.length == 0 ? '' : cComps.join('');
                html += '<dt class="category">' + cName + '<div class="trigger"></dt><div class="components">' + dds+'</div>';
            }
            ...
            compLink.click().next().find('a:first').click();
        },
        initNavHandler : function() {
            ...
            allLinks.click(function(ev) {
                        ...
                        location.href = href.split('#')[0] + '#' + compName;
                        ev.preventDefault();
                        ev.stopPropagation();
                        return false;
                    });
