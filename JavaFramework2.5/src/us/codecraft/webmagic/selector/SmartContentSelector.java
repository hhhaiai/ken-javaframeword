...
/**
 * readability算法，基础是找到所有p标签的父节点 写的比较乱，最终效果还在尝试中
 * 
 * @author code4crafter@gmail.com <br>
 *         Date: 13-4-21 Time: 下午4:42
 */
public class SmartContentSelector implements Selector {
	...
	public String select(String text) {
		...
		try {
			tagNode = htmlCleaner.clean(text);
		} catch (IOException e) {
		...
		for (Map.Entry<TagNode, Double> entry : pDensityCountMap.entrySet()) {
			...
			// logger.debug("p\t" + entry.getKey().getName() + "#" +
			// entry.getKey().getAttributeByName("id") +
			// "@" + entry.getKey().getAttributeByName("class") + ":" +
			// entry.getValue());
			// }
			sortList.add(entry);
		}
		...
		Collections.sort(sortList,
				new Comparator<Map.Entry<TagNode, Double>>() {
					@Override
					public int compare(Map.Entry<TagNode, Double> o1,
							Map.Entry<TagNode, Double> o2) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("p\t" + contentNode.getName() + "#"
					+ contentNode.getAttributeByName("id") + "@"
					+ contentNode.getAttributeByName("class"));
		}
		return htmlCleaner.getInnerHtml(contentNode);
	}

	private void addCounter(TagNode node, Map<TagNode, Double> countMap,
			double delta) {
	...
	private void countPdensity(TagNode[] nodes,
			Map<TagNode, Double> pDensityCountMap) {
	...
	private TagNode findLowestCommonParent(List<TagNode> tagNodes,
			int maxMargin, Map<TagNode, AtomicInteger> countMap) {
